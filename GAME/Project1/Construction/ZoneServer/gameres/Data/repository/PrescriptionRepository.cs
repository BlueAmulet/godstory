...
{	
	//直接给道具
	if(AddItemToInventory(%Player.GetPlayerID(), GetPrescriptionData(%PresId, 22), GetPrescriptionData(%PresId, 24)))
		{			
			//添加生活技能熟练度
...
function Produce_GetItem(%ItemList_Purple,%ItemList_Blue,%ItemList_Green,%ItemList_White,%OddsList_Purple,%OddsList_Blue,%OddsList_Green,%OddsList_White,%Odds_ColorList,%OddsModeList)
{
...
//	%WhiteOddsListCount 	= getWordCount(%OddsList_White);		//白色几率数量
	...
	%WhiteOdds 	= getWord(%Odds_ColorList , 0);//白色几率
	%GreenOdds 	= getWord(%Odds_ColorList , 1);//绿色几率
	%BlueOdds 		= getWord(%Odds_ColorList , 2);//蓝色几率
	%PurpleOdds 	= getWord(%Odds_ColorList , 3);//紫色几率
	
	%tmpOdds 		= %WhiteOdds + %GreenOdds + %BlueOdds + %PurpleOdds;
    echo("几率总和:" @ %tmpOdds);
...
//		{
			...
			{
				if(%rndResult > %PurpleOdds && %rndResult <= (%BlueOdds + %PurpleOdds))
				{
					//echo("蓝色道具随机.");
			...
			{
				if(%rndResult > (%BlueOdds + %PurpleOdds) && %rndResult <= (%BlueOdds + %PurpleOdds + %GreenOdds))
				{
					//echo("绿色道具随机.");
      ...
      {
	      if(%rndResult > (%BlueOdds + %PurpleOdds + %GreenOdds) && %rndResult <= (%BlueOdds + %PurpleOdds + %GreenOdds + %WhiteOdds))
	      {
...
function ProduceGetRndItem(%ItemList , %OddsList)
{
	...
	if(%tmpOddsList != %tmpItemCount)
	{
        ...
        echo("几率:" @ %rndResult);
		return getWord(%ItemList , %rndResult);
	}
	else
	{
		//获取总几率
...
//特殊合成公式
function Produce_Make_Special(%Player, %PresId)
{
}
...
//	
//	//计算方法设定
//	//总几率 | 几率计算方式 | 计算方式 | 没几率必出的颜色
//	//总几率：
//	//几率计算方式.	0 : 由已定义的总几率决定；1：由道具几率总和决定.
//	//计算方式.	0：至少出现最差的;1：按照几率计算.
...
//	
//	if(strlen(%ResultItem) != 9 || %ResultItem != 0)
//	{
//		//错误，这里进行错误处理
//		//返回错误编码，由客户端显示错误文字，编码查看
//		//FR客户端\SXGame\gameres\client\SysString.ini
//		
//		return 4103;
//	}
//	
//	//这里返回为0的原因：几率设置时，没有设置默认颜色几率，导致可能未随机到任何一件道具，合成成功，但没任何道具。
//	//可以做特殊处理，或者其它。。。。
//	if(%ResultItem == 0)
//	{
//		return 4103;
...
////	%ItemAdd = %Player.PutItem(%ResultItem, 1);
////	%ItemAdd = %Player.AddItem();
//	
//	//return 0;	//制作成功
//	
////	%resultCode = Produce_GetResult(%ItemList_Purple,%ItemList_Blue,%ItemList_Green,%ItemList_White,%OddsList_Purple,%OddsList_Blue,%OddsList_Green,%OddsList_White,%Odds_ColorList,%OddsModeList);
////	return %resultCode;
//}
...
function Produce_630000001(%Player, %PresId)
{
	...
	if(strlen(%ResultItem) != 9 || %ResultItem != 0) {return 4103;}
...
function Produce_630202001(%Player, %PresId)
{
	...
	%Odds_ColorList = "7000 3000 0 0";

	%OddsModeList = "10000 0";
...
function Produce_630206001(%Player, %PresId)
{
	...
	%Odds_ColorList = "100 8000 1900 0";
