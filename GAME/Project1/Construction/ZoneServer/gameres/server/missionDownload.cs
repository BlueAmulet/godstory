...
function serverCmdMissionStartPhase2Ack(%client, %seq, %playerDB, %crc )
{
   // Make sure to ignore calls from a previous mission load
   if (%seq != $missionSequence || !$MissionRunning)
      return;
   ...
   //进行客户端CRC校验
	 if( !%client.SyncResCRC(%crc) )
	 {
	 		echo( "Client Mission Crc Error !" );
	    commandToClient(%client, 'MissionCrcError' );
	    return ;
	 }
...
function serverCmdMissionStartPhase3Ack(%client, %seq,%UID,%PlayerId)
{
   // Make sure to ignore calls from a previous mission load
   if(%seq != $missionSequence || !$MissionRunning)
      return;
   ...
   %client.startMission();
   %client.onClientEnterGame(%UID,%PlayerId);
}
