//-----------------------------------------------
// Copyright ?PowerEngine
//-----------------------------------------------
//<edit>thinking:因为在main.cs调用次序错误,为了不改变面有界面样子,所以注释现使用的样式.(调用关系如下)
...
//$lightEditor::profileCheckImage = $lightEditor::profilePath @ (($platform $= "macos") ? "osxCheck" : "PowerCheck");
//$lightEditor::profileMenuImage = $lightEditor::profilePath @ (($platform $= "macos") ? "osxMenu" : "PowerMenu");
//
...
function lightEditor::setPanel(%this, %panel)
{
   ...
   lightEditorWindow.text = "PowerEngine Lighting Kit " @ %this.activePanel.datablockType_text;
}
...
function lightEditor::onWake(%this)
{
   ...
   %this.constants_menuName[0] = "文件";
   %this.constants_menuName[1] = "重新光照";
   %this.constants_menuName[2] = "编辑";
   %this.constants_menuName[3] = "属性";
   ...
   lightEditorMenuBar.addMenu(%menuname, %menuid);
   %i = 1;
   lightEditorMenuBar.addMenuItem(%menuname, "光照编辑", %i);
   lightEditorMenuBar.scriptCommand[%menuname, %i] = "lightEditor.setPanel(lightEditorLightEditorPanel);";
   %i = %i + 1;
   lightEditorMenuBar.addMenuItem(%menuname, "滤光编辑", %i);
...
function lightEditor::createDB(%this, %name)
{
   ...
   if(!isFile(%path))
   {
      ...
      %file.writeLine("//--- SERVER OBJECT WRITE BEGIN ---");
      ...
      %file.writeLine("};");
      %file.writeLine("//--- SERVER OBJECT WRITE END ---");
      %file.delete();
