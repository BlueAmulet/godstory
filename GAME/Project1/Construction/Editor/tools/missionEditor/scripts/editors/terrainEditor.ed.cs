...
function ETerrainEditor::changeMaterial(%this, %matIndex)
{
   ...
   if(%ret)
   {
      ...
      %file = %dlg.FileName;
      ETerrainEditor.reloadTerrainTexture();
   }
...
function EPainter::setup(%this, %selectedIndex)
{
  ...
  // %valid = true;
   for(%i = 0; %i < getRecordCount(%mats); %i++)
   {
      ...
      ("ETerrainMaterialDelete" @ %i).setActive(true);
      ("ETerrainMaterialPaint" @ %i).setActive(%mat !$= "");
      if(%mat $= "")
      {
         ("ETerrainMaterialChange" @ %i).setText("Add...");
         ("ETerrainMaterialChange" @ %i).setActive(true);
         ("ETerrainMaterialDelete" @ %i).setActive(false);
         //if(%valid)
         //   %valid = false;
         //else
...
function ETerrainEditor::DeleteMaterial(%this,%index)
{
	//先设置加载标识，再删除纹理
	ETerrainEditor.reloadTerrainTexture();
	ETerrainEditor.destoryMaterialMap(%index);
	EPainter.setup(%index);
}
