...
function GuiEditorStartCreate()
{
   ...
   guiEditCanvas.pushDialog(NewGuiDialog);
}
...
function GuiEditorCreate()
{
   ...
   guiEditCanvas.popDialog(NewGuiDialog);
...
{ 
   ...
   guiEditCanvas.setContent(GuiEditorGui);
...
{
   guiEditCanvas.pushDialog(GuiEditorPrefsDlg);
}
...
function GuiEditorPaletteDragList::onMouseDragged(%this)
{
   ...
   %cursorpos = guiEditCanvas.getCursorPos();
   ...
   guiEditCanvas.getContent().add(%dragCtrl);
...
function GuiEditor::updateUndoMenu(%this)
{
   ...
   %undoitem = "³·Ïû" SPC %nextUndo TAB %cmdCtrl SPC "Z" TAB "GuiEditor.undo();";
   %redoitem = "ÖØ×ö" SPC %nextRedo TAB %cmdCtrl @ "-shift Z" TAB "GuiEditor.redo();";
