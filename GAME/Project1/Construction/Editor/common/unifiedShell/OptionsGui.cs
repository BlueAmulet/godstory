...
function applyOptions()
{
   ...
   // check if PowerEngine will require a restart for the new driver
   ...
   if (%oldDriver !$= $pref::Video::displayDevice)
   {
      MessageBoxOKCancel("Change Video Device Now?",
         "Changing your video device requires PowerEngine to be restarted. Selecting \"Ok\" will restart PowerEngine with the new settings. Selecting \"Cancel\" will keep the new video settings and apply them next time you restart PowerEngine.",
         "restartInstance();", "");
   }
