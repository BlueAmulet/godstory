...
function applyAVOptions()
{
   ...
   if (%newDriver !$= $pref::Video::displayDevice)
   {
      MessageBoxYesNo( "Change Video Device Now?",
         "Do you want to RESTART PowerEngine and switch to the selected video device now? If you hit yes, PowerEngine will restart using the new video device. If you select no, your change will take effect at next run.",
         "restartInstance();", "");
