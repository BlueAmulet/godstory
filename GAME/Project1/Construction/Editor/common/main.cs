...
function onStart()
{
   ...
   exec("gameres/client/defaults.cs");
   exec("gameres/server/defaults.cs");

   // Preferences (overide defaults)
   if (isFile("gameres/client/prefs.cs"))
      exec("gameres/client/prefs.cs");
   if (isFile("gameres/server/prefs.cs"))
      exec("gameres/server/prefs.cs");
