...
public:
    ...
    struct State
    {
        State(void) : onEnter(0),onLeave(0),onExpire(0),onExec(0) {}

        default_handler  onEnter;
        default_handler  onLeave;
        default_handler  onExpire;
        _eventHandler    onExec;
    };

    CTimerStateMachine(_parent* parent) : m_parent(parent),m_curState(-1),m_preState(-1),m_expireTime(0) {}
    ...
    {
        if (0 == m_expireTime || -1 == m_curState)
            return;
