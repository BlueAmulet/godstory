...
public:
    ...
    GEXPORT GSysFile(const char *ppath, SInt flags = Open_Read|Open_Buffered, SInt mode = Mode_ReadWrite); 

    // ** Open & management 
    // Will fail if file's already open
    // For some platforms (PS2,PSP,Wii) buffering will be used even if Open_Buffered flag is not set because 
    // of these platforms' file system limitation.
    GEXPORT bool    Open(const char *ppath, SInt flags = Open_Read|Open_Buffered, SInt mode = Mode_ReadWrite);
