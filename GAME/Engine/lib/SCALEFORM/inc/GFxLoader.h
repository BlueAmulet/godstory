...
public:
    ...
    virtual bool GetParameter (const char* parameter, GFxString* value) = 0;
...
public:
    ...
    virtual GFile*      OpenFile(const char *purl) = 0;
    ...
    virtual GFile*      OpenFileEx(const char *purl, class GFxLog *plog) = 0;    
};
...
public:
    ...
    static GImage*          LoadBuiltinImage(GFile* pfile,
                                             ...
                                             GFxLog* plog, GFxJpegSupportBase* pjpegState, GFxPNGSupportBase* ppngState);
};
...
public:
    ...
    virtual GImage* ReadJpeg(GFile* pin) = 0;
    ...
    virtual GImage* ReadSwfJpeg2(GFile* pin) = 0;

    // Create and read a new image, using a input object that
    // already has tables loaded.
    virtual GImage* ReadSwfJpeg2WithTables(GJPEGInput* pjin) = 0;

    // For reading SWF JPEG3-style image data, like ordinary JPEG, 
    // but stores the data pin GImage format.
    virtual GImage* ReadSwfJpeg3(GFile* pin) = 0;
};
...
public:
    ...
    // Create and read a new image from the stream.
    virtual GImage* ReadJpeg(GFile* pin);

    // Create and read a new image from the stream.  Image is pin
    // SWF JPEG2-style Format (the encoding tables come first pin a
    // separate "stream" -- otherwise it's just normal JPEG).  The
    // IJG documentation describes this as "abbreviated" format.
    virtual GImage* ReadSwfJpeg2(GFile* pin);

    // Create and read a new image, using a input object that
    // already has tables loaded.
    virtual GImage* ReadSwfJpeg2WithTables(GJPEGInput* pjin);

    // For reading SWF JPEG3-style image data, like ordinary JPEG, 
    // but stores the data pin GImage format.
    virtual GImage* ReadSwfJpeg3(GFile* pin);
};
...
public:
    ...
    virtual GImage* CreateImage(GFile* pin) = 0;
};
...
{
public:
    virtual GImage* CreateImage(GFile* pin);
};
...
#ifdef GFC_USE_ZLIB
              const GPtr<GFxZlibSupportBase>& zlib = *new GFxZlibSupport,
#else
              const GPtr<GFxZlibSupportBase>& zlib = NULL,
#endif
#ifdef GFC_USE_LIBJPEG
              const GPtr<GFxJpegSupportBase>& pjpeg = *new GFxJpegSupport
#else
              const GPtr<GFxJpegSupportBase>& pjpeg = NULL
#endif
              );
