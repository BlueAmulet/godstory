...
public:
    ...
    public:
        ...
        Stats()         { Clear();}
        void Clear()    { Triangles = 0; Lines = 0; Primitives = 0; Masks = 0;}           
    };
    ...
    struct UserData
    {
        ...
        UByte           PropFlags;
        UserData()
            : pString(NULL), pFloat(NULL), PropFlags(UD_None) { }
    };
    // Passes data attached in ActionScript (MovieClip.rendererString and MovieClip.rendererFloat).
    // If one of the values was not specified for this object, the corresponding pointer is null.
    virtual void        PushUserData(UserData* pdata) { GUNUSED(pdata); }
