...
public:
    ...
    T   valueAt(unsigned i) const           { return Array[i]; }
...
//------------------------------------------------------------------------
template<class T, int Extra=5> class GPodVectorAdaptiveBounds
{
public:
    ~GPodVectorAdaptiveBounds() 
    { 
        if (Array)
            GPodAllocator<T>::free_array(Array, UpperBound-LowerBound+1); 
    }

    GPodVectorAdaptiveBounds() : 
        LowerBound(GFC_MAX_SINT32), UpperBound(GFC_MIN_SINT32), Array(0), Seed(0)
    {}
    ...
    const T& operator [] (int i) const
    {
        return (i >= LowerBound && i <= UpperBound &&
                LowerBound < UpperBound) ? Array[i - LowerBound] : Seed;
    }
    ...
    T& operator [] (int idx)
    {
        ...
        if (idx > UpperBound)
        {
            ...
            {
                LowerBound = UpperBound = idx;
                return Seed;
            }
            oldSize = UpperBound - LowerBound + 1;
            newSize = idx        - LowerBound + 1;
            if (newSize < oldSize + Extra)
                newSize = oldSize + Extra;
            newArray = GPodAllocator<T>::allocate_array(newSize);
            if (Array)
            {
                memcpy(newArray, Array, oldSize * sizeof(T));
                GPodAllocator<T>::free_array(Array, oldSize);
            }
            else
            {
                newArray[0] = Seed;
            ...
            memset(newArray + oldSize, 0, (newSize - oldSize) * sizeof(T));
            Array = newArray;
            UpperBound = LowerBound + newSize - 1;
            return Array[idx - LowerBound];
        }
        if (idx < LowerBound)
        {
            oldSize = UpperBound - LowerBound + 1;
            newSize = UpperBound - idx        + 1;
            if (newSize < oldSize + Extra)
                newSize = oldSize + Extra;
            newArray = GPodAllocator<T>::allocate_array(newSize);
            if (Array)
            {
                memcpy(newArray + (newSize - oldSize), Array, oldSize * sizeof(T));
                GPodAllocator<T>::free_array(Array, oldSize);
            }
            else
            {
                newArray[newSize-1] = Seed;
                Seed                = T(0);
            }
            memset(newArray, 0, (newSize - oldSize) * sizeof(T));
            Array = newArray;
            LowerBound = UpperBound - (newSize - 1);
            return Array[idx - LowerBound];
        }
        return (LowerBound < UpperBound) ? Array[idx - LowerBound] : Seed;
    }
