...
void Path::initPersistFields()
{
   ...
   addField("mPathID", TypeS32, Offset(mPathID, Path));
   //
}

void Path::OnSerialize( CTinyStream& stream )
{
...
void Path::OnUnserialize( CTinyStream& stream )
{
...
void Path::updatePath()
{
 // If we need to, allocate a path index from the manager
	...
	{
#ifdef NTJ_CLIENT
		mPathIndex = gClientPathManager->allocatePathId(this);
#else
		mPathIndex = gServerPathManager->allocatePathId(this);
#endif
	}
   sortMarkers();
...
#ifdef NTJ_CLIENT
   gClientPathManager->updatePath(mPathID, mIsLooping, positions, rotations, times, smoothingTypes);
#else
   gServerPathManager->updatePath(mPathID, mIsLooping, positions, rotations, times, smoothingTypes);
#endif
}
...
void Marker::OnSerialize( CTinyStream& stream )
{
	__super::OnSerialize( stream );
	...
	stream << mKnotType;
	stream << mMSToNext;
	stream << mSmoothingType;
}

void Marker::OnUnserialize( CTinyStream& stream )
{
	__super::OnUnserialize( stream );
...
//--------------------------------------------------------------------------
U64 Marker::packUpdate(NetConnection* con, U64 mask, BitStream* stream)
{
