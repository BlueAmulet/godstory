...
void ProcessMinWin()
{
	...
	if(!g_MultThreadWorkMgr->isBegin())		//Ray: 最小化状态全部丢弃
	{
		g_MultThreadWorkMgr->endProcess();
	}
	else
		g_MultThreadWorkMgr->setBegin(false);
#endif
}

void SceneGraph::onZombify()
{
#ifdef USE_MULTITHREAD_ANIMATE
	g_MultThreadWorkMgr->endProcess();
	g_MultThreadWorkMgr->zombify(true);
#endif
}
...
SceneGraph::SceneGraph(bool isClient)
{
   ...
   VECTOR_SET_ASSOCIATION(mTerrainList);
      
   // Default to the standard lighting kit lighting manager
   mLightManager = AvailableLightManagers::get()->getLMByName(DEFAULT_LIGHTING_SYSTEM);
   AssertFatal(mLightManager, "No light manager available!");
   ...
   mStaticObjDistance = 250;
   mFogColor.set(128, 128, 128);
   mFogColorDirty = true;        // This will trigger a re-build of the entire fog bitmap the first time
   ...
   mLastView2WorldMX.identity();
   mLastWorld2ViewMX.identity();

#ifdef USE_MULTITHREAD_ANIMATE
   if(mIsClient)
   {
	   Process::notify(ProcessMinWin, PROCESS_LAST_ORDER);
   }
#endif
}
...
SceneGraph::~SceneGraph()
{
   ...
   AvailableLightManagers::get()->cleanupCurrentLM();   
   ...
   if(mIsClient)
   {
	   Process::remove(ProcessMinWin);
   }
#endif
}

//TerrainBlock* SceneGraph::getTerrain(F32 x,F32 y)
//{
//	for (int i=0; i<mTerrainList.size(); ++i)
//	{
//		Point3F pos = mTerrainList[i]->getPosition();
//		F32 len = mTerrainList[i]->getSquareSize() * TerrainBlock::BlockSize;
//		if(x > pos.x && x < pos.x + len && y > pos.y && y > pos.y + len)
//			return mTerrainList[i];
//	}
//	return NULL;
//}
...
void SceneGraph::updateScene(const U32 objectMask)
{
	PROFILE_START(updateScene);
	...
	Point3F cp;
	mLastView2WorldMX.getColumn(3, &cp);
	...
	mCurSceneState = new SceneState(NULL,
		mCurrZoneEnd,
		left, right,
		bottom, top,
		nearPlane,
	...
	mCurSceneState->mFlipCull = isReflectPass();

	//PROFILE_START(BuildFogTexture);
	buildFogTexture( mCurSceneState );
	//PROFILE_END(BuildFogTexture);
	...
	mLightManager->registerGlobalLights(false);
	//PROFILE_END(RegisterLights);

	processSunLight();

	mInCameraObjects.clear();
	clippedRectCount = 0;

#ifdef DEBUG
	mInCameraStatics.clear();
	mInCameraShadows.clear();
	mTrDPtimes = mSdDPtimes = mSaDPtimes = mSkDPtimes = mQueryTimes = mSkipObjects = 0;
#endif

#ifdef USE_MULTITHREAD_ANIMATE
	g_MultThreadWorkMgr->enable(!isReflectPass());

	if(g_MultThreadWorkMgr->isEnable())
	{
		PROFILE_START(ProcessEnd);
		g_MultThreadWorkMgr->endProcess(false);
		PROFILE_END(ProcessEnd);
	}
#endif

	TSMesh::setRefract(isReflectPass());

	DetailManager::beginPrepRender();

	PROFILE_START(BuildSceneTree);
	buildSceneTree(mCurSceneState, startObject, startZone, 1, objectMask);
	PROFILE_END(BuildSceneTree);

	DetailManager::endPrepRender();
	PROFILE_END(updateScene);
}

void SceneGraph::doRenderScene()
{
	PROFILE_START(doRenderScene);

#ifdef USE_MULTITHREAD_ANIMATE
	PROFILE_START(beginProcess);
	if(g_MultThreadWorkMgr->isEnable())
		g_MultThreadWorkMgr->beginProcess();
	PROFILE_END(beginProcess);
#endif
	...
	traverseSceneTree(mCurSceneState);
	PROFILE_END(TraverseScene);

#ifdef USE_MULTITHREAD_ANIMATE
	if(g_MultThreadWorkMgr->isEnable())
		g_MultThreadWorkMgr->tryProcess();
	else
	{
		g_MultThreadWorkMgr->zombify(false);
	...
	PROFILE_END(UnregisterLights);

	delete mCurSceneState;
	mCurSceneState = NULL;

	if(!isReflectPass())
		smFrameKey++;

	gRenderInstManager.doQuery();

	PROFILE_START(doRenderScene_clear);
	gRenderInstManager.clear();
	PROFILE_END(doRenderScene_clear);

	PROFILE_END(doRenderScene);
}

void SceneGraph::renderScene(const U32 objectMask)
{
	updateScene(objectMask);
	doRenderScene();
}
...
void SceneGraph::buildFogTexture(SceneState *pState)
{
   ...
   if (!bool(mFogTexture))
   {
      ...
      mBlackFogTexture = GFXTexHandle(new GBitmap(FogTextureDistSize, FogTextureHeightSize, false, GFXFormatR8G8B8A8), &GFXDefaultPersistentProfile, true);
   ...
   if (!pState /*&& (mLastState || mFogDirty) */)
   {
   ...
   // If the fog color is dirty, repopulate the fog bitmap in it's entirety. Otherwise
   // we only have to modify the alpha.
   if( mFogColorDirty )
   {
      ColorI *pixel = reinterpret_cast<ColorI *>( fogBitmap->getWritableBits() );
      AssertFatal( fogBitmap->bytesPerPixel == 4, "Assumption invalid! (Casting to ColorI)" );

      for( int i = 0; i < fogBitmap->getHeight() * fogBitmap->getWidth(); i++ )
         *pixel++ = mFogColor;

      ColorI *blackpixel = reinterpret_cast<ColorI *>( blackfogBitmap->getWritableBits() );
      AssertFatal( fogBitmap->bytesPerPixel == 4, "Assumption invalid! (Casting to ColorI)" );

      ColorI mBlackFogColor = ColorI(0,0,0,255);
      for( int i = 0; i < blackfogBitmap->getHeight() * blackfogBitmap->getWidth(); i++ )
         *blackpixel++ = mBlackFogColor;

      mFogColorDirty = false;
   }

   for(U32 j = 0; j < fogBitmap->getHeight(); j++)
   {
      F32 dist = distStart;
      ColorI *ptr  = reinterpret_cast<ColorI *>( fogBitmap->getAddress(0, j) );
      ColorI *blackptr  = reinterpret_cast<ColorI *>( blackfogBitmap->getAddress(0, j) );
      ...
      {
         const U32 fog = (U32)((fogStart + mHazeArray[i]) * 255);
...
#if !defined( POWER_SHIPPING ) && !defined( POWER_OS_XENON )
...
//         glCullFace(GL_FRONT);
      ...
      PROFILE_END(RenderCurrentImages);
...
inline void scopeCallback(SceneObject* obj, ScopingInfo* pInfo)
{
   ...
   if (obj->isScopeable()) {
      ...
      if (difSq < pInfo->scopeDistSquared && obj->canAddToScope(pInfo->connection)) {
         ...
         if ( realDif - obj->getWorldSphere().radius < pInfo->scopeDist  && 
			  obj->canAddToScope(pInfo->connection)) {
            ptr->objectInScope(obj);
         }
      }
   }
...
{
	GameConnection* pConn = NULL;
	pConn = dynamic_cast<GameConnection*>(netConnection);
	int nLayerId = 10;
	if( pConn )
		nLayerId = pConn->GetLayerId();
   // Find the start zone...
   SceneObject* startObject;
   U32          startZone;
   ...
   for (i = 0; i < mCurrZoneEnd; i++) {
      ...
      if (zoneScopeState[i] == true) {
         ...
         while (pWalk != NULL) {
            ...
            if (pObject->mLastStateKey != smStateKey &&( pObject->getLayerID() == nLayerId || pObject->getLayerID() == 0 ) ) {
...
bool SceneGraph::addObjectToScene(SceneObject* obj)
{
   ...
   if (obj->getType() & EnvironmentObjectType) {
      if (dynamic_cast<Sky*>(obj) != NULL) 
	  {
		 ...
		 {
			 g_ScreenFXMgr.setScreenMask(StringTable->insert("~/data/mask/Mask.png"));
			 //g_ScreenFXMgr.setScreenMask(mCurrSky->getScreenMask());
		 }
#endif
      }
   }
   ...
   if (obj->getType() & DecalObjectType)
   {
	   sgDecalProjector *pDecal = dynamic_cast<sgDecalProjector*>(obj);
	   if(pDecal)
...
void SceneGraph::removeObjectFromScene(SceneObject* obj)
{
   if (obj->mSceneManager != NULL) {
	  ...
	  if (obj->getType() & DecalObjectType)
	  {
		  for(int i=mDecalList.size()-1;i>=0;i--)
		  {
			  if(mDecalList[i] == obj)
			  {
				mDecalList.erase(i);
...
void SceneGraph::setStaticObjDistance(F32 dist)
{
	mHazeArrayDirty = true;
	mStaticObjDistance = dist;
}

void SceneGraph::setObjSenseDistance(F32 dist)
{
	mHazeArrayDirty = true;
	mObjSenseDistance = dist;
}
...
void SceneGraph::setSunColor(ColorF *ambientColor,ColorF *lightColor,ColorF *shadowColor)
{
#ifdef NTJ_CLIENT
	if(!mCurrSun)
		return ;
	...
	if(ambientColor)
		mCurrSun->setAmbientColor(*ambientColor);

	if(lightColor)
		mCurrSun->setLightColor(*lightColor);
#endif
}

void SceneGraph::processSunLight()
{
#ifdef NTJ_CLIENT
	if(!mCurrSun || !mCurrSun->mDirty)
		return ;

	mCurrSun->processLight();

#endif
}

void SceneGraph::refreshTerrainLight()
{
#ifdef NTJ_CLIENT
	if(!mCurrSun)
		return ;
	...
	for (U32 i=0; i<mTerrainList.size(); i++)
	{
		terrain = static_cast<TerrainBlock*>(mTerrainList[i]);
		if (terrain && terrain->isResourceLoaded())
		{
			terrain->freshLightmapByShader(mCurrSun->mLightAmbient * mCurrSun->mTerrColorScale, mCurrSun->mLightColor * mCurrSun->mTerrColorScale, mCurrSun->mBackColor * mCurrSun->mTerrColorScale);
...
void SceneGraph::setCurrentTerrain(TerrainBlock* ter)
{
	mCurrTerrain = ter; 
    ...
    {
        WaterBlock *pWater = static_cast<WaterBlock*>(mWaterList[i]);
        if(pWater)
            pWater->triggerTerrainChange(mCurrTerrain);
    }
#endif

// #ifdef NTJ_CLIENT
// 	//Ray: 客户端由于地形卸载的问题需要重新刷新相关物件
// 	for (S32 i = 0; i < mDecalList.size(); i++)
// 	{
// 		sgDecalProjector* pDecal = static_cast<sgDecalProjector*>(mDecalList[i]);
// 		if(pDecal)
// 			pDecal->triggerTerrainChange(mCurrTerrain);
// 	}
// #endif	
}

void SceneGraph::addToTerrainList(SceneObject* obj)
{
	mTerrainList.push_back(static_cast<TerrainBlock*>(obj));
}

void SceneGraph::removeFromTerrainList(SceneObject* obj)
{
	...
	for(int i=0; i<mTerrainList.size(); ++i)
	{
		if(obj == mTerrainList[i])
		{
			mTerrainList.erase(i);
			break;
		}
	}
}
...
bool SceneGraph::setLightingSystem(const char* sName)
{
   if ((gClientSceneGraph) && (gServerSceneGraph))
   {
      LightManager* lm = AvailableLightManagers::get()->getLMByName(sName);
      if (lm)
      {
         gClientSceneGraph->mLightManager = lm;
         gServerSceneGraph->mLightManager = lm;         
      ...
      AssertFatal(false, "The scene graphs have not been created yet!");
      Con::errorf("The scene graph is not yet active, lighting system changes are not allowed.");
      return false;
   }
}

ConsoleFunction(setLightingSystem, bool, 1, 3, "string sLightingSystemName")
{   
   bool ret = SceneGraph::setLightingSystem(argv[1]);
   return ret;
}
                ...
                "will be regenerated only if the lighting cache files can be written.")
{
   ...
   {
      if (argc > 1)
         return gClientSceneGraph->getLightManager()->lightScene(argv[1], argv[2]);
      else
         return gClientSceneGraph->getLightManager()->lightScene(argv[1], NULL);
   } else {
      return false;
   }
}
