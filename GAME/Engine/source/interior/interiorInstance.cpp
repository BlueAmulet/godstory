...
//---------------------------------------------------------------------------
//#define STATEBLOCK
GFXStateBlock* InteriorInstance::mZTrueSB = NULL;
//--------------------------------------------------------------------------
                                       ...
                                       const U32 startZone, const bool modifyBaseState)
{
   ...
   PROFILE_SCOPE(InteriorPrepRenderImage);
...
//------------------------------------------------------------------------------
U64 InteriorInstance::packUpdate(NetConnection* c, U64 mask, BitStream* stream)
{
   U64 retMask = Parent::packUpdate(c, mask, stream);
...
void InteriorInstance::updateReflection()
{
   ...
   static MatrixF invRotMat(EulerF( -(M_PI_F / 2.0f), 0.0f, 0.0f));
...
#ifdef STATEBLOCK
		AssertFatal(mZTrueSB, "InteriorInstance::updateReflection -- mZTrueSB cannot be NULL.");
		mZTrueSB->apply();
#else
		GFX->setZEnable( true );
#endif
...
void InteriorInstance::resetStateBlock()
{
	//mZTrueSB
	GFX->beginStateBlock();
...
void InteriorInstance::releaseStateBlock()
{
	if (mZTrueSB)
	{
		mZTrueSB->release();
...
void InteriorInstance::initsb()
{
	if (mZTrueSB == NULL)
	{
		mZTrueSB = new GFXD3D9StateBlock;
		mZTrueSB->registerResourceWithDevice(GFX);
		mZTrueSB->mZombify = &releaseStateBlock;
		mZTrueSB->mResurrect = &resetStateBlock;

		resetStateBlock();
	}
}

void InteriorInstance::shutdown()
{
