...
Interior::~Interior()
{
   ...
   for(i = 0; i < mLightDirMapsTex.size(); i++)
   {
      mLightDirMapsTex[i] = NULL;
   }
...
bool Interior::prepForRendering(const char* path)
{
   ...
   mMaterialList->load(InteriorTexture, path, false);
   ...
   for(U32 i=0; i<mStaticMeshes.size(); i++)
      mStaticMeshes[i]->prepForRendering(path);
...
{
   PROFILE_SCOPE(InteriorTraverseZone);
...
{
   PROFILE_SCOPE(InteriorZoneTraversal);
                                 ...
                                 U32 &startVert )
{
   ...
   if( !node.matInst )
   {
      char* name = mMaterialList->mMaterialNames[ node.baseTexIndex ];

      if ( name &&
         ( dStrcmp( name, "NULL" ) != 0 &&
           dStrcmp( name, "ORIGIN" ) != 0 &&
           dStrcmp( name, "TRIGGER" ) != 0 &&
           dStrcmp( name, "FORCEFIELD" ) != 0 &&
           dStrcmp( name, "EMITTER" ) != 0 ) )
      {
         Con::errorf( "material unmapped: %s", name );
      }
...
void Interior::createZoneVBs()
{
   ...
   GFXVertexPNTTB *vbVerts = vbVerts = mVertBuff.lock();
