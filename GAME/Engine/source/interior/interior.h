...
class InteriorLightPlugin
{
public:
   // Starts the lighting for an interior instance, return true if there is lighting to do!
   virtual bool interiorInstInit(InteriorInstance* intInst) = 0;
   // Starts the lighting for a zone, return true if there is lighting to do!
   virtual bool zoneInit(S32 zoneid) = 0;
   // Finally, this is given a list of renderinstances without lighting the plugin is responsible for
   // submitting them to the render instance manager
   virtual void processRI(Vector<RenderInst*>& list) = 0;
};
...
//   void debugShowVertexColors(MaterialList* pMaterials);      
  ...
  private:
   ...
   void renderLights(InteriorInstance *intInst, SceneGraphData &sgData, RenderInst *coreRi, const ZoneVisDeterminer &zonevis);
  ...
  public:
   void purgeLODData();
private:
	static GFXStateBlock* mSetPreBatSB;
