                                  ...
                                  const Point3F& /*scale*/)
{
   ...
   pTope.buildBox(box, MatrixF(true), Point3F(1.0f, 1.0f, 1.0f));
...
void Interior::scanZoneNew(InteriorPolytope& polytope,
                           ...
                           U32*           numZones)
{
   PROFILE_SCOPE(InteriorScanZoneNew);
   // Submit the first volume of the poly tope to the bsp tree
   static InteriorPolytope::VolumeStack stack;
   stack.reserve(128);
   stack.setSize(1);
                          ...
                          U32*           numZones)
{
   if (isBSPLeafIndex(node) == false) 
   {
      ...
      PlaneF::Side side = rPlane.whichSideBox(center, axisx, axisy, axisz, Point3F(0.0f, 0.0f, 0.0f));
      if (planeIsFlipped(rNode.planeIndex))
                         ...
                         U32*           numZones)
{
   ...
   Point3F xRad((box.max.x - box.min.x) * 0.5f, 0.0f, 0.0f);
   Point3F yRad(0.0f, (box.max.y - box.min.y) * 0.5f, 0.0f);
   Point3F zRad(0.0f, 0.0f, (box.max.z - box.min.z) * 0.5f);
