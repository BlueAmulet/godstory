...
public:
   ...
   inline U32 getMaterialCount() { return (U32)mMaterials.size(); }
   inline const char * getMaterialName(U32 index) { return mMaterialNames[index]; }
   ...
   // material properties
   inline void setTextureType(TextureHandleType type) { mTextureType = type; }
   inline void setClampToEdge(bool tf) { mClampToEdge = tf; }
   ...
   virtual void mapMaterials();
   inline MatInstance * getMaterialInst( U32 texIndex )
   {
   ...
   virtual void prepareMaterial(int ,Material *) {return;};
