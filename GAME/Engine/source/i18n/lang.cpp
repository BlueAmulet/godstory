...
LangFile::LangFile(const UTF8 *langName /* = NULL */)
{
	if(langName)
	{
		S32 length = dStrlen(langName) + 1;
		mLangName = new UTF8 [length];
		dStrcpy(mLangName, length, langName);
	}
	else
		mLangName = NULL;
...
{
	Stream * stream = ResourceManager->openStream((const char*)filename);
	if(stream != NULL)
	{
...
U32 LangFile::addString(const UTF8 *str)
{
	...
	UTF8 *newstr = new UTF8 [length];
	dStrcpy(newstr, length, str);
...
void LangFile::setLangName(const UTF8 *newName)
{
	...
	S32 length = dStrlen(newName) + 1;
	mLangName = new UTF8 [length];
	dStrcpy(mLangName, length, newName);
}
...
void LangFile::setLangFile(const UTF8 *langFile)
{
	...
	S32 length = dStrlen(langFile) + 1;
	mLangFile = new UTF8 [dStrlen(langFile) + 1];
	dStrcpy(mLangFile, length, langFile);
}
...
UTF8 *sanitiseVarName(const UTF8 *varName, UTF8 *buffer, U32 bufsize)
{
	...
	dStrcpy(buffer, bufsize, (const UTF8*)"I18N::");
...
UTF8 *getCurrentModVarName(UTF8 *buffer, U32 bufsize)
{
	...
	dStrncpy(varName, sizeof(varName), cbName, slash - (const UTF8*)cbName);
