...
public:
   ...
   enum Flags
   {
      ...
      NoMSAA         = BIT(10), ///< No Multisample Anti-aliasing
   };
   ...
   void incActiveCopies() { mActiveCount++; }
   void decActiveCopies() { AssertFatal(mActiveCount != 0, "Ran out of extant copies!"); mActiveCount--; }   
...
// Depth buffer texture, no MSAA
GFX_DeclareTextureProfile(GFXDefaultZTargetNoMSAAProfile);

GFX_DeclareTextureProfile(GFXDynamicDiffuseProfile);

// Vertex Texture for gpu skinmesh
GFX_DeclareTextureProfile(GFXDefaultVertexMapProfile);
// r2vb for gpu skinmesh
GFX_DeclareTextureProfile(GFXDefaultDMapProfile);
GFX_DeclareTextureProfile(GFXDefaultDMapDebugProfile);
