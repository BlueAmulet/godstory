...
{
   //考虑到资源的异步加载,framealloc目前不具备多线程安全
                  ...
                  const U32 filter) const
{
   ...
   AssertFatal(getFormat() == GFXFormatR8G8B8 || getFormat() == GFXFormatR8G8B8A8 || getFormat() == GFXFormatR8G8B8X8 || getFormat() == GFXFormatA8, "GBitmap::writePNG: ONLY RGB bitmap writing supported at this time.");
   if (internalFormat != GFXFormatR8G8B8 && internalFormat != GFXFormatR8G8B8A8 && internalFormat != GFXFormatR8G8B8X8 && internalFormat != GFXFormatA8)
      return (false);
   ...
   else if (getFormat() == GFXFormatR8G8B8A8 || getFormat() == GFXFormatR8G8B8X8) {
      png_set_IHDR(png_ptr, info_ptr,
         width, height,               // the width & height
