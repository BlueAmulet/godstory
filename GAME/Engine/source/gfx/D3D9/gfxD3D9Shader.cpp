                              ...
                              LPSTR pFullPath, DWORD cbFullPath)
{
   ...
   if( psTemp && !( dStrncmp( pFileName, "game:\\", 6 ) == 0 ) ) 
   {
      ...
      dStrncpy( path, 512, _pShaderFileName, loc );
      path[loc] = 0;  
   ...
   dStrcat( path, 512, tmpFileName );

   if( cbFullPath > 0 )
   {
      dMemcpy( pFullPath, path, getMin( cbFullPath, dStrlen( path ) ) );
      pFullPath[getMin( cbFullPath, dStrlen( path ) )] = 0;
   }
...
void GFXD3D9Shader::initShader( const char *file, const char *target )
{
   ...
   D3DXMACRO defines[3];
   defines[0].Name = "POWER";
   defines[0].Definition = "1";
   defines[1].Name = "POWER_VERSION";
   char versionBuf[32];
   ...
   // compile HLSL shader
   if( dStrstr( (const char *)file, ".hlsl")|| (bUncompress = bool(dStrstr( (const char *)file, ".gmp"))) )
   {
      ...
      {
         // Ok it's not in the shader gen manager, so ask PowerEngine for it
         s = new FileStream();
         ...
         if( s != NULL )
         {
            buffer = (char *)fam.alloc( s->getStreamSize() );
            s->read( s->getStreamSize(), buffer );
#ifdef SHADERGEN_COMPRESS//需要解压
				if (bUncompress)
				{
					res = uncompress(buf, &size, (const unsigned char*)buffer, s->getStreamSize());
					AssertWarn(res==S_OK, "GFXD3D9Shader::initShader--解压shader数据失败！");
					res = GFXD3DX.D3DXCompileShader( (const char*)buf, size, defines, smD3DXInclude, "main", 
						target, flags, &code, &errorBuff, NULL );
				} 
				else
				{
