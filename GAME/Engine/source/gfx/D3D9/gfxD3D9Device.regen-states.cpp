...
void GFXD3D9Device::regenStates() 
{
   ...
   {
	   int realStage = stage<(TEXTURE_STAGE_COUNT-VERTEX_TEXTURE_NUM) ? stage : TEXTURE_STAGE_COUNT-stage-1+D3DDMAPSAMPLER;

		if( (realStage<getNumSamplers()) || (realStage>=D3DDMAPSAMPLER && realStage<=D3DVERTEXTEXTURESAMPLER3) )
		{
      ...
      for( U32 state = GFXTSS_FIRST; state < GFXTSS_COUNT; state++ ) 
      {
         ...
         temp = 0;
         mD3DDevice->GetTextureStageState( realStage, GFXD3D9TextureStageState[state], &temp );
         ...
         Con::printf( "   initTextureState( %d, %d, %d );", realStage, state, temp );
      }
   }
#endif
