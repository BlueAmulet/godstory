...
GFXLockedRect *GFXD3D9TextureObject::lock(U32 mipLevel /*= 0*/, RectI *inRect /*= NULL*/)
{
   ...
   if( mProfile->isRenderTarget() )
   {
      ...
      PROFILE_SCOPE(GFXD3D9TextureObject_lockRT);
...
bool GFXD3D9TextureObject::copyToBmp(GBitmap* bmp)
{
   ...
   PROFILE_END(GFXD3D9TextureObject_copyToBmp_pixCopy);
   ...
   PROFILE_END(GFXD3D9TextureObject_copyToBmp);
...
bool GFXD3D9TextureObject::getLegal()
{
	if( mProfile->isRenderTarget() )
	{
		if( !mLockTex || 
...
void GFXD3D9TextureObject::savePng( const char* path )
{
	//D3DLOCKED_RECT rt;
	//mD3DSurface->LockRect(&rt, 0, 0);
	//mD3DSurface->UnlockRect();
	//D3DXSaveSurfaceToFile(path, D3DXIFF_BMP, mD3DSurface, NULL, NULL);
	D3DXSaveTextureToFileA(path, D3DXIFF_BMP, mD3DTexture, NULL);
}