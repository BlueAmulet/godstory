...
{
	int realStage = stage<(TEXTURE_STAGE_COUNT-VERTEX_TEXTURE_NUM) ? stage : VERTEX_TEXTURE_NUM+stage-TEXTURE_STAGE_COUNT+D3DDMAPSAMPLER;
   ...
   switch( type )
   {
      ...
      case GFXSAMPMipFilter:
         mD3DDevice->SetSamplerState( realStage, GFXD3D9SamplerState[type], GFXD3D9TextureFilter[value] );
         break;

      case GFXSAMPAddressU:
      case GFXSAMPAddressV:
      case GFXSAMPAddressW:
         mD3DDevice->SetSamplerState( realStage, GFXD3D9SamplerState[type], GFXD3D9TextureAddress[value] );
         break;

      default:
         mD3DDevice->SetSamplerState( realStage, GFXD3D9SamplerState[type], value );
         break;
   }
}
...
void GFXD3D9Device::releaseDefaultPoolResources() 
{
   ...
   for( U32 i=0; i<mVolatileVBList.size(); i++ )
   {
      ...
      // mVolatileVBList[i]->vb->Release();
       CStat::Instance()->UnLog(unsigned int(mVolatileVBList[i].getPointer()));
   ...
   if( mDynamicPB != NULL )
   {
       CStat::Instance()->UnLog((unsigned int)mDynamicPB.getPointer());
...
void GFXD3D9Device::reacquireDefaultPoolResources() 
{
   ...
   CStat::Instance()->LogIndexBuf((unsigned int)mDynamicPB.getPointer(),sizeof( U16 ) * MAX_DYNAMIC_INDICES,0,GFXBufferTypeDynamic);
   CStat::Instance()->SetDesc((unsigned int)mDynamicPB.getPointer(),"Index Pool");
...
//   Con::printf("Created buff with type %x", vertFlags);
   ...
   CStat::Instance()->LogVertBuf((unsigned int)newBuff.getPointer(),vertSize * MAX_DYNAMIC_VERTS,GFXBufferTypeVolatile);
   CStat::Instance()->SetDesc((unsigned int)newBuff.getPointer(),"VB Pool");
...
void GFXD3D9Device::setVB( GFXVertexBuffer *buffer ) 
{
   ...
   D3D9Assert( mD3DDevice->SetStreamSource( mCurrentVB->mStreamNum, mCurrentVB->vb, 0, mCurrentVB->mVertexSize ), "Failed to set stream source" );
   if(mCurrentVB->mStreamNum==0)
   {
		D3D9Assert( mD3DDevice->SetFVF( mCurrentVB->mVertexType ), "Failed to set FVF" );
   }
}

void GFXD3D9Device::setStream( GFXVertexBuffer *buffer ) 
{
	AssertFatal( mCurrentOpenAllocVB == NULL, "Calling setVertexBuffer() when a vertex buffer is still open for editing" );
	mCurrentVB = static_cast<GFXD3D9VertexBuffer *>( buffer );
	D3D9Assert( mD3DDevice->SetStreamSource( mCurrentVB->mStreamNum, mCurrentVB->vb, 0, mCurrentVB->mVertexSize ), "Failed to set stream source" );
	if(mCurrentVB->vDecl)
		D3D9Assert(mD3DDevice->SetVertexDeclaration(mCurrentVB->vDecl),"Failed to set stream declaration");
}
//-----------------------------------------------------------------------------
...
bool GFXD3D9Device::CreateVertexDeclaration(D3DVERTEXELEMENT9 *pVertexElements,IDirect3DVertexDeclaration9 **ppDecl)
{
	return SUCCEEDED(mD3DDevice->CreateVertexDeclaration(pVertexElements,ppDecl));
}
...
//-----------------------------------------------------------------------------
void GFXD3D9Device::setVertexShaderConstF( U32 reg, const float *data, U32 size, U32 realCntFloat )
{
    //PROFILE_START(setVertexShaderConstF);

#ifdef SHADER_CONST_FILTER
    void* filterPtr = &(mVSConstFilter[reg*4]);
    size_t len = size * realCntFloat * sizeof(F32);
    if ( memcmp( data, filterPtr, len ) == 0 )
    {
        //PROFILE_END(setVertexShaderConstF); 
        return;
    }
    memcpy( filterPtr, data, len ); 
    mD3DDevice->SetVertexShaderConstantF( reg, (float*)filterPtr, size );          
#else
...
//-----------------------------------------------------------------------------
void GFXD3D9Device::setPixelShaderConstF( U32 reg, const float *data, U32 size, U32 realCntFloat )
{
    //PROFILE_START(setPixelShaderConstF);

#ifdef SHADER_CONST_FILTER
    void* filterPtr = &(mPSConstFilter[reg*4]);
    size_t len = size * realCntFloat * sizeof(F32);
    if ( memcmp( data, filterPtr, len ) == 0 )
    {
        //PROFILE_END(setPixelShaderConstF); 
        return;
    }
    memcpy( filterPtr, data, len ); 
    mD3DDevice->SetPixelShaderConstantF( reg, (float*)filterPtr, size ); 
#else
...
GFXPrimitiveBuffer * GFXD3D9Device::allocPrimitiveBuffer(U32 numIndices, U32 numPrimitives, GFXBufferType bufferType)
{
   ...
   else
   {
      ...
      CStat::Instance()->LogIndexBuf((unsigned int)res,sizeof(U16) * numIndices,0,bufferType);
   }
...
// allocVertexBuffer
//-----------------------------------------------------------------------------
GFXVertexBuffer * GFXD3D9Device::allocVertexBuffer( U32 numVerts, U32 vertFlags, U32 vertSize, GFXBufferType bufferType,U8 streamNum )
{
...
{
     CStat::Instance()->UnLog((unsigned int)vertBuff);
...
{
   int realTextureUnit = textureUnit<(TEXTURE_STAGE_COUNT-VERTEX_TEXTURE_NUM) ? textureUnit : VERTEX_TEXTURE_NUM+textureUnit-TEXTURE_STAGE_COUNT+D3DDMAPSAMPLER;

   if( texture == NULL )
   {
      D3D9Assert(mD3DDevice->SetTexture( realTextureUnit, NULL ), "Failed to set texture to null!");
      return;
   }

   GFXD3D9TextureObject *tex = (GFXD3D9TextureObject *) texture;
   D3D9Assert(mD3DDevice->SetTexture( realTextureUnit, tex->getTex()), "Failed to set texture to valid value!");
}
...
bool GFXD3D9Device::CreateVolumeTexture(StringTableEntry texname, IDirect3DVolumeTexture9** tex)
{
	if(FAILED(D3DXCreateVolumeTextureFromFileExA(mD3DDevice, texname, 0,0,0,1,0,D3DFMT_UNKNOWN,D3DPOOL_MANAGED, D3DX_FILTER_NONE, D3DX_FILTER_NONE, 0,NULL, NULL, tex)))
		return false;
	return true;
}

void GFXD3D9Device::SetVolumeTexture(U32 Sampler, IDirect3DVolumeTexture9* tex)
{
	mD3DDevice->SetTexture(Sampler, tex);
}