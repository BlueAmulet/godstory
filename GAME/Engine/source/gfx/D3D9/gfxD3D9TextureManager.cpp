...
GFXD3D9TextureManager::GFXD3D9TextureManager( LPDIRECT3DDEVICE9 d3ddevice ) 
{
   ...
   supportsAutoMips = true;
   supportsRGB24Textures = false;
}

void GFXD3D9TextureManager::queryProfile()
{
	static bool inited = false;
	if(!inited)
	{
		// Some relevant helper information...
		supportsAutoMips = GFX->getCardProfiler()->queryProfile("autoMipMapLevel", true);
		supportsRGB24Textures = GFX->getCardProfiler()->queryProfile("allowRGB24BitTextures", false);
		inited = true;
	}
}
//-----------------------------------------------------------------------------
                                               ...
                                               bool forceMips)
{
   ...
   if(profile->getType() == GFXTextureProfile::DMap)
   {
		usage |= D3DUSAGE_DMAP;
   }
   ...
   else
   {
      ...
      D3DMULTISAMPLE_TYPE multisampleType = d3d->getMultisampleType();
      U32 multisampleLevel = d3d->getMultisampleLevel();
      if (retTex->mProfile->noMSAA())
      {
      ...
      if(retTex->mProfile->isZTarget())
      {
         D3D9Assert(mD3DDevice->CreateDepthStencilSurface(width, height, d3dTextureFormat,
            multisampleType, multisampleLevel, TRUE, retTex->getSurfacePtr(), NULL), "Failed to create Z surface" );
      }
      else
      {
         ...
         if (retTex->mProfile->isRenderTargetZBuffer() && (mDeviceCaps.Caps2 && D3DDEVCAPS2_CAN_STRETCHRECT_FROM_TEXTURES))
         {
            D3D9Assert(mD3DDevice->CreateRenderTarget(width, height, d3dTextureFormat, 
               multisampleType, multisampleLevel, false, retTex->getSurfacePtr(), NULL),
   ...
   CStat::Instance()->LogTexture((unsigned int)retTex,width,height,format,pool,"",profile ? profile->isRenderTarget() : 0);
   CStat::Instance()->SetDesc((unsigned int)retTex,"非文件加载");
}
...
//------------------------------------------------------------------------------
      ...
      else if( pDL->getFormat() == GFXFormatR8G8B8 && texture->mFormat == GFXFormatR8G8B8X8 )
      {
         ...
         AssertFatal( !supportsRGB24Textures, 
   ...
   CStat::Instance()->LogTexture((unsigned int)aTexture,pDL->getWidth(0),pDL->getHeight(0),pDL->getFormat(),0,"",0);
   CStat::Instance()->SetDesc((unsigned int)aTexture,"从BMP加载");
   return true;          
}
...
bool GFXD3D9TextureManager::_loadTexture(GFXTextureObject *aTexture, DDSFile *dds)
{
   ...
   validateTexParams( dds->getHeight(), dds->getWidth(), aTexture->mProfile, maxDownloadMip );
   U32 nbMipMapLevel  = maxDownloadMip;
   ...
   CStat::Instance()->LogTexture((unsigned int)aTexture,dds->getWidth(0),dds->getHeight(0),GFXD3D9TextureFormat[dds->mFormat],0,"",0);
   CStat::Instance()->SetDesc((unsigned int)aTexture,"从DDS加载");
   return true;
}
