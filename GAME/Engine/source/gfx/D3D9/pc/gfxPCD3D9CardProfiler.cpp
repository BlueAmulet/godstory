...
void GFXPCD3D9CardProfiler::setupCardCapabilities()
{
   ...
   //Ray: for gui SeparateAlphaBlend
   setCapability( dStrdup( "SeparateAlphaBlend" ), caps.PrimitiveMiscCaps & D3DPMISCCAPS_SEPARATEALPHABLEND );
   ...
   HRESULT hr128b = pD3D->CheckDeviceFormat( D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, 
	   displayMode.Format, 0 /* usage flags = 0 */, 
	   D3DRTYPE_TEXTURE, GFXD3D9TextureFormat[GFXFormatR32G32B32A32F] );
   if(SUCCEEDED(hr128b))
   {
	   hr = pD3D->CheckDeviceFormat( D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, 
	    displayMode.Format, D3DUSAGE_QUERY_VERTEXTEXTURE , 
	    D3DRTYPE_TEXTURE, GFXD3D9TextureFormat[GFXFormatR32G32B32A32F] );
	   setCapability( dStrdup( "allowRGBA32FBitVertexTextures" ), SUCCEEDED( hr ) ); 

	   hr = pD3D->CheckDeviceFormat(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, displayMode.Format, 0, D3DRTYPE_SURFACE, (D3DFORMAT)R2VB_FOURCC_R2VB);
	   setCapability( dStrdup( "allowATIR2VB" ), SUCCEEDED( hr )); 
   }
