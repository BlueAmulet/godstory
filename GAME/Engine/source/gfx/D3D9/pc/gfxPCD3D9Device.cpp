...
#include "gfx/D3D/DXDebugInfoLog.h"
...
D3DPRESENT_PARAMETERS GFXPCD3D9Device::setupPresentParams( const GFXVideoMode &mode, const HWND &hwnd )
{
   ...
   validateMultisampleParams(fmt, !mode.fullScreen);
...
void GFXPCD3D9Device::enumerateVideoModes() 
{
   ...
   for( S32 i = 0; i < formats.size(); i++ ) 
   {
      ...
      mD3D->CheckDeviceMultiSampleType(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, formats[i], TRUE, D3DMULTISAMPLE_NONMASKABLE, &MaxSampleQualities);
...
void GFXPCD3D9Device::init( const GFXVideoMode &mode, PlatformWindow *window /* = NULL */ )
{
   ...
   useNVPerfHud = Con::getBoolVariable("$pref::Video::useNVPerfHud", false);
#endif
...
HRESULT hres = E_FAIL;
   ...
   else
   {
   ...
   #if !defined(POWER_DEBUG_RENDER) && !defined(D3D_DEBUG_INFO)
      ...
      if (FAILED(hres) && hres != D3DERR_OUTOFVIDEOMEMORY)
      {
         ...
         hres = mD3D->CreateDevice( D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, 
               ((Win32Window*)window)->getHWND(), deviceFlags, 
               &d3dpp, &mD3DDevice );
...
//-----------------------------------------------------------------------------
GFXTexHandle &GFXPCD3D9Device::getSfxBackBuffer(int Idx)
{
	AssertFatal(Idx<MAX_BACKBUFFER_NUM,"超过SfxBackBuffer的最大数量");

	if(!mSfxBackBuffer[Idx] ||  mSfxBackBuffer[Idx].getHeight() !=m_d3dpp.BackBufferHeight-1 ||  mSfxBackBuffer[Idx].getWidth() !=m_d3dpp.BackBufferWidth-1)
	{
		mSfxBackBuffer[Idx].set( m_d3dpp.BackBufferWidth-1, m_d3dpp.BackBufferHeight-1, GFXFormatR8G8B8, &GFXDefaultRenderTargetProfile );
	}

	return mSfxBackBuffer[Idx];
}

void GFXPCD3D9Device::copyBBToSfxBuff(int Idx)
{
	AssertFatal(Idx<MAX_BACKBUFFER_NUM,"超过SfxBackBuffer的最大数量");
   ...
   GFXD3D9TextureObject *texObj = (GFXD3D9TextureObject*)(GFXTextureObject*)mSfxBackBuffer[Idx];
...
void GFXPCD3D9Device::copyBBToBuff(GFXTexHandle &TexBuff)
{
	if( !TexBuff)
		return;

	IDirect3DSurface9 *surf;
	GFXD3D9TextureObject *texObj = (GFXD3D9TextureObject*)(GFXTextureObject*)TexBuff;
	texObj->get2DTex()->GetSurfaceLevel( 0, &surf );
	mD3DDevice->StretchRect( mDeviceBackbuffer, NULL, surf, NULL, D3DTEXF_NONE );
...
void GFXPCD3D9Device::validateMultisampleParams(D3DFORMAT format, BOOL windowed)
{
   if (mMultisampleType != D3DMULTISAMPLE_NONE)
   {
      DWORD MaxSampleQualities;      
      HRESULT hres = mD3D->CheckDeviceMultiSampleType(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, format, windowed, D3DMULTISAMPLE_NONMASKABLE, &MaxSampleQualities);
	  // <Edit> [4/20/2009 joy] 这里暂时不判断PixelShaderVersion
...
void GFXPCD3D9Device::reset( D3DPRESENT_PARAMETERS &d3dpp )
{
   ...
   validateMultisampleParams(d3dpp.BackBufferFormat, d3dpp.Windowed);
   ...
   {
      Con::printf("GFXPCD3D9Device::reset - depthstencil 0x%p has %d ref's", mDeviceDepthStencil, mDeviceDepthStencil->AddRef()-1);
