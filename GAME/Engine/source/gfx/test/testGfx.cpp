...
public:
   ...
   void go()
   {
      ...
      // Setup our events.
      mWindow->displayEvent.notify(this, &SimpleGFXRenderFramework::handleDrawEvent);
      mWindow->appEvent.notify    (this, &SimpleGFXRenderFramework::handleAppEvent);
      mWindow->idleEvent.notify   (this, &SimpleGFXRenderFramework::handleIdleEvent);
      ...
      // Clean everything up.
      mWindow->displayEvent.remove(this, &SimpleGFXRenderFramework::handleDrawEvent);
      mWindow->appEvent.remove    (this, &SimpleGFXRenderFramework::handleAppEvent);
      mWindow->idleEvent.remove   (this, &SimpleGFXRenderFramework::handleIdleEvent);
   }
...
CreateInteractiveTest(TestGFXWireCube, "GFX/WireCube")
{
   ...
   void run()
   {
      mRenderer.renderSignal.notify(this, &TestGFXWireCube::drawCB);
      mRenderer.go();
      mRenderer.destroy();
   }
};
...
CreateInteractiveTest(TextGFXTextRender, "GFX/TextRender")
{
   ...
   void handleDrawEvent(GFXDevice* d)
   {
      if(mFont.isNull())
      ...
      //d->drawText(mFont, Point2I(x/10, y), "The quick brown fox jumps over the lazy dog 1234567890", NULL);
   }
   void run()
   {
      mRenderer.renderSignal.notify(this, &TextGFXTextRender::handleDrawEvent);
      mRenderer.go();
      mFont = NULL;
...
CreateInteractiveTest(TestGFXLineDraw, "GFX/Line")
{
   ...
   void run()
   {
      mRenderer.renderSignal.notify(this, &TestGFXLineDraw::handleDrawEvent);
      mRenderer.go();
      mRenderer.destroy();
   }
};
...
CreateInteractiveTest(TestGFXRectOutline, "GFX/RectOutline")
{
   ...
   void run()
   {
      mRenderer.renderSignal.notify(this, &TestGFXRectOutline::handleDrawEvent);
      mRenderer.go();
      mRenderer.destroy();
   }
};
...
CreateInteractiveTest(TestGFXDrawBitmap, "GFX/DrawBitmap")
{
   ...
   void run()
   {
      mRenderer.renderSignal.notify(this, &TestGFXDrawBitmap::handleDrawEvent);
      mRenderer.go();
      mRenderer.destroy();
   }
};
...
CreateInteractiveTest(TestGFXDraw2DSquare, "GFX/Draw2DSquare")
{
   ...
   void run()
   {
      mRenderer.renderSignal.notify(this, &TestGFXDraw2DSquare::handleDrawEvent);
      mRenderer.go();
      mRenderer.destroy();
   }
};
...
CreateInteractiveTest(TestGFXRectFill, "GFX/RectFill")
{
   ...
   void run()
   {
      mRenderer.renderSignal.notify(this, &TestGFXRectFill::handleDrawEvent);
      mRenderer.go();
      mRenderer.destroy();
   }
};
...
CreateInteractiveTest(TestGFXViewport, "GFX/Viewport")
{
   ...
   void run()
   {
      mRenderer.renderSignal.notify(this, &TestGFXViewport::handleDrawEvent);
      mRenderer.go();
      mRenderer.destroy();
   }
};

// This test will display a green square in the middle of the screen about 1/5 the size of the 
// screen.  This tests setting a cliprect with an alternative coordinate system.
CreateInteractiveTest(TestGFXSetClipRectOrtho, "GFX/SetClipRectOrtho")
{
   SimpleGFXRenderFramework mRenderer;

   // Made static so I can call this from other tests.
   void handleDrawEvent(GFXDevice *d)
   {
      // Cheesy, but the screen rect will get clipped, so no harm no foul, right?
      RectI orth(253952,253952,8192,8192);
      d->setClipRectOrtho(RectI(0,0,2048,2048), orth);
      ...
      const F32 center = orth.point.x + orth.extent.x / 2;
      const F32 size = orth.extent.x / 10;
      const F32 neg = center - size;
      const F32 pos = center + size;

      PrimBuild::vertex3f( neg, neg, 0.0 );

      PrimBuild::vertex3f( neg,  pos, 0.0 );

      PrimBuild::vertex3f(  pos,  pos, 0.0 );

      PrimBuild::vertex3f(  pos, neg, 0.0 ); 
      PrimBuild::end();
   }

   void run()
   {
      mRenderer.renderSignal.notify(this, &TestGFXSetClipRectOrtho::handleDrawEvent);
      mRenderer.go();
      mRenderer.destroy();
   }
};
...
CreateInteractiveTest(TestGFXSolidCube, "GFX/SolidCube")
{
   ...
   void run()
   {
      mRenderer.renderSignal.notify(this, &TestGFXSolidCube::handleDrawEvent);
      mRenderer.go();
      mCube.destroy();
      mRenderer.destroy();
   }
...
CreateInteractiveTest(TestGFXLitBunny, "GFX/LitBunny")
{
   ...
   void run()
   {
      mRenderer.renderSignal.notify(this, &TestGFXLitBunny::handleDrawEvent);
      mRenderer.go();
      mRenderer.destroy();
   }
};
...
CreateInteractiveTest(TestGFXTextureCube, "GFX/TextureCube")
{
   ...
   void run()
   {
      mRenderer.renderSignal.notify(this, &TestGFXTextureCube::handleDrawEvent);
      mRenderer.go();
      mTex = NULL;
      mCube.destroy();
      mRenderer.destroy();
...
CreateInteractiveTest(TestGFXTextureLock, "GFX/TextureLock")
{
   ...
   void run()
   {
      mRenderer.renderSignal.notify(this, &TestGFXTextureLock::handleDrawEvent);
      mRenderer.go();
      mTex = NULL;
      mCube.destroy();
      mRenderer.destroy();
...
CreateInteractiveTest(TestGFXMultiTextureCube, "GFX/MultiTextureCube")
{
	...
	void run()
	{
		mRenderer.renderSignal.notify(this, &TestGFXMultiTextureCube::handleDrawEvent);
		mRenderer.go();
      mTex0 = NULL;
      mTex1 = NULL;
      mCube.destroy();
...
CreateInteractiveTest(TestGFXRenderTargetCube, "GFX/RenderTargetCube")
{
   ...
   void handleDrawEvent(GFXDevice *d)
   {
      ...
      // Render to the window...
      {
         ...
         d->setWorldMatrix(worldMatrix);
         d->clear( GFXClearZBuffer | GFXClearStencil | GFXClearTarget, ColorI( 1.0, 1.0, 1.0 ), 1.0f, 0 );
   ...
   {
      mRenderTarget = NULL;
      mRenderer.renderSignal.notify(this, &TestGFXRenderTargetCube::handleDrawEvent);
      mRenderer.go();
...
CreateInteractiveTest(TestGFXRenderTargetStack, "GFX/RenderTargetStack")
{
   ...
   void run()
   {
      ...
      mRenderer.renderSignal.notify(this, &TestGFXRenderTargetStack::handleDrawEvent);
      mRenderer.go();
...
// Draws a huge green rect to a texture starting at the center, then we render a box using that as the texture.
// The result should be a small green rect in the corner of the cube, clipped to be pretty small.  
CreateInteractiveTest(TestGFXRenderTargetClipRect, "GFX/RenderTargetClipRect")
{
   SimpleGFXRenderFramework mRenderer;
   CubeBuilder mCube;
   GFXTexHandle mTex;
   ...
   void drawCube(GFXDevice *d)
   {
      ...
      d->setupGenericShaders();
      d->setBaseRenderState();
      d->setZFunc(GFXCmpLessEqual);
      //d->setCullMode(GFXCullNone);
      d->setVertexColorEnable(false);
      d->setAlphaBlendEnable(false);
   ...
   void handleDrawEvent(GFXDevice *d)
   {
      ...
      // Update the render target.
      {
         ...
         mRenderTarget->attachTexture(GFXTextureTarget::DepthStencil, GFXTextureTarget::sDefaultDepthStencil);
         d->setActiveRenderTarget(mRenderTarget);
         d->clear( GFXClearZBuffer | GFXClearStencil | GFXClearTarget, ColorI( 255, 255, 255 ), 1.0f, 0 );
         
         RectI orth(253952,253952,8192,8192);         
         d->setClipRectOrtho(RectI(0,0,64,64), orth);
         ...
         const F32 size = orth.extent.x / 10;
         const F32 neg = center - size;
         const F32 pos = center + (size*1000);
      ...
      // Render to the window...
      {
         ...
         F32 spinDiddle=0.0f;
         worldMatrix *= MatrixF(EulerF(0,0, 90.f - spinDiddle ));
         worldMatrix.setPosition(Point3F(0, 5, 0));
   ...
   {
      mRenderTarget = NULL;
      mRenderer.renderSignal.notify(this, &TestGFXRenderTargetClipRect::handleDrawEvent);
      mRenderer.go();
...
CreateInteractiveTest(TestGFXDeviceSwitching, "GFX/DeviceSwitching")
{
   ...
   void handleAppEvent(DeviceId d, U16 event)
   {
      if(event == WindowClose && d == mWindow->getDeviceId())
         Process::requestShutdown();
   }
   ...
   void run()
   {
      ...
      for(S32 i=0; i<adapters.size(); i++)
      {
         ...
         // Hook in our events.
         // Setup our events.
         mWindow->displayEvent.notify(this, &TestGFXDeviceSwitching::handleDrawEvent);
         mWindow->appEvent.notify    (this, &TestGFXDeviceSwitching::handleAppEvent);
         mWindow->idleEvent.notify   (this, &TestGFXDeviceSwitching::handleIdleEvent);
