...
{
   dSprintf(outBuff, buffLen, "%s/%s %d (%s).fnt", Con::getVariable("$GUI::fontCacheDirectory"), faceName, size, getCharSetName(0));
}
...
{
   char buf[256];
   dSprintf(buf, sizeof(buf), "%s/%s %d (%s).fnt", cacheDirectory, faceName, size, getCharSetName(charset));
...
#ifdef _XBOX
   AssertFatal( false, "Not that this message shows up, but font creation is not supported on the Xbox platform" );
#endif
   ...
   if (platFont == NULL)
   {
      ...
      if (dStricmp(faceName, "arial") == 0)
         fontName = "Helvetica";
      else if (dStricmp(faceName, "lucida console") == 0)
         fontName = "Monaco";
      else if (dStricmp(faceName, "monaco") == 0)
         fontName = "Courier";
      else
         AssertISV( false, "Could not create font" );
...
// ----------------------------------------------------------------------------
// 获取字符的轮廓字体或点阵字体的字模像素数据，登记到字符信息列表mCharInfoList
...
const PlatformFont::CharInfo &GFont::getCharInfo(const UTF16 in_charIndex)
{
	...
	PROFILE_END(NewFontGetCharInfo);
...
// ----------------------------------------------------------------------------
// 根据"chinesefont.txt"文件提供的3700多常用汉字生成cache字库
ConsoleFunction(BuildOftenChinese, void, 2, 2, "BuildOftenChinese(%txt)")
{
	Stream* fs = ResourceManager->openStream(argv[1]);
	if(!fs)
		return;
	U32 filelen = fs->getStreamSize();
	char* szBuffer = new char[filelen + 1];
	dMemset(szBuffer, 0, filelen);
	char* p = szBuffer;
	for(int i = 0; i < filelen; i++)
	{
		fs->read(sizeof(char), p++);
	}
	...
	for(int i = 0; i < 5; i++)
	{
		...
		{
			Resource<GFont> f = GFont::create(FontName[i], j, Con::getVariable("$GUI::fontCacheDirectory"));
			if(f.isNull())
			{
				Con::errorf("populateFontCacheString - could not load font '%s %d'!", argv[1], dAtoi(argv[2]));
				return;
                ...
                "exported as one long strip.")
{
   ...
   Resource<GFont> f = GFont::create(fontName, fontSize, Con::getVariable("$GUI::fontCacheDirectory"));
...
bool GFont::read(Stream& io_rStream)
{
	...
	for(i = 0; i < numSheets; i++)
	{
		...
		char buf[30];
		dSprintf(buf, sizeof(buf), "newfont_%d", smSheetIdCount++);
		GFXTexHandle handle = GFXTexHandle(bmp, &GFXFontTextureProfile, true);
