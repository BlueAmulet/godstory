...
extern ResourceInstance* constructNewFont(Stream& stream, ResourceObject *);

GFX_DeclareTextureProfile(GFXFontTextureProfile);

const char FontName[][32] =
{
	"宋体",
	"楷体_GB2312",
	"黑体",
...
{
   friend ResourceInstance* constructNewFont(Stream& stream, ResourceObject *);
...
private:
   ...
   // 字符表纹理（即将若干字符字模数据映射到一张纹理上，用于索引查询后纹理渲染）
   Vector<GFXTexHandle>mTextureSheets;		// 字符表纹理列表

   S32 mCurX;
   S32 mCurY;
   S32 mCurSheet;

   bool mNeedSave;							// 是否需要保存
   ...
   // 字符信息列表，必须通过getCharInfo(U32)方法来访问获取
   Vector<PlatformFont::CharInfo>  mCharInfoList;

   // 字符信息列表的索引数组，例如: mRemapTable[1837] = 19; 其中1837指的是'中'的字符编码
								   //19为mCharInfoList的数组下标，mCharInfoList[19]
