...
bool GBitmap::readDDS(Stream& io_rStream)
{
   dds = new DDSFile();
   isDDSFile = dds->read(io_rStream);

   return isDDSFile;
}

bool GBitmap::writeDDS(Stream& io_rStream) const
{
   return false;
}
...
DDSFile::DDSFile(DDSFile &source)
{
	mFlags		= source.mFlags;
	mHeight		= source.mHeight;
	mWidth		= source.mWidth;
	mDepth		= source.mDepth;
	mPitchOrLinearSize = source.mPitchOrLinearSize;
	mMipMapCount	= source.mMipMapCount;
	mHasAlpha	=	source.mHasAlpha;

	mFormat		=	source.mFormat;
	mBytesPerPixel	= source.mBytesPerPixel; 
	mFourCC		=	source.mFourCC;

	SurfaceData *pData;
	U8 *pMip;
	for(int i=0;i<source.mSurfaces.size();i++)
	{
		pData = new SurfaceData();
		mSurfaces.push_back(pData);
		for(int j=0;j<source.mSurfaces[i]->mMips.size();j++)
		{
			U32 size = source.getSurfaceSize(mHeight, mWidth, j);
			pMip = new U8[size];
			dMemcpy(pMip,source.mSurfaces[i]->mMips[j],size);
			pData->mMips.push_back(pMip);
		}
	}
}
...
ConsoleFunction(getActiveDDSFiles, S32, 1, 1, "() - Returns active DDSes in memory!")
{
