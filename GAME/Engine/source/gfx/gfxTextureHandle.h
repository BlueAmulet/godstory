...
public:
   ...
   // load texture
   GFXTexHandle( StringTableEntry texName, GFXTextureProfile *profile, bool noWarning = false);
   bool set( StringTableEntry texName, GFXTextureProfile *profile, bool noWarning = false );
   ...
   bool readBackBuffer( Point2I &upperLeft ) { return getPointer() ? getPointer()->readBackBuffer( upperLeft ) : false; }
   ...
   //---------------------------------------------------------------------------
   GBitmap * getBitmap();
	bool getLegal(){return getPointer()->getLegal();}
