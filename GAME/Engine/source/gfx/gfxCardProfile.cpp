...
void GFXCardProfiler::loadProfileScript(const char* aScriptName)
{
   ...
   dStrcpy(scriptName, sizeof(scriptName), (dStrlen( profilePath ) > 0) ? profilePath : "profile");
   dStrcat(scriptName, sizeof(scriptName), "/");
   dStrcat(scriptName, sizeof(scriptName), aScriptName);
...
GFXCardProfiler::~GFXCardProfiler()
{
	...
	Map<const char*, U32>::Iterator it = mCapDictionary.begin();
	while(it != mCapDictionary.end())
	{
		tmpList.push_back(it->key);
		++it;
	}
