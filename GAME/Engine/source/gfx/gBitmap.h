...
extern ResourceInstance* constructBitmapBMP(Stream& stream, ResourceObject *);
extern ResourceInstance* constructBitmapPNG(Stream& stream, ResourceObject *);
extern ResourceInstance* constructBitmapJPEG(Stream& stream, ResourceObject *);
extern ResourceInstance* constructBitmapGIF(Stream& stream, ResourceObject *);
extern ResourceInstance* constructBitmapMNG(Stream& stream, ResourceObject *);
extern ResourceInstance* constructBitmapDBM(Stream& stream, ResourceObject *);
extern ResourceInstance* constructBitmapDDS(Stream& stream, ResourceObject *);
...
class GBitmap: public ResourceInstance
{
  ...
  public:
   ...
   bool		   TempSetFormatForBmp(GFXFormat fmt);
   ...
   inline U8*  getBits(const S32 in_x, const S32 in_y, const U32 mipLevel = 0);
   inline const U8* getBits(const S32 in_x, const S32 in_y, const U32 mipLevel = 0) const;
   ...
   // CodeReview: For now, I am supporting add and subtract ops. RGB, and RGBX texture formats [5/11/2007 Pat]
...
//   bool preserveSize;
  ...
  public:
   ...
   bool readDDS(Stream& io_rStream);               // located in ddsLoader.cpp
   bool writeDDS(Stream& io_rStream) const;        // located in ddsLoader.cpp
...
inline U32 GBitmap::getWidth(const U32 in_mipLevel) const
{
   if (isDDSFile)
   {
      AssertFatal(in_mipLevel < dds->getNumMipLevels(),
                  avar("GBitmap::getWidth: mip level out of range: (%d, %d)",
                       in_mipLevel, dds->getNumMipLevels()));

      return dds->getWidth(in_mipLevel);
   }
   else
   {
...
inline U32 GBitmap::getHeight(const U32 in_mipLevel) const
{
   ...
   {
      AssertFatal(in_mipLevel < dds->getNumMipLevels(),
                  avar("GBitmap::getHeight: mip level out of range: (%d, %d)",
                       in_mipLevel, dds->getNumMipLevels()));

      return dds->getHeight(in_mipLevel);
   }
   else
   {
      AssertFatal(in_mipLevel < numMipLevels,
...
inline U32 GBitmap::getDepth(const U32 in_mipLevel) const
{
   ...
   {
      AssertFatal(in_mipLevel < dds->getNumMipLevels(),
                  avar("GBitmap::getDepth: mip level out of range: (%d, %d)",
                       in_mipLevel, dds->getNumMipLevels()));

      return dds->getDepth(in_mipLevel);
   }
   else
   {
      AssertFatal(in_mipLevel < numMipLevels,
...
U8* GBitmap::getBits(const S32 in_x, const S32 in_y, const U32 mipLevel)
{
	return ((U8*)getBits(mipLevel) + ((in_y * getWidth(mipLevel)) + in_x) * bytesPerPixel);
}

const U8* GBitmap::getBits(const S32 in_x, const S32 in_y, const U32 mipLevel) const
{
...
inline bool GBitmap::hasTransparentPixels() const
{
   if (isDDSFile) 
      return dds->hasTransparentPixels();
   else
...
{
   const U32 memSize = getWidth() * getHeight() * bytesPerPixel;
