...
{
   public:
		virtual bool getLegal(){return false;}
		virtual void savePng(const char* path);
#ifdef POWER_DEBUG
     ...
     bool hasTransparency;
      ...
      /// Copy the back buffer into this texture. This texture will contain back
      /// buffer data from the rectangle:
      /// RectI( upperLeft, Point2I( getWidth(), getHeight() ) )
      ///
      /// @note This texture must use a render target profile
      /// @param upperLeft Coordinates on the screen to start the capture
	  ...
	  bool setDirty(bool flag=false){bool tmp = mDirty; mDirty = flag; return tmp;}
};
