...
GFXTextureObject *GFXTextureManager::createTexture( GBitmap *bmp, GFXTextureProfile *profile, bool deleteBmp)
{
   ...
   if (bmp->isDDS())
   {
      GFXTextureObject *ret = createTexture(bmp->dds, profile, deleteBmp, fileName);	 
	  if (deleteBmp)
	  {
		  bmp->dds = NULL;
		  delete bmp;
   ...
   ret->hasTransparency = realBmp->hasTransparentPixels();
...
GFXTextureObject *GFXTextureManager::createTexture( DDSFile *dds, GFXTextureProfile *profile, bool deleteDDS, StringTableEntry fileName)
{
   BACKGROUND_LOAD_LOCK;
   ...
   if (fileName) 
   {
       ret->mTextureFileName = StringTable->insert(fileName);
   } 
   else 
   {
   ...
   ret->hasTransparency = dds->hasTransparentPixels();
...
GFXTextureObject *GFXTextureManager::createTexture( const char *filename, GFXTextureProfile *profile, bool noWarning )
{
   BACKGROUND_LOAD_LOCK;
...
// Delete texture
//-----------------------------------------------------------------------------
void GFXTextureManager::deleteTexture( const char* filename )
{
    ResourceObject * ro = GBitmap::findBmpResource(filename);
    if (ro)
    {
        StringTableEntry fileName = ro->getFullPath();
        GFXTextureObject * textureObject = hashFind(fileName);
        if ( textureObject )
...
ConsoleFunction( reloadTextures, void, 1, 1, "" )
{
   if( ResourceManager )
   {
      ...
      for ( ResourceObject * obj = ResourceManager->getNextResource(); obj != NULL; obj = ResourceManager->getNextResource() )
      {
         if( obj->crc != InvalidCRC )
         {
            ...
            if( stream )
            {
               ...
               if( crc != obj->crc )
               {
                  ...
                  char filename[256];
                  dStrcpy( filename, sizeof(filename), obj->path );
                  dStrcat( filename, sizeof(filename), "/" );
                  dStrcat( filename, sizeof(filename), obj->name );
