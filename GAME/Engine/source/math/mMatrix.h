...
{
private:
   F32 m[16];     ///< Note: PowerEngine uses column major matrices (WRONG)
                  ///<       PowerEngine uses ROW MAJOR matrices. 
...
public:
   ...
   MatrixF& affineInverse();                    ///< Take inverse of matrix assuming it is affine (rotation, scale, sheer, translation only).
   ...
   const static MatrixF smIdentity;
...
inline void MatrixF::getRow(S32 row, Point4F *cptr) const
{
   row *= 4;
   cptr->x = m[row++];
   cptr->y = m[row++];
...
inline void MatrixF::getRow(S32 row, Point3F *cptr) const
{
   row *= 4;
   cptr->x = m[row++];
   cptr->y = m[row++];
...
inline void MatrixF::setRow(S32 row, const Point4F &cptr)
{
   row *= 4;
   m[row++] = cptr.x;
   m[row++] = cptr.y;
...
inline void MatrixF::setRow(S32 row, const Point3F &cptr)
{
   row *= 4;
   m[row++] = cptr.x;
   m[row++] = cptr.y;
