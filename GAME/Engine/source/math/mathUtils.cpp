...
bool interpolateV3(F32 * pCurr,F32 * pEnd,F32* pStep)
{
	bool ret = false;
	...
	{
		if(pEnd[i] > pCurr[i])
			pCurr[i] = mClampF(pCurr[i] + pStep[i], pCurr[i], pEnd[i]);
		else if(pEnd[i] < pCurr[i])
			pCurr[i] = mClampF(pCurr[i] + pStep[i], pEnd[i], pCurr[i]);
...
bool interpolateColor(F32 * pCurr,F32 * pEnd,F32 step)
{
	bool ret = false;
	...
	{
		if(pEnd[i] > pCurr[i])
			pCurr[i] = mClampF(pCurr[i] + step, 0.f, pEnd[i]);
		else if(pEnd[i] < pCurr[i])
			pCurr[i] = mClampF(pCurr[i] - step, pEnd[i], 1.f);
