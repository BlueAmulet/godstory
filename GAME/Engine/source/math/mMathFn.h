...
inline bool mIsZero(const F32 val, const F32 epsilon = 0.0001f )
{
...
inline F32 mLogBase(const F32 val, const F32 base)
{
...
inline F64 mLogBase(const F64 val, const F64 base)
{
   return (F64) log(val)/log(base);
}
...
inline F32 clampToZero(F32& input)
{
...
{
   // If x is a floating point variable, then (x != x) will be TRUE iff x has the value NaN. 
