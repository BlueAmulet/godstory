...
class Point2F
{
  ...
  public:
   ...
   operator const F32*() const { return (F32*)(&x); }
...
class Point3F
{
  ...
  public:
   ...
   #pragma message(ENGINE(新增const参数的equal方法))
   bool  equal( const Point3F &compare );
...
public:
	...
	Point3HF(const Point3F& pt)
	{
		x = half(pt.x);
		y = half(pt.y);
		z = half(pt.z);
	}
	Point3HF(const half _x, const half _y, const half _z)
	{
		x = _x;
		y = _y;
		z = _z;
...
public:
	...
	Point4HF(const half _x, const half _y, const half _z,const half _w)
	{
		x = _x;
		y = _y;
		z = _z;
	...
	{
		Point4F tmp;
		tmp.set(float(x),float(y),float(z),float(w));
		return tmp;
	}
};
//------------------------------------------------------------------------------
