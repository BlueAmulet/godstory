...
static const char ipSize = sizeof(int);
static const char portSize = sizeof(short);
static const char crcSize = sizeof(int);

static const int MinPacketDataSize = ipSize+portSize+crcSize;
...
PacketReceiveExEvent	Net::smPacketReceiveEx;

stdext::hash_set<int>	Net::hashSocketMap;
							   ...
							   char* remoteAddr = NULL, S32 port = -1)
{
	...
	sock->state = state;
	if (remoteAddr)
		dStrcpy(sock->remoteAddr, sizeof(sock->remoteAddr), remoteAddr);
	if (port != -1)
...
void dsp(char *pData,int &size)
{
	U32 ret = calculateCRC(pData, size, 0xFFFFFFFF);
	U32 *pCrc = (U32 *)&pData[size];
	*pCrc = ret;
	size += crcSize;
...
int Net::mLastRecvTime = 0;
int Net::mLastSendTime = 0;
int Net::mSendSize = 0;
int Net::mRecvSize = 0;
int Net::mRSPS = 0;
int Net::mSSPS = 0;
...
bool Net::init()
{
#if defined( POWER_OS_WIN32 ) || defined( POWER_OS_XENON )
	if(!initCount)
	{
...
NetSocket Net::openConnectTo(const char *addressString)
{
	...
	char remoteAddr[256];
	dStrcpy(remoteAddr, sizeof(remoteAddr), addressString);
...
int Net::openPortEx(S32 port)
{
	int	udpS = socket(AF_INET, SOCK_DGRAM, 0);

	if(udpS != InvalidSocket)
	{
		Net::Error error;
		error = bind(udpS, port);

		if(error == NoError)
			error = setBufferSize(udpS, 32768);

		if(error == NoError)
			error = setBroadcast(udpS, true);

		if(error == NoError)
			error = setBlocking(udpS, false);

		if(error == NoError)
			Con::printf("New UDP initialized on port %d", port);
		else
		{
			::closesocket(udpS);
			Con::printf("Unable to initialize UDP - error %d", error);
...
void Net::closePortEx()
{
	stdext::hash_set<int>::iterator it;

	for( it = hashSocketMap.begin(); it != hashSocketMap.end(); it++ )
	{
		if( *it != InvalidSocket )
			::closesocket( *it );
	}
...
Net::Error Net::sendto(const NetAddress *address, const U8 *buffer, S32  bufferSize)
{
	...
	mSendSize += bufferSize;
	if( GetTickCount() - mLastSendTime > 1000 )
	{
		mSSPS = mSendSize;
		mSendSize = 0;
		mLastSendTime = GetTickCount();
	}

	//Con::printf("packetSize = %d",bufferSize);
...
#ifdef  NTJ_CLIENT
		dsp((char *)buffer,bufferSize);
#endif

		//Ray: 通过GateServer转发数据
		int *pIp = (int *)&buffer[bufferSize];
		short *pPort = (short *)&buffer[bufferSize + ipSize];
		*pIp = ipAddr.sin_addr.s_addr;
		*pPort = ipAddr.sin_port;
		bufferSize+=ipSize+portSize;

#ifdef NTJ_CLIENT
		ipAddr.sin_addr.s_addr = g_ClientGameplayState->getGateIP();
		ipAddr.sin_port = g_ClientGameplayState->getGatePort();

#elif NTJ_SERVER
		ipAddr.sin_addr.s_addr = g_ServerGameplayState->getGateIP();
		ipAddr.sin_port = g_ServerGameplayState->getGatePort();
#endif
#endif
        ...
        AuthCrypt::Instance()->EncryptSend((uint8*)buffer,bufferSize);
#endif
...
Net::Error Net::sendtoEx(const NetAddress *address, int udpS, const U8 *buffer, S32  bufferSize)
{
	if(Journal::IsPlaying())
		return NoError;
	...
	if(address->type == NetAddress::IPAddress)
	{
		...
		*pPort = ipAddr.sin_port;
		...
		if(::sendto(udpS, (const char*)buffer, bufferSize, 0,
			(sockaddr *) &ipAddr, sizeof(sockaddr_in)) == SOCKET_ERROR)
			return getLastError();
		else
			return NoError;
...
void Net::process()
{
	...
	RawData tmpBuffer;
	tmpBuffer.alloc(MaxPacketBuffSize);
	...
	// here we handle addition udp sockets
	stdext::hash_set<int>::iterator it;
	...
	for( it = hashSocketMap.begin(); it != hashSocketMap.end(); it++ )
	{
		...
		for(;;)
		{
			...
			if(udpS != InvalidSocket)
				bytesRead = recvfrom(udpS, (char *) tmpBuffer.data, MaxPacketBuffSize, 0, &sa, &addrLen);

			if(bytesRead == -1)
				break;

#ifdef ENABLE_ENCRPY_PACKET
            AuthCrypt::Instance()->DecryptRecv((uint8*)tmpBuffer.data,bytesRead);
#endif
...
#ifdef  NTJ_NETGATE
				//Ray: 通过GateServer转发数据
				if(bytesRead < MinPacketDataSize)
					continue;

				pPort = (unsigned short *)(&tmpBuffer.data[bytesRead-portSize]);
				pIp = (int *)(&tmpBuffer.data[bytesRead-(ipSize+portSize)]);

				tmp.sin_family = sa.sa_family;
				tmp.sin_port = *pPort;
				tmp.sin_addr.s_addr = *pIp;
				dMemcpy(&sa,&tmp,sizeof(sockaddr));

				bytesRead-=ipSize+portSize;

#ifdef NTJ_SERVER
				if(!udsp(tmpBuffer.data,bytesRead))
					continue;
#endif
			...
			Net::smPacketReceiveEx.trigger(srcAddress, udpS, tmpBuffer);
	...
	for(;;)
	{
		...
		if(udpSocket != InvalidSocket)
			bytesRead = recvfrom(udpSocket, (char *) tmpBuffer.data, MaxPacketBuffSize, 0, &sa, &addrLen);
		...
		tmpBuffer.size = bytesRead;
		//Con::printf("PACKET SIZE:%d", bytesRead );

		mRecvSize += bytesRead;
		if( GetTickCount() - mLastRecvTime > 1000 )
		{
			mRSPS = mRecvSize;
			mRecvSize = 0;
		...
		/* no increment, this is done at end of loop body */)
	{
		...
		case ::Connected:
			...
			bytesRead = 0;
			readBuff.alloc(MaxPacketBuffSize);
			err = Net::recv(currentSock->fd, (U8*)readBuff.data, MaxPacketBuffSize, &bytesRead);
			if(err == Net::NoError)
			{
				if (bytesRead > 0)
				{
...
ConsoleFunction( SptNet_GetSSPS, int, 1,1, "" )
{
	return Net::mSSPS;
}

ConsoleFunction( SptNet_GetRSPS, int, 1,1, "" )
{
	return Net::mRSPS;
}
