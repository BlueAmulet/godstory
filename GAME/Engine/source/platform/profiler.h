...
///
/// First, you must #define POWER_ENABLE_PROFILER in profiler.h in order to
...
public:
   ...
   void dumpToFile(const char *fileName,bool isBinary = false);

   // dump to file in binary format
   void dumpToFileInBinary(const char* fileName);

   /// Enable profiling
   void enable(bool enabled);
   bool isEnabled() { return mNextEnable; }
   /// Helper function for macro definition PROFILE_START
   void hashPush(ProfilerNodeData *data);
   /// Helper function for macro definition PROFILE_END
   //void hashPop(ProfilerNodeData *expected=NULL);
   void hashPop(const char* name);
...
struct ProfilerData
{
    ProfilerNodeData* pNode; ///< link to root node.
...
struct ProfilerNodeData
{
   ...
   static ProfilerNodeData *sRootList;

   ProfilerNodeData(const char *name);
};

#define PROFILE_START(name) \
static ProfilerNodeData pdata##name##obj (#name); \
if(gProfiler) gProfiler->hashPush(& pdata##name##obj )

#define PROFILE_END(name) if(gProfiler) gProfiler->hashPop(#name)
...
{
public:
   ScopedProfiler(ProfilerNodeData *data) 
   {
	   m_data = data;
   ...
   ~ScopedProfiler() 
   {
      if (gProfiler) gProfiler->hashPop(m_data->mName);
   }

private:
	ProfilerNodeData* m_data;
};
