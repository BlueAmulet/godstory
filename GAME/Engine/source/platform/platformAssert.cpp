...
static bool askToEnterDebugger(const char* message )
{
   ...
   if(!haveAsked )
   {
      ...
      dSprintf( tempBuff, 1024, "PowerEngine has encountered an assertion with message\n\n"
         "%s\n\n"
         "Would you like to use the debugger? If you cancel, you won't be asked"
         " again until you restart PowerEngine.", message);
...
bool PlatformAssert::processOutput(Type assertType, const char* outstr)
{
	if(assertType == Fatal)
		Con::errorf(outstr);
	else
