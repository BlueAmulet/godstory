...
public:
   ...
   static bool processOutput(Type assertType, const char* outstr);
};

#ifdef DEBUG
#ifndef POWER_ENABLE_ASSERTS
#define POWER_ENABLE_ASSERTS
   ...
   #define AssertRelease(x, y)  AssertFatal(x, y)

#else
   #define AssertFatal(x, y)   { if (((bool)(x))==(bool)0) PlatformAssert::processOutput(PlatformAssert::Fatal, y);}
   #define AssertWarn(x, y)    { if (((bool)(x))==(bool)0) PlatformAssert::processOutput(PlatformAssert::Warning, y);}

   // <Edit> [3/4/2009 joy] 增加Release/Debug下都可用的Assert
   #define AssertRelease(x, y)       \
         { if (((bool)(x))==(bool)0) \
            { if ( PlatformAssert::processAssert(PlatformAssert::Fatal, "", 0,  y) ) { Platform::debugBreak(); } } }
#endif
