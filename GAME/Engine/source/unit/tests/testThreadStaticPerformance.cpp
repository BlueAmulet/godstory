...
#if defined(POWER_ENABLE_THREAD_STATICS) && defined(POWER_ENABLE_PROFILER)
...
CreateUnitTest( TestThreadStaticPerformance, "Core/ThreadStaticPerformance" )
{
   void run()
   {
      ...
      PROFILE_END(ThreadStaticPerf_TrueStaticAssign);

      PROFILE_START(ThreadStaticPerf_InstanceStaticAssign);
      for( int i = 0; i < TEST_SIZE; i++ )
         ATTS_( gInstancedStaticFoo, 1 ) = testValue[i];
      PROFILE_END(ThreadStaticPerf_InstanceStaticAssign);
