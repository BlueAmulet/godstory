...
class CachedInterfaceExampleComponent : public SimObject
{
   typedef SimObject Parent;

   ValueWrapperInterface<U32> mMyId;
   static U32 smNumInstances;
   ValueWrapperInterface<U32> *mpU32; // CodeReview [patw, 2, 17, 2007] Make ref objects when this is in Jugg
...
public:
   ...
   bool onComponentRegister( SimComponent *owner )
   {
      ...
      {
         mpU32 = dynamic_cast<ValueWrapperInterface<U32> *>( (*i) );
...
CreateUnitTest(TestComponentInterfacing, "Components/Composition")
{
   void run()
   {
      ...
      SimObject *testComponent = new SimObject();
