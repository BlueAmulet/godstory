...
//---------------------------------------------------------------------------
//////#define STATEBLOCK
GFXStateBlock* VolumeLight::mSetSB = NULL;
GFXStateBlock* VolumeLight::mStoreSB = NULL;
GFXStateBlock* VolumeLight::mClearSB = NULL;
...
U64 VolumeLight::packUpdate(NetConnection * con, U64 mask, BitStream * stream)
{
   // Pack Parent.
...
void VolumeLight::renderObject(SceneState *state, RenderInst *ri)
{
	...
	AssertFatal(mSetSB, "VolumeLight::renderObject -- mSetSB cannot be NULL.");
	mSetSB->apply();
	AssertFatal(mStoreSB, "VolumeLight::renderObject -- mStoreSB cannot be NULL.");
	mStoreSB->capture();
	...
	// Cleanup
	AssertFatal(mClearSB, "VolumeLight::renderObject -- mClearSB cannot be NULL.");
	mClearSB->apply();
	AssertFatal(mStoreSB, "VolumeLight::renderObject -- mStoreSB cannot be NULL.");
	mStoreSB->apply();
...
void VolumeLight::renderObject(SceneState *state, RenderInst *ri)
{
   ...
   GFX->setTextureStageAlphaOp(0, (GFXTextureOp)alphaOp);
...
void VolumeLight::resetStateBlock()
{
	//mSetSB
	GFX->beginStateBlock();
...
void VolumeLight::releaseStateBlock()
{
	if (mSetSB)
	{
		mSetSB->release();
