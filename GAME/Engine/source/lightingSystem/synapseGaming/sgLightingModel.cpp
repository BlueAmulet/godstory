...
ConsoleFunction(lightingModelCount, S32, 1, 1, "Get the number of Lighting Pack Lighting Models.")
{
	return sgLightingModelManager::sgGetLightingModelCount();
}

ConsoleFunction(lightingModelName, const char *, 2, 2, "Get the name of the Lighting Pack Lighting Model.")
{
	U32 index;
	index = dAtoi(argv[1]);
...
{
   // Currently unsupported by fixed function
   if (GFX->getPixelShaderVersion() == 0.0)
      return NULL;

   PROFILE_SCOPE(sgLightingModel_sgGetDynamicLightingTextureOmni);
...
{
   PROFILE_SCOPE(sgLightingModel_sgGetDynamicLightingTextureSpot);
	// load these on demand...
	if(!sgGeneratedDynamicLightingTexture)
	{
		sgGenerateDynamicLightingTexture();
...
void sgLightingModel::cleanup()
{
   sgDynamicLightingTextureSpot = NULL;
   sgDynamicLightingTextureOmni = NULL;
}
...
void sgLightingModel::sgGenerateDynamicLightingTexture()
{
	sgLightInfo *oldlight = sgLight;

	// setup basic light->..
	sgLightInfo* light = SGLM->createSGLightInfo();
	light->mType = LightInfo::Point;
	light->mRadius = 10.0;
	light->mColor = ColorF(1.0, 1.0, 1.0);
	light->sgLocalAmbientAmount = 1.0;
   ...
   // upload...
   sgDynamicLightingTextureOmni = GFX->getTextureManager()->createTexture(SG_DYNAMIC_LIGHTING_TEXTURE_SIZE,
      SG_DYNAMIC_LIGHTING_TEXTURE_SIZE, SG_DYNAMIC_LIGHTING_TEXTURE_SIZE,
      bits, GFXFormatL8, &GFXDefaultStaticDiffuseProfile);
   ...
   for(U32 x=0; x<SG_DYNAMIC_LIGHTING_TEXTURE_SIZE; x++)
   {
	...
	// upload...
	sgDynamicLightingTextureSpot = GFX->getTextureManager()->createTexture(SG_DYNAMIC_LIGHTING_TEXTURE_SIZE,
		SG_DYNAMIC_LIGHTING_TEXTURE_SIZE, SG_DYNAMIC_LIGHTING_TEXTURE_SIZE,
		bits, GFXFormatL8, &GFXDefaultStaticDiffuseProfile);
	...
	sgTestLightingTexture = GFX->getTextureManager()->createTexture(SG_DYNAMIC_LIGHTING_TEXTURE_SIZE,
		SG_DYNAMIC_LIGHTING_TEXTURE_SIZE, SG_DYNAMIC_LIGHTING_TEXTURE_SIZE,
		bits, GFXFormatL8, &GFXDefaultStaticDiffuseProfile);
...
#pragma message(ENGINE(分值比重的计算考虑物体包围球的半径和灯光的辐射半径))
	F32 radsq = (light->mRadius + sphere.radius) * (light->mRadius + sphere.radius);
	if(radsq <= 0.0f)
		return 0.0f;
...
void sgLightingModelAdvanced::sgSetState(sgLightInfo *light)
{
...
void sgLightingModelGLBase::sgSetState(sgLightInfo *light)
{
...
void sgLightingModelManager::cleanup()
{
   for (U32 i = 0; i < sgLightingModels.size(); i++)
   {
      sgLightingModels[i]->cleanup();
   }
}