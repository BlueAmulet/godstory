                           ...
                           S32 & count32, S32 & count16, S32 & count8)
{
   ...
   if (smReadVersion<17)
   {
      for (i=0; i<(S32)sequences.size(); i++)
      {
         ...
         rearrangeKeyframeData(seq,kfStart[i],(U8*)(memBuffer32+nodeStateStart32),(U8*)(memBuffer16+nodeStateStart16),(U8*)(memBuffer32+objectStateStart),(U8*)(memBuffer32+decalStateStart),3*sizeof(S32),4*sizeof(S16),3*sizeof(S32));
      }
   }
...
void TSShape::rearrangeKeyframeData(Sequence & seq, S32 startKeyframe, U8 * pRot, U8 * pTrans, U8 * pos, U8 * pds, S32 rotSize, S32 tranSize, S32 osSize)
{
...
void TSShape::exportSequences(Stream * s)
{
   ...
   if(smVersion>=30)
   {
	   s->write(defaultRotations.size());
	   for (i=0;i<defaultRotations.size();i++)
	   {
		   s->write(defaultRotations[i].x);
		   s->write(defaultRotations[i].y);
		   s->write(defaultRotations[i].z);
		   s->write(defaultRotations[i].w);
	   }

	   s->write(defaultTranslations.size());
	   for (i=0;i<defaultTranslations.size();i++)
	   {
		   s->write(defaultTranslations[i].x);
		   s->write(defaultTranslations[i].y);
		   s->write(defaultTranslations[i].z);
	   }
   }

   if(smVersion>=31)
   {
	   s->write(objectStates.size());
	   for (i=0;i<objectStates.size();i++)
	   {
		   s->write(objectStates[i].vis);
		   s->write(objectStates[i].frameIndex);
		   s->write(objectStates[i].matFrameIndex);
	   }
   }
...
bool TSShape::importSequences(Stream * s)
{
   ...
   {
	   char msg[1024];
	   dSprintf(msg,1024,"模型节点数%d,而动作档节点数为%d,不匹配请重导",sz,nodes.size());
	   Con::errorf(ConsoleLogEntry::General,msg);
	   return false;
   }
   ...
   for (i=0;i<sz;i++)
   {
      ...
      if (nodeMap[i]<0)
      {
         ...
         //            "Sequence import warning:  sequence node \"%s\" not found in base shape.",names[nameIndex]);
   ...
   S32 initNodeRots = smReadVersion>=30 ? defaultRotations.size() : 0;
   S32 initNodeTrans = smReadVersion>=30 ? defaultTranslations.size() : 0;
   S32 initobjectStates = smReadVersion>=31 ? objectStates.size() : 0;
   ...
   {
	   s->read(&sz);
	   S32 oldSz = defaultRotations.size();
	   defaultRotations.setSize(sz+oldSz);
	   for (i=oldSz;i<sz+oldSz;i++)
	   {
		   s->read(&defaultRotations[i].x);
		   s->read(&defaultRotations[i].y);
		   s->read(&defaultRotations[i].z);
		   s->read(&defaultRotations[i].w);
	   }

	   s->read(&sz);
	   oldSz = defaultTranslations.size();
	   defaultTranslations.setSize(sz+oldSz);
	   for (i=oldSz;i<sz+oldSz;i++)
	   {
		   s->read(&defaultTranslations[i].x);
		   s->read(&defaultTranslations[i].y);
		   s->read(&defaultTranslations[i].z);
	   }
   }
   ...
   {
	   s->read(&sz);
	   S32 oldSz = objectStates.size();
	   objectStates.setSize(sz+oldSz);
	   for (i=oldSz;i<oldSz+sz;i++)
	   {
		   s->read(&objectStates[i].vis);
		   s->read(&objectStates[i].frameIndex);
		   s->read(&objectStates[i].matFrameIndex);
	   }
   }
   ...
   for (i=0;i<sz;i++)
   {
	  ...
	  if(smReadVersion>=30)
	  {
		  seq.defRotation += initNodeRots;
		  seq.defTranslation += initNodeTrans;

		  if(smReadVersion>=31)
			  seq.baseObjectState += initobjectStates;
	  }
