...
bool TSShapeInstance::buildPolyList(AbstractPolyList * polyList, S32 dl)
{
   ...
   if (start<end)
   {
      ...
      for (S32 i=start; i<end; i++)
      {
		 ...
		 TSShapeInstance::smRenderData.currentObjectInstance = mesh;
...
bool TSShapeInstance::castRay(const Point3F & a, const Point3F & b, RayInfo * rayInfo, S32 dl)
{
   ...
   if (start<end)
   {
      ...
      MatrixF mat;
      MatrixF * previousMat = mMeshObjects[start].getTransform();
	  if(previousMat) //Ray: 和下面需要判断的道理一样，SkinMesh的结构和TMesh结构不同
	  {
			mat = *previousMat;
			mat.inverse();
			mat.mulP(a,&ta);
...
bool TSShapeInstance::castShadow(const Point3F & a, const Point3F & b, RayInfo * rayInfo, S32 dl)
{
	// if dl==-1, nothing to do
	if (dl==-1)
		return false;
	...
	if (start<end)
	{
		...
		for (S32 i=start; i<end; i++)
		{
			...
			// collide...
			if (mesh->castShadow(od,ta,tb,mMaterialList,rayInfo))
			{
				clearStatics();
				return true;
			}
...
bool TSShapeInstance::MeshObjectInstance::castShadow(S32 objectDetail, const Point3F & start, const Point3F & end,TSMaterialList* materialList,RayInfo * rayInfo)
{
	TSMesh * mesh = getMesh(objectDetail);
	if (mesh && visible>0.01f)
		return mesh->castShadow(frame,start,end,materialList,rayInfo);
	return false;
}
...
bool TSShapeInstance::buildPolyListOpcode( S32 dl, AbstractPolyList *polyList, const Box3F &box )
{
   ...
   if (start<end)
   {
      ...
      for (S32 i=start; i<end; i++)
      {
         ...
         // collide...
         emitted |= mesh->buildPolyListOpcode(od,polyList,localBox);
      }
...
bool TSShapeInstance::MeshObjectInstance::buildPolyListOpcode(S32 objectDetail, AbstractPolyList * polyList, const Box3F &box)
{
   TSMesh * mesh = getMesh(objectDetail);
   if (mesh && visible>0.01f)
      return mesh->buildPolyListOpcode(frame,polyList,box);
   return false;
}
...
bool TSShapeInstance::buildConvexOpcode(MatrixF objMat, Point3F objScale, S32 dl, const Box3F &bounds, Convex *c, Convex *list )
{
   // if dl==-1, nothing to do
   if (dl==-1)
      return false;
