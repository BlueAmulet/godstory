...
public:
   ...
   virtual void unpack(NetConnection* /*ps*/, BitStream *bstream)
      { char buf[256]; bstream->readString(buf,256); msg = dStrdup(buf); }
...
public:
   ...
   SimpleNetObject()
   {
      ...
      dStrcpy(message, sizeof(message), "Hello World!");
   }
   ...
   {
      stream->readString(message,256);
      Con::printf("Got message: %s", message);
   }
   void setMessage(const char *msg)
   {
      setMaskBits(1);
      dStrcpy(message, sizeof(message), msg);
   }
