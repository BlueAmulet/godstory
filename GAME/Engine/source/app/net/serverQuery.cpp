...
static U8 sendPacketData[Net::MaxPacketBuffSize];
      ...
      U8 filterFlags)
{
   ...
   pushServerFavorites();
      ...
      if ( !sActiveFilter.gameType || dStricmp( sActiveFilter.gameType, "Any" ) != 0 )
      {
         ...
         dStrcpy( sActiveFilter.gameType, 4, "Any" );
      }
      if ( !sActiveFilter.missionType || dStricmp( sActiveFilter.missionType, "Any" ) != 0 )
      {
         ...
         dStrcpy( sActiveFilter.missionType, 4, "Any" );
      }
      ...
      U16 minCPU, U8 filterFlags, U8 buddyCount, U32* buddyList )
{
   ...
   if ( buddyCount == 0 )
   {
      ...
      {
		 length = dStrlen( gameType ) + 1;
         sActiveFilter.gameType = (char*) dRealloc( sActiveFilter.gameType, length);
         dStrcpy( sActiveFilter.gameType, length, gameType );
      }

      if ( !sActiveFilter.missionType || dStrcmp( sActiveFilter.missionType, missionType ) != 0 )
      {
		length = dStrlen( missionType ) + 1;
         sActiveFilter.missionType = (char*) dRealloc( sActiveFilter.missionType, length );
         dStrcpy( sActiveFilter.missionType, length, missionType );
      }
...
static void pushServerFavorites()
{
   ...
   for ( S32 i = 0; i < count; i++ )
   {
      ...
      if ( server )
      {
         ...
         {
            len = sz > 24 ? 24 : sz;
            dStrncpy( serverName, sizeof(serverName), server, len );
            serverName[len] = 0;
            dStrncpy( addrString, sizeof(addrString), server + ( sz + 1 ), 255 );
			...
			strLength = dStrlen( serverName ) + 1;
            si->name = (char*) dRealloc( (void*) si->name, strLength );
            dStrcpy( si->name, strLength, serverName );
            si->isFavorite = true;
            pushPingRequest( &addr );
         }
      }
...
void addFakeServers( S32 howMany )
{
   ...
   for ( S32 i = 0; i < howMany; i++ )
   {
	  ...
	  length = dStrlen( buf ) + 1;
      newServer.name = (char*) dMalloc( length );
      dStrcpy( newServer.name, length, buf );

	  length = dStrlen("Fake") + 1;
      newServer.gameType = (char*) dMalloc( length );
      dStrcpy( newServer.gameType, length, "Fake" );

	  length = dStrlen("FakeMissionType") + 1;
      newServer.missionType = (char*) dMalloc( length );
      dStrcpy( newServer.missionType, length, "FakeMissionType" );

	  length = dStrlen("FakeMapName") + 1;
      newServer.missionName = (char*) dMalloc( length );
      dStrcpy( newServer.missionName, length, "FakeMapName" );
...
static void processPingsAndQueries( U32 session, bool schedule )
{
   ...
   else
   {
      ...
      if ( foundCount == 0 )
         dStrcpy( msg, sizeof(msg), "No servers found." );
      else if ( foundCount == 1 )
         dStrcpy( msg, sizeof(msg), "One server found." );
      else
...
static void handleGameMasterInfoRequest( const NetAddress* address, U32 key, U8 flags )
{
   ...
   if ( GNet->doesAllowConnections() )
   {
      ...
      out->write( temp32 );
      temp8 = 0;
#if defined(POWER_OS_LINUX) || defined(POWER_OS_OPENBSD)
	  ...
	  length = dStrlen( guidList ) + 1;
      char* buf = new char[length];
      dStrcpy( buf, length, guidList );
...
static void handleGamePingRequest( const NetAddress* address, U32 key, U8 flags )
{
   ...
   if ( GNet->doesAllowConnections() )
   {
      ...
      char serverName[25];
      dStrncpy( serverName, sizeof(serverName), Con::getVariable( "Pref::Server::Name" ), 24 );
      serverName[24] = 0;
      if ( flags & ServerFilter::NoStringCompress )
...
static void handleGamePingResponse( const NetAddress* address, BitStream* stream, U32 key, U8 /*flags*/ )
{
   ...
   {
	  length = dStrlen( buf ) + 1;
      si->name = (char*) dMalloc( length );
      dStrcpy( si->name, length, buf );
   }
...
static void handleGameInfoResponse( const NetAddress* address, BitStream* stream, U32 /*key*/, U8 /*flags*/ )
{
   ...
   // Get the rules set:
   char stringBuf[256];   // Who knows how big this should be?
   stream->readString( stringBuf,256 );
   if ( !si->gameType || dStricmp( si->gameType, stringBuf ) != 0 )
   {
	  length = dStrlen( stringBuf ) + 1;
      si->gameType = (char*) dRealloc( (void*) si->gameType, length );
      dStrcpy( si->gameType, length, stringBuf );
   ...
   {
      length = dStrlen( stringBuf ) + 1;
      si->missionType = (char*) dRealloc( (void*) si->missionType, length );
      dStrcpy( si->missionType, length, stringBuf );
   ...
   {
	  length = dStrlen( stringBuf ) + 1;
      si->missionName = (char*) dRealloc( (void*) si->missionName, length );
      dStrcpy( si->missionName, length, stringBuf );
   }
   ...
   {
      length = dStrlen( stringBuf ) + 1;
      si->infoString = (char*) dRealloc( (void*) si->infoString, length );
      dStrcpy( si->infoString, length, stringBuf );
   }
   ...
   {
	  length = dStrlen( stringBuf ) + 1;
      si->statusString = (char*) dRealloc( (void*) si->statusString, length );
      dStrcpy( si->statusString, length, stringBuf );
   }
