...
void TerrainBlock::buildConvex(const Box3F& box,Convex* convex)
{
   ...
   S32 xStart = (S32)mFloor( osBox.min.x / mSquareSize );
   S32 xEnd   = (S32)mCeil ( osBox.max.x / mSquareSize );
   S32 yStart = (S32)mFloor( osBox.min.y / mSquareSize );
   S32 yEnd   = (S32)mCeil ( osBox.max.y / mSquareSize );
   ...
   for (S32 y = yStart; y < yEnd; y++) {
      ...
      for (S32 x = xStart; x < xEnd; x++) {
         ...
         cp->box.min.set((F32)(x * mSquareSize), (F32)(y * mSquareSize), fixedToFloat(gs->minHeight));
         cp->box.max.x = cp->box.min.x + mSquareSize;
         cp->box.max.y = cp->box.min.y + mSquareSize;
...
// If we want terrain collision debug rendering we need to write this
// routine for gfx.
#if 0
...
#pragma message(ENGINE(去掉这个判断，aBlockPos非零时也要计算))
