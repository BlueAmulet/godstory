...
void ProcessList::advanceObjects()
{
   ...
   {
      if(pobj->mDeleteInNextTick)
      {
         GameBase* pGB = static_cast<GameBase*>(pobj);
         if(pGB && pGB->isServerObject())
         {
            pGB->deleteObject();
            continue;
         }
		 AssertFatal(false, "ProcessList::advanceObjects : error delete object!");
      }
      pobj->plUnlink();
