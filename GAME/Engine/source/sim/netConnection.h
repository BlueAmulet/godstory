...
///
/// @note PowerEngine implements two methods of network data passing; this is one of them.
/// See NetConnection for details of the other, which is referred to as ghosting.
///
/// PowerEngine's network layer lets you pass events to/from the server. There are three
/// types of events:
...
/// to register this event type with PowerEngine's networking layer, so that it can be
...
/// a client sent them.
///
/// @note PowerEngine allows you to call NetConnection::setLastError() on the NetConnection passed to
...
public:
   ...
   virtual void prepare(NetConnection *ps){};		//数据准备，游戏逻辑事件中使用
...
/// PowerEngine network connection.
///
/// @section NetConnection_intro Introduction
///
/// NetConnection is the glue that binds a networked PowerEngine game together. It combines
...
///   - <b>Demo Recording</b> is also implemented in NetConnection. A demo in PowerEngine is a log
...
///
/// Ghosting is the most complex, and most powerful, part of PowerEngine's networking capabilities. It
...
public:
   ...
   struct GhostRef
   {
      U64 mask;                  ///< States we transmitted.
...
public:
   ...
   static NetConnection *lookup(const NetAddress *remoteAddress, int socket);
...
public:
   ...
   static Signal<> smGhostAlwaysDone;
...
public:  // required for MSVC
   ...
   U64 updateMask;                        ///< Flags indicating what state data needs to be transferred.
