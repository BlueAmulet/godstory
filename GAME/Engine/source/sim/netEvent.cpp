...
void NetConnection::eventWritePacket(BitStream *bstream, PacketNotify *notify)
{
   ...
   while(mUnorderedSendEventQueueHead)
   {
	  ...
	  if(gMaxPackOnceSize<size)
	  {
		  ...
		  char tmp[1024];
		  sprintf(tmp,"gMaxPackOnceSize 在mUnorderedSendEventQueueHead时超过最大值 eventName:%s",ev->mEvent->getClassRep()->getClassName());
		  AssertFatal(size <= Net::OnePacketDataSize ,tmp);
	  }
#endif
   ...
   while(mSendEventQueueHead)
   {
	  ...
	  if(gMaxPackOnceSize<size)
	  {
		  gMaxPackOnceSize = size;
		  char tmp[1024];
		  sprintf(tmp,"gMaxPackOnceSize 在mSendEventQueueHead时超过最大值 eventName:%s",ev->mEvent->getClassRep()->getClassName());
		  AssertFatal(size <= Net::OnePacketDataSize ,tmp);
	  }
#endif
   }
