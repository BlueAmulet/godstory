...
{
   for(S32 i = mPendingConnections.size()-1; i >= 0; i--)
   {
      if(mPendingConnections[i] == connection)
	  {
         mPendingConnections.erase_fast(i);
	  }
   }
}
...
NetConnection *NetInterface::findPendingConnection(const NetAddress *address, int udpS, U32 connectSequence)
{
	for(U32 i = 0; i < mPendingConnections.size(); i++)
		if(Net::compareAddresses(address, mPendingConnections[i]->getNetAddress()) &&
			connectSequence == mPendingConnections[i]->getSequence() && mPendingConnections[i]->GetSocket() == udpS )
			return mPendingConnections[i];
	return NULL;
}

void NetInterface::processPacketReceiveEventEx( NetAddress srcAddress, int udpSocket, RawData packetData )
{
	...
	if(packetData.data[0] & 0x01) // it's a protocol packet...
	{
		...
		NetConnection *conn = NetConnection::lookup(&srcAddress, udpSocket);
		if(conn)
	...
	else
	{
		...
		else
		{
			...
			case ConnectChallengeResponse:
				handleConnectChallengeResponse(addr, udpSocket, &pStream);
				break;
			case ConnectAccept:
				handleConnectAccept(addr, udpSocket, &pStream);
				break;
			case Disconnect:
				handleDisconnect(addr, udpSocket, &pStream);
				break;
			case ConnectReject:
				handleConnectReject(addr, udpSocket, &pStream);
				break;
			}
		}
	}
...
// The PowerEngine does a two phase connect handshake to
...
void NetInterface::sendConnectChallengeRequest(NetConnection *conn)
{
   ...
   if( conn->GetSocket() == 0 )
	...
	BitStream::sendPacketStream(conn->getNetAddress(), conn->GetSocket());
}
...
void NetInterface::handleConnectChallengeResponse(const NetAddress *address, int udpS, BitStream *stream)
{
	Con::printf("Got Connect challenge Response");
	U32 connectSequence;
	stream->read(&connectSequence);

	NetConnection *conn = findPendingConnection(address, udpS, connectSequence);
	if(!conn || conn->getConnectionState() != NetConnection::AwaitingChallengeResponse)
		return;
...
void NetInterface::handleConnectRequest(const NetAddress *address, BitStream *stream)
{
   ...
   stream->readString(connectionClass,255);
...
void NetInterface::handleConnectAccept(const NetAddress *address, int udpS, BitStream *stream)
{
	U32 connectSequence;
	stream->read(&connectSequence);
	NetConnection *conn = findPendingConnection(address, udpS, connectSequence);
...
void NetInterface::handleConnectReject(const NetAddress *address, BitStream *stream)
{
   ...
   removePendingConnection(conn);
   char reason[256];
   stream->readString(reason,256);
   conn->onConnectionRejected(reason);
   conn->deleteObject();
}

void NetInterface::handleConnectReject(const NetAddress *address, int udpS, BitStream *stream)
{
	U32 connectSequence;
	stream->read(&connectSequence);
	NetConnection *conn = findPendingConnection(address, udpS, connectSequence);
...
void NetInterface::handleDisconnect(const NetAddress *address, int udpS, BitStream *stream)
{
	NetConnection *conn = NetConnection::lookup(address, udpS );
	if(!conn)
		return;
