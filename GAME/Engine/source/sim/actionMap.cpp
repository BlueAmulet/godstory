...
void ActionMap::dumpActionMap(const char* fileName, const bool append) const
{
   if (fileName != NULL) {
      ...
      else
      {
         ...
         dStrcpy( lineBuffer, sizeof(lineBuffer), "// PowerEngine Input Map File\n" );
      ...
      for (S32 i = 0; i < mDeviceMaps.size(); i++) {
         ...
         for (S32 j = 0; j < pDevMap->nodeMap.size(); j++) {
            ...
            if (rNode.flags & (Node::HasScale|Node::HasDeadZone|Node::Ranged|Node::Inverted)) {
               ...
               dStrcat(lineBuffer, sizeof(lineBuffer), buff);
            }
            ...
            if (rNode.flags & Node::BindCmd) {
               if (rNode.makeConsoleCommand) {
                  dStrcat(lineBuffer, sizeof(lineBuffer), ", \"");
               ...
               dStrcat(lineBuffer, sizeof(lineBuffer), ", ");
               dStrcat(lineBuffer, sizeof(lineBuffer), rNode.consoleFunction);
            }
      ...
      for (S32 i = 0; i < mDeviceMaps.size(); i++) {
         ...
         for (S32 j = 0; j < pDevMap->nodeMap.size(); j++) {
            ...
            if (rNode.flags & (Node::HasScale|Node::HasDeadZone|Node::Ranged|Node::Inverted)) {
               ...
               dStrcat(finalBuffer, sizeof(finalBuffer), buff);
            }
            ...
            if (rNode.flags & Node::BindCmd) {
               if (rNode.makeConsoleCommand) {
                  dStrcat(finalBuffer, sizeof(finalBuffer), ", \"");
                  dStrcat(finalBuffer, sizeof(finalBuffer), rNode.makeConsoleCommand);
                  dStrcat(finalBuffer, sizeof(finalBuffer), "\"");
               } else {
                  dStrcat(finalBuffer, sizeof(finalBuffer), ", \"\"");
               }
               if (rNode.breakConsoleCommand) {
                  dStrcat(finalBuffer, sizeof(finalBuffer), ", \"");
                  dStrcat(finalBuffer, sizeof(finalBuffer), rNode.breakConsoleCommand);
                  dStrcat(finalBuffer, sizeof(finalBuffer), "\"");
               }
               else
                  dStrcat(finalBuffer, sizeof(finalBuffer), ", \"\"");
               ...
               dStrcat(finalBuffer, sizeof(finalBuffer), ", ");
               dStrcat(finalBuffer, sizeof(finalBuffer), rNode.consoleFunction);
            }
...
{
   char copyBuffer[256];
   dStrcpy(copyBuffer, sizeof(copyBuffer), pEventString);
   ...
   if (pSpace != NULL) {
	  ...
	  char * context = NULL;
      char* pModifier = dStrtok(copyBuffer, "-", &context);
      while (pModifier != NULL) {
         ...
         pModifier = dStrtok(NULL, "-", &context);
      }
   } else {
      // No.
      pDescriptor->flags = 0;
   ...
   while ( nextBoundNode( command, devMapIndex, nodeIndex ) )
   {
      ...
      if ( getDeviceName( deviceMap->deviceType, deviceMap->deviceInst, deviceBuffer ) )
      {
         ...
         if ( getKeyString( node->action, keyBuffer ) )
         {
            ...
            dStrcat( returnString, 1024, buffer );
         }
      }
	...
	if ( getDeviceTypeAndInstance( device, deviceType, deviceInst ) )
	{
		...
		if ( createEventDescriptor( action, &eventDescriptor ) )
		{
			   ...
			   if ( mapNode->flags & Node::HasDeadZone )
            {
				   ...
				   char* returnString = Con::getReturnBuffer( length );
				   dStrcpy( returnString, length, buf );
				   return( returnString );
				}
				else
				   return( "0 0" );				   		
   ...
   if ( (asciiCode != 0) && dIsDecentChar((char)asciiCode))
   {
      for (U32 i = 0; gAsciiMap[i].asciiCode != 0xFFFF; i++) {
         if (gAsciiMap[i].asciiCode == asciiCode)
         {
		    ...
		    dMemcpy(buffer, gAsciiMap[i].pDescription, dStrlen(gAsciiMap[i].pDescription) + 1);
            return true;
         }
      }
   ...
   else
   {
      ...
      for (U32 i = 0; gVirtualMap[i].code != 0xFFFFFFFF; i++) {
         if (gVirtualMap[i].code == action) {
			...
			dMemcpy(buffer, gVirtualMap[i].pDescription, dStrlen(gVirtualMap[i].pDescription) + 1);
            return true;
         }
      }
   }
...
bool ActionMap::processAction(const InputEventInfo* pEvent)
{
   ...
   if (pEvent->action == SI_MAKE || pEvent->objInst == KEY_SNAPSHOT) {//[2009/7/22][thinking]: 增加屏幕打印键处理
...
ConsoleMethod( ActionMap, bindObj, void, 6, 11, "(device, action, [modifier spec, mod...], command, object)")
{
...
ConsoleMethod( ActionMap, unbindObj, void, 5, 5, "(device, action, object)")
{
...
CodeMapping gVirtualMap[] =
{
   ...
   { "PrintScreen",   SI_KEY,    KEY_SNAPSHOT    },
