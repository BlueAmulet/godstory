...
U32 NetStringTable::addString(const char *string)
{
   ...
   table[e].refCount++;
   length = dStrlen(string) + 1;
   table[e].string = (char *) allocator->alloc(length);
   dStrcpy(table[e].string, length, string);
...
void NetStringTable::repack()
{
   ...
   for(U32 walk = firstValid; walk; walk = table[walk].link)
   {
	  ...
	  length = dStrlen(prevStr) + 1;
      table[walk].string = (char *) newAllocator->alloc(length);
      dStrcpy(table[walk].string, length, prevStr);
   }
   delete allocator;
