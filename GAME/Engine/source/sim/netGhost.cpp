...
Signal<>    NetConnection::smGhostAlwaysDone;
...
void NetConnection::ghostWritePacket(BitStream *bstream, PacketNotify *notify)
{
   ...
   {
#ifdef POWER_DEBUG	  
	   U32 buffPos = bstream->getPosition();
#endif
      ...
      U64 updateMask = walk->updateMask;
      ...
      else
      {
         ...
         {
            classId = walk->obj->getClassId(getNetClassGroup());
...
#ifdef NTJ_SERVER
			bstream->writeInt(walk->obj->getId(), 32);
#endif
         }
#ifdef POWER_DEBUG_NET
         else {
         ...
         U32 beginSize = bstream->getCurPos();
#endif
         U64 retMask = walk->obj->packUpdate(this, updateMask, bstream);
#ifdef POWER_NET_STATS
...
#ifdef POWER_DEBUG	  
	  int size = bstream->getPosition()-buffPos;
	  if(gMaxPackOnceSize<size)
	  {
		  gMaxPackOnceSize = size;
		  AssertFatal(size <= Net::OnePacketDataSize ,"gMaxPackOnceSize 在ghost时超过最大值");
	  }
#endif
   }
...
void NetConnection::ghostReadPacket(BitStream *bstream)
{
   ...
   while(bstream->readFlag())
   {
...
#ifdef NTJ_CLIENT
		 ShapeBase *pShapeObj = dynamic_cast<ShapeBase *>(mLocalGhosts[index]);
		 if(pShapeObj)
			 pShapeObj->onRemoveFromClient();

		 GameObject* pGameObj = dynamic_cast<GameObject*>(mLocalGhosts[index]);
		 if(pGameObj)
		 {
			 g_ClientGameplayState->removeObjectList(pGameObj->getId());
      ...
      else
      {
...
#endif
#ifdef NTJ_CLIENT
			obj->mServerId = (U32) bstream->readInt(32);
#endif
		 ...
		 ShapeBase *pShapeObj = dynamic_cast<ShapeBase *>(mLocalGhosts[index]);
		 if(pShapeObj)
			 pShapeObj->onAddToClient();

		 GameObject* pGameObj = dynamic_cast<GameObject*>(mLocalGhosts[index]);
		 Player* pSelf = g_ClientGameplayState->GetControlPlayer();
		 if(pGameObj)
		 {
			 g_ClientGameplayState->setObjectList(pGameObj->getId(),pGameObj);
			 if((pGameObj->getGameObjectMask() & NpcObjectType) && !pGameObj->isCombative())	// 是否交互型NPC
			 {
				 if (pSelf)
					 pSelf->inNpcScope(pGameObj);
			 }
			 else
			 if(pGameObj->getGameObjectMask() & PlayerObjectType)	// 对象是玩家
			 {
				 Player* player = static_cast<Player*>(obj);
				 char szName[COMMON_STRING_LENGTH + 2];
				 dSprintf(szName, COMMON_STRING_LENGTH + 2, "P_%s", player->getObjectName());
				 player->assignName(szName);				
			 }
		 }
...
void NetConnection::detachObject(GhostInfo *info)
{
   ...
   if(!info->updateMask)
   {
      info->updateMask = 0xFFFFFFFFFFFFFFFF;
...
void NetConnection::objectInScope(NetObject *obj)
{
   ...
   giptr->updateMask = 0xFFFFFFFFFFFFFFFF;
...
#ifdef NTJ_EDITOR
   //Ray: 静态物件不再发送
   for(SimSet::iterator i = ghostAlwaysSet->begin(); i != ghostAlwaysSet->end(); i++)
   {
    ...
    if (getLocalClientConnection() == this)
    {
        ...
        for (j = mGhostZeroUpdateIndex - 1; j >= 0; j--)
        {
            ...
            if (mGhostArray[j]->obj)
            {
                ...
                mGhostArray[j]->obj->packUpdate(this, 0xFFFFFFFFFFFFFFFF, &mStream);
...
void NetConnection::ghostWriteStartBlock(ResizeBitStream *stream)
{
   ...
   for(U32 i = 0; i < MaxGhostCount; i++)
   {
      if(mLocalGhosts[i])
      {
         mLocalGhosts[i]->packUpdate(this, 0xFFFFFFFFFFFFFFFF, stream);
         stream->validate();
      }
   }
}
