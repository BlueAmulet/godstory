...
///
/// One of the most powerful aspects of PowerEngine's networking code is its support
...
///
/// The basis of the ghost implementation in PowerEngine is NetObject. It tracks the dirty flags for the
...
/// @endcode
///
/// Above is the standard boilerplate code for a PowerEngine class. You can find out more about this in SimObject.
///
/// @code
///    char message1[256];
///    char message2[256];
...
/// NetObject. It is important that we use this, as it makes PowerEngine perform certain initialization tasks
...
#ifdef NTJ_CLIENT
   U32 mServerId;					// <Edit> [3/27/2009 joy] 在客户端保存zone服务器的SimObject::mId，作为客户端寻找服务器对象的唯一标识
#endif
...
public:
   ...
   void setMaskBits(U64 orMask);
   inline U64 getMaskBits() { return mDirtyMaskBits;}

   /// Clear the specified bits from the dirty mask.
   ///
   /// @param   orMask   Bits to clear
   void clearMaskBits(U64 orMask);
   virtual U64 filterMaskBits(U64 mask, NetConnection * connection) { return mask; }

   void setClientObject() {mNetFlags |= IsGhost;}
   void setGhostable(bool val) { if(val) mNetFlags |= Ghostable; else mNetFlags &= ~Ghostable;}
   ...
   U32 getNetIndex()								//Ray: 废弃这个方法，防止客户端获取mNetIndex后回传给服务器操作对象。
   {
#ifdef NTJ_SERVER
      return mNetIndex;
#else
...
#ifdef NTJ_CLIENT
   U32 getServerId() { return mServerId;}			// <Edit> [3/27/2009 joy] 客户端获取mServerId后回传给服务器操作对象
#endif
   ...
   NetConnection* getNetConnection()	{ return mConn; }
   void setNetConnection( NetConnection* pConn ) { mConn = pConn; }
};
