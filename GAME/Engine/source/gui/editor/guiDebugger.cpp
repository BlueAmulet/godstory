...
void DbgFileView::AdjustCellSize()
{
   ...
   {
      S32 cellWidth = gFileXOffset + mFont->getStrWidth(( UTF8 *)mFileView[i].text);
...
bool DbgFileView::findString(const char *text)
{
	...
	dStrncpy(mFindString, sizeof(mFindString), text, 255);
...
S32 DbgFileView::findMouseOverChar(const char *text, S32 stringPosition)
{
   ...
   dStrncpy(tempBuf, sizeof(tempBuf), text, 512);
   ...
   do {
      ...
      *bufPtr = '\0';
      ...
      // mouse position?
      if ((S32)mFont->getStrWidth(( UTF8 *)tempBuf) > stringPosition) {
         // Yep.
...
bool DbgFileView::findMouseOverVariable()
{
   ...
   if(cell.x >= 0 && cell.x < mSize.x && cell.y >= 0 && cell.y < mSize.y)
   {
      ...
      dStrcpy(tempBuf, sizeof(tempBuf), mFileView[cell.y].text);
...
void DbgFileView::onPreRender()
{
	setUpdate();
   char oldVar[256];
   dStrcpy(oldVar, sizeof(oldVar), mMouseOverVariable);
...
void DbgFileView::onRenderCell(Point2I offset, Point2I cell, bool selected, bool)
{
   ...
   if (selected)
   {
      ...
      else if (mBlockStart >= 0 && mBlockEnd > mBlockStart && mBlockEnd <= S32(dStrlen(mFileView[cell.y].text) + 1))
      {
         ...
         //get the end coord
         tempBuf[mBlockEnd] = '\0';
         endPos = mFont->getStrWidth(( UTF8 *)tempBuf);

         //get the start coord
         tempBuf[mBlockStart] = '\0';
         startPos = mFont->getStrWidth(( UTF8 *)tempBuf);
