...
ConsoleMethod( GuiFilterCtrl, getValue, const char*, 2, 2, "Return a tuple containing all the values in the filter.")
{
	...
	*buffer = 0;
	...
	{
		char value[32];
		dSprintf(value, sizeof(value), "%1.5f ", *(filter->begin()+i) );
		dStrcat(buffer, sizeof(buffer), value);
	}
...
void GuiFilterCtrl::onRender(Point2I offset, const RectI &updateRect)
{
	...
	GFX->getDrawUtil()->drawLine(pos.x, pos.y + ext.y, pos.x + ext.x, pos.y, ColorF( 0.9f, 0.9f, 0.9f ) );
...
void Filter::set(S32 argc, const char *argv[])
{
	...
	{  // in the form of one string "1.0 1.0 1.0"
		char list[1024];
		dStrcpy(list, sizeof(list), *argv);    // strtok modifies the string so we need to copy it
		char * context = NULL;
