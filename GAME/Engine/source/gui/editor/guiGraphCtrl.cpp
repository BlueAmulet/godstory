...
//---------------------------------------------------------------------------
////#define STATEBLOCK
GFXStateBlock* GuiGraphCtrl::mAlphaBlendTrueSB = NULL;
GFXStateBlock* GuiGraphCtrl::mAlphaBlendFalseSB = NULL;
...
static const EnumTable::Enums enumGraphTypes[] = {
...
static const EnumTable gGraphTypeTable( 4, &enumGraphTypes[0] );
...
void GuiGraphCtrl::onRender(Point2I offset, const RectI &updateRect)
{
...
#ifdef STATEBLOCK
	AssertFatal(mAlphaBlendTrueSB, "GuiGraphCtrl::onRender -- mAlphaBlendTrueSB cannot be NULL.");
	mAlphaBlendTrueSB->apply();
#else
...
#ifdef STATEBLOCK
	AssertFatal(mAlphaBlendFalseSB, "GuiGraphCtrl::onRender -- mAlphaBlendFalseSB cannot be NULL.");
	...
	for (int k = 0; k < MaxPlots; k++)
	{
		...
		if(mPlots[k].mGraphType == Bar)
		{
         ...
         PrimBuild::color( mPlots[k].mGraphColor );
            ...
            PrimBuild::vertex2i(getPosition().x + temp1, 
					(getPosition().y + getExtent().y) - (S32)(getDatum(k, sample) * Scale));

				PrimBuild::vertex2i(getPosition().x + temp2, 
					(getPosition().y + getExtent().y) - (S32)(getDatum(k, sample) * Scale));
		...
		else if((mPlots[k].mGraphType == Point) || (mPlots[k].mGraphType == Polyline))
		{
			...
			for (S32 sample = 0; sample < getExtent().x; sample++)
			{
				...
				PrimBuild::vertex2i(getPosition().x + temp,
					(getPosition().y + getExtent().y) - (S32)(getDatum(k, sample) * Scale));
			}
...
void GuiGraphCtrl::resetStateBlock()
{
	//mAlphaBlendTrueSB
	GFX->beginStateBlock();
...
void GuiGraphCtrl::releaseStateBlock()
{
...
void GuiGraphCtrl::init()
{
	if (mAlphaBlendTrueSB == NULL)
	{
		mAlphaBlendTrueSB = new GFXD3D9StateBlock;
		mAlphaBlendTrueSB->registerResourceWithDevice(GFX);
		mAlphaBlendTrueSB->mZombify = &releaseStateBlock;
		mAlphaBlendTrueSB->mResurrect = &resetStateBlock;
