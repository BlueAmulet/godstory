...
void GuiMessageVectorCtrl::createSpecialMarkers(SpecialMarkers& rSpecial, const char* string)
{
   ...
   while (pCurr[0] != '\0') {
      ...
      for (U32 i = 0; i < 16; i++) {
         ...
         dStrncpy(matchBuffer, sizeof(matchBuffer), mAllowedMatches[i], 500);
         matchBuffer[499] = '\0';
         dStrcat(matchBuffer, sizeof(matchBuffer), "://");
...
void GuiMessageVectorCtrl::createLineWrapping(LineWrapping& rWrapping, const char* string)
{
   ...
   while (currLine < tempBreaks.size()) {
      ...
      U32 baseLength = mProfile->mFont->getStrNWidth(( UTF8 *)&string[rLine.start], rLine.end-rLine.start+1);
      if (baseLength > splitWidth) {
         // DMMNOTE: Replace with bin search eventually
         U32 currPos = 0;
         U32 breakPos = 0;
         for (currPos = 0; currPos < rLine.end-rLine.start+1; currPos++) {
            U32 currLength = mProfile->mFont->getStrNWidth(( UTF8 *)&string[rLine.start], currPos+1);
...
bool GuiMessageVectorCtrl::onWake()
{
   ...
   for (U32 i = 0; i < 256; i++) {
       {
         if (mProfile->mFont->getStrWidth((char*)(i)) > mMinSensibleWidth)
            mMinSensibleWidth = mProfile->mFont->getStrWidth((char*)(i));
      }
   }
...
void GuiMessageVectorCtrl::findSpecialFromCoord(const Point2I& point, S32* specialLine, S32* specialRef)
{
   ...
   while (line) {
      U32 newX = currX + mProfile->mFont->getStrNWidth(( UTF8 *)mMessageVector->getLine(elemIndex).message,
...
void GuiMessageVectorCtrl::onMouseUp(const GuiEvent& event)
{
   ...
   if (currSpecialRef != -1 &&
      ...
      dMemcpy(copyURL, &mMessageVector->getLine(currSpecialLine).message[specialStart], specialEnd - specialStart + 1);
