...
S32 GuiAviBitmapCtrl::vcmDraw(U64 dwICflags)
{
	...
	for (U32 j = 0; j < mHeightCount; j++)
	{
		...
		for (U32 i = 0; i < mWidthCount; i++)
		{
			...
			{
				const U8 *src = &mPBuf[bmp->bytesPerPixel*((mBitmapHeight-(y+lp+1))*mBitmapAlignedWidth + x)];
				U8 *dest = bmp->getAddress(0, lp);

				// counting on the artist to switch the R & B channels so we don't have to in runtime
				if (!mSwapRB)
					dMemcpy(dest, src, width * bmp->bytesPerPixel);
				else
					Swizzles::bgr.ToBuffer( dest, src, width * bmp->bytesPerPixel );
			}
...
bool GuiAviBitmapCtrl::onWake()
{
	...
	mTextureHandles = new GFXTexHandle[mNumTextures];
	for (U32 j = 0; j < mHeightCount; j++)
	{
		U32 y = j * 256;
		...
		for (U32 i = 0; i < mWidthCount; i++)
		{
			...
			mTextureHandles[index] = GFXTexHandle(nameBuffer, RegisteredTexture, true);
			if (!bool(mTextureHandles[index]))
			{
				...
				mTextureHandles[index] = GFXTexHandle(nameBuffer, bmp, true);
			}
		}
	}
...
void GuiAviBitmapCtrl::onRender(Point2I offset, const RectI &updateRect)
{
	...
	if (mTextureHandles)
	{
		...
		for (U32 i = 0; i < mWidthCount; i++)
		{
			for (U32 j = 0; j < mHeightCount; j++)
			{
				GFXTexHandle t = mTextureHandles[j * mWidthCount + i];
				RectI stretchRegion;
		...
		if (mLetterBox)
		{
			...
			RectI lowerRect(offset.x, getHeight() - verticalDisplace - 1,
				getWidth() + 2, verticalDisplace + 2);
	...
	if (mTextureHandles)
	{
		...
		for (U32 j = 0; j < mHeightCount; j++)
		{
			...
			for (U32 i = 0; i < mWidthCount; i++)
			{
				...
				for (U32 lp = 0; lp < height; lp++)
				{
					const U8 *src = &mPBuf[bmp->bytesPerPixel*((y+lp)*mBitmapAlignedWidth + x)];
					U8 *dest = bmp->getAddress(0, lp);
