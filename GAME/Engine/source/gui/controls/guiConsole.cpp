...
S32 GuiConsole::getMaxWidth(S32 startIndex, S32 endIndex)
{
   ...
   for(S32 i = startIndex; i <= endIndex; i++)
      result = getMax(result, (S32)(mFont->getStrWidth(( UTF8 *)log[i].mString)));
...
void GuiConsole::onRenderCell(Point2I offset, Point2I cell, bool /*selected*/, bool /*mouseOver*/)
{
   ...
   ColorI mFontColor(255,255,255);
   switch (entry.mLevel)
   {
      case ConsoleLogEntry::Normal:  mFontColor = mProfile->mFontColor;break; 
      case ConsoleLogEntry::Warning: mFontColor = mProfile->mFontColorHL;break;
      case ConsoleLogEntry::Error:   mFontColor = mProfile->mFontColorNA;break;
   }
   GFX->getDrawUtil()->drawText(mFont, Point2I(offset.x + 3, offset.y), entry.mString, &mFontColor);
