...
GuiTextListCtrl::GuiTextListCtrl()
{
   ...
   m_TextListBackgroundImageFile = StringTable->insert("");
   m_TextListBackgroundTexture = NULL;
...
void GuiTextListCtrl::initPersistFields()
{
   ...
   addField("TextListSelectImageFile",  TypeString, Offset(m_TextListSelectImageFile,GuiTextListCtrl));
   addField("TextListMouseOnImageFile", TypeString, Offset(m_TextListMouseOnImageFile,GuiTextListCtrl));
   addField("TextListBackgroundImageFile", TypeString, Offset(m_TextListBackgroundImageFile,GuiTextListCtrl));
}
...
bool GuiTextListCtrl::onWake()
{
   ...
   if(m_TextListBackgroundImageFile && m_TextListBackgroundImageFile[0])
	   m_TextListBackgroundTexture =  GFXTexHandle(m_TextListBackgroundImageFile, &GFXDefaultStaticDiffuseProfile, true);
...
void GuiTextListCtrl::onRenderCell(Point2I offset, Point2I cell, bool selected, bool mouseOver)
{
   ...
   {
	   if (m_TextListBackgroundTexture)
	   {
		   GFX->getDrawUtil()->clearBitmapModulation();
		   GFX->getDrawUtil()->drawBitmapStretch(m_TextListBackgroundTexture,highlightRect);
	   }
      ...
      if(mColumnOffsets[index] >= 0)
      {
		 ...
		 if ( mList[cell.y].active )
		 {
			 if (selected || (mProfile->mMouseOverSelected && mouseOver))
			 {
				 ...
				 //GFX->getDrawUtil()->setBitmapModulation(mProfile->mFontColorHL);
				 GFX->getDrawUtil()->setBitmapModulation(mUseSetColor ? mList[cell.y].color : mProfile->mFillColorHL);
			 }
			 else
			 {
				 if (!mUseSetColor)
                     ...
                     GFX->getDrawUtil()->setBitmapModulation(mouseOver ? mProfile->mFontColorHL : mList[cell.y].color);
			 }
		 }
		 else
			 GFX->getDrawUtil()->setBitmapModulation( mProfile->mFontColorNA );
		 ...
		 if(index < mColumnOffsets.size()-1)
			 clipExtent = Point2I(mColumnOffsets[index+1] - mColumnOffsets[index] - 2,mCellSize.y);
		 else
			 clipExtent = Point2I(getBounds().extent.x - mColumnOffsets[index] - 2,mCellSize.y);
...
void GuiTextListCtrl::addEntryTakeColor(U32 id, const char *text,ColorI& color)
{
   Entry e;
   e.text = dStrdup(text);
   e.id = id;
...
void GuiTextListCtrl::setSize(Point2I newSize)
{
   ...
   if ( mFont )
   {
	  ...
	  if ( mCellSize.y <= mFont->getHeight())
	  {
...
void GuiTextListCtrl::sortID(bool increasing /* = true */)
{
	if ( getNumEntries() < 2 )
		return;
...
ConsoleMethod(GuiTextListCtrl,addRowTakeColor,S32, 5, 5,"(int id, string text,ColorI color)")
{
	S32 ret = object->mList.size();

	ColorI textColor(255,0,0, 255);
	U32 RGBA = dAtol(argv[4]);
	U8 r = (RGBA >> 16) & 0xFF;
	U8 g = (RGBA >> 8) & 0xFF;
	U8 b = RGBA & 0xFF;
	textColor.set(r,g,b);

	object->addEntryTakeColor(dAtoi(argv[2]), argv[3], textColor);
...
ConsoleMethod(GuiTextListCtrl,sortID,void,3,3,"(bool increasing )")
{
