...
void GuiGameListOptionsCtrl::onRender(Point2I offset, const RectI &updateRect)
{
   ...
   for (Vector<Parent::Row *>::iterator row = mRows.begin(); row < mRows.end(); ++row)
   {
      ...
      bool hasOptions = (myRow->mOptions.size() > 0);
      if (hasOptions)
      {
         ...
         CommonFontEX * font = profile->mFont;
         StringTableEntry text = myRow->mOptions[myRow->mSelectedOption];
         S32 textWidth = font->getStrWidth(const_cast<char*>(text));
...
void GuiGameListOptionsCtrl::addRow(const char* label, const char* optionsList, bool wrapOptions, const char* callback, S32 icon, S32 yPad, bool enabled)
{
   ...
   Row * row = new Row();
   Vector<StringTableEntry> options;
...
bool GuiGameListOptionsCtrl::hasValidProfile() const
{
   ...
   AssertFatal(profile, avar("GuiGameListOptionsCtrl: %s can't be created with a profile of type %s. Please create it with a profile of type GuiGameListOptionsProfile.", getName(), mProfile->getClassName()));
   return profile;
}
...
ConsoleMethod(GuiGameListOptionsCtrl, addRow, void, 6, 9,
...
{
   object->addRow(argv[2], argv[3], dAtob(argv[4]), argv[5], argc > 6 ? dAtoi(argv[6]) : -1, argc > 7 ? dAtoi(argv[7]) : 0, argc > 8 ? dAtob(argv[8]) : true);
}

ConsoleMethod(GuiGameListOptionsCtrl, getCurrentOption, const char *, 3, 3,
              "(int rowIndex)\n"
...
{
   return object->getCurrentOption(dAtoi(argv[2]));
}

ConsoleMethod(GuiGameListOptionsCtrl, selectOption, bool, 4, 4,
              "(int rowIndex, string option)\n"
...
{
   return object->selectOption(dAtoi(argv[2]), argv[3]);
}

ConsoleMethod(GuiGameListOptionsCtrl, setOptions, void, 4, 4,
              "(int rowIndex, string options)\n"
...
{
   object->setOptions(dAtoi(argv[2]), argv[3]);
}
...
{
   Parent::enforceConstraints();
   AssertFatal((mHitAreaUpperLeft.x <= mColumnSplit) && (mColumnSplit <= mHitAreaLowerRight.x), avar("GuiGameListOptionsProfile: You can't create %s with a ColumnSplit outside the hit area. You set the split to %d. Please change the ColumnSplit to be in the range [%d, %d].", getName(), mColumnSplit, mHitAreaUpperLeft.x, mHitAreaLowerRight.x));
