...
void GuiTreeViewCtrl::Item::getDisplayText(U32 bufLen, char *buf)
{
	...
	else
	{
		...
		if(getText())
			dStrncpy(buf, bufLen, getText(), bufLen);
	}
}

const S32 GuiTreeViewCtrl::Item::getDisplayTextWidth(CommonFontEX *font)
{
	if( !font )
		return 0;

	FrameAllocatorMarker txtAlloc;
	U32 bufLen = getDisplayTextLength() + 2;// <Edit>:[soar]:修改了内存空间分配不够导致内存溢出的问题
	if( bufLen == 0 )
		return 0;
...
GuiTreeViewCtrl::GuiTreeViewCtrl()
{
	...
	mLastMoveHitItem	 = NULL;
	mMouseDownTempItem   = NULL;
	bIsHitSel = false;
	...
	mTexRolloverName  = StringTable->insert("");
	mTexRollover      = NULL;
	mTexSelectedName  = StringTable->insert("");
	mTexSelected      = NULL;
...
void GuiTreeViewCtrl::initPersistFields()
{
	...
	addField("TexRolloverName",      TypeString, Offset(mTexRolloverName,      GuiTreeViewCtrl));
	addField("TexSelectedName",      TypeString, Offset(mTexSelectedName,      GuiTreeViewCtrl));
	addField("Sorted",               TypeBool,   Offset(mSorted,               GuiTreeViewCtrl));
	addField("SortedMid",            TypeBool,   Offset(mSortedMid,            GuiTreeViewCtrl));
	addField("SortedNumMid",         TypeBool,   Offset(mSortedNumMid,         GuiTreeViewCtrl));
	endGroup("TreeView");
}
...
#ifdef NTJ_CLIENT
	S32 rWidth = ( tabLevel + 1 ) * mTabSize + item->mDrawer.getRealLineWidth();
	if ( mProfile->mBitmapArrayRects.size() > 0 )
		rWidth += mProfile->mBitmapArrayRects[0].extent.x;
	if ( rWidth > mMaxWidth )
		mMaxWidth = rWidth;

	//item->mDrawer.setControlOffset(Point2I(mProfile->mBitmapArrayRects[0].extent.x * (tabLevel + 1),mItemHeight * tabLevel));
#endif
	mVisibleItems.push_back( item );

#ifndef NTJ_CLIENT
	if ( mProfile != NULL && mProfile->mFont )
	{
		S32 width = ( tabLevel + 1 ) * mTabSize + item->getDisplayTextWidth(mProfile->mFont);
		if ( mProfile->mBitmapArrayRects.size() > 0 )
			width += mProfile->mBitmapArrayRects[0].extent.x;

		// width += (item->mTabLevel + 1) * mItemHeight; // using mItemHeight for icon width, close enough     // <Edit>:[soar]:修改了树控件宽度不断增加的问题
...
S32 GuiTreeViewCtrl::insertItem(S32 parentId, const char * text, const char * value, const char * iconString, S16 normalImage, S16 expandedImage, S16 tag, const char * missionId)
{
	...
	if(text)
	{
		S32 length = dStrlen( text ) + 1;
		tmp = new char[length];
		dStrcpy( tmp, length, text );
	}
	else
	{
		tmp = new char[1];
	...
	pNewItem->mDrawer.setDefaultStyle( style );
	pNewItem->mDrawer.setDefaultLineHeight(mItemHeight);
	pNewItem->mDrawer.setWidth( 1024 );
	pNewItem->mDrawer.setParentControl( this );
	pNewItem->mDrawer.setContent( tmp );

	pNewItem->mTag = (void*)tag;
	dStrcpy( pNewItem->mMissionId, 32, missionId );

	tmp = NULL;
	if(value)
	{
		S32 length = dStrlen( value ) + 1;
		tmp = new char[length];
		dStrcpy( tmp, length, value );
	}
	else
	{
		tmp = new char[1];
	...
	if(parentId == 0)
	{
		...
		if( mRoot != NULL )
		{
			...
			int n = 0;
			while( pTreeTraverse != NULL )
		 {
			 if( strcmp( pNewItem->getText(), pTreeTraverse->getText() ) <= 0 && mSorted )
			 {
				 n = 1;
				 break;
			 }
			 if( strcmp( pNewItem->mMissionId, pTreeTraverse->mMissionId ) <= 0 && mSortedMid )
			 {
				 n = 1;
				 break;
			 }
			 if((atoi(pNewItem->mMissionId) <= atoi(pTreeTraverse->mMissionId)) && mSortedNumMid)
			 {
				 n = 1;
				 break;
			 }

			 if( pTreeTraverse->mNext  == NULL )
				 break;
			...
			if( n != 0 )
		 {
			 pNewItem->mPrevious = pTreeTraverse->mPrevious;

			 if( pTreeTraverse->mPrevious )
				 pTreeTraverse->mPrevious->mNext = pNewItem;
			 else
				 mRoot = pNewItem;

			 pNewItem->mNext = pTreeTraverse;
			 pTreeTraverse->mPrevious = pNewItem;
		 }
			else
		 {
...
bool GuiTreeViewCtrl::onWake()
{
	...
	if (!mTexRollover)
	{
		if (mTexRolloverName[0] != '\0')
		{
			mTexRollover = GFXTexHandle(mTexRolloverName,&GFXDefaultStaticDiffuseProfile,true);
		}
	}

	if (!mTexSelected)
	{
		if (mTexSelectedName[0] != '\0')
		{
			mTexSelected = GFXTexHandle(mTexSelectedName,&GFXDefaultStaticDiffuseProfile,true);
		}
	}
...
bool GuiTreeViewCtrl::buildIconTable(const char * icons)
{
	...
	if (!icons)
	{
		icons = "common/gui/images/treeview/default:"
			"common/gui/images/treeview/simgroup:"
			"common/gui/images/treeview/simgroup_closed:"
			"common/gui/images/treeview/simgroup_selected:"
			"common/gui/images/treeview/simgroup_selected_closed:"
			"common/gui/images/treeview/audio:"
			"common/gui/images/treeview/camera:"
			"common/gui/images/treeview/fxfoliage:"
			"common/gui/images/treeview/fxlight:"
			"common/gui/images/treeview/fxshapereplicator:"
			"common/gui/images/treeview/fxsunlight:"
			"common/gui/images/treeview/hidden:"
			"common/gui/images/treeview/interior:"
			"common/gui/images/treeview/lightning:"
			"common/gui/images/treeview/shll_icon_passworded_hi:"
			"common/gui/images/treeview/shll_icon_passworded:"
			"common/gui/images/treeview/mission_area:"
			"common/gui/images/treeview/particle:"
			"common/gui/images/treeview/path:"
			"common/gui/images/treeview/pathmarker:"
			"common/gui/images/treeview/physical_area:"
			"common/gui/images/treeview/precipitation:"
			"common/gui/images/treeview/shape:"
			"common/gui/images/treeview/sky:"
			"common/gui/images/treeview/static_shape:"
			"common/gui/images/treeview/sun:"
			"common/gui/images/treeview/terrain:"
			"common/gui/images/treeview/trigger:"
			"common/gui/images/treeview/water:"
			"common/gui/images/treeview/default";
	}
	...
	dStrncpy( drawText, sizeof(char) * (textLen + 4), icons, textLen );
	drawText[textLen] = '\0';

	U32 numIcons = 0;
	char *buf = (char*)txtBuff.alloc(sizeof(char) * 256);
	char * context = NULL;
	char* token = dStrtok( drawText, ":", &context );

	// Count the number of icons and store them.
	while (token && numIcons < MaxIcons)
	{
		dSprintf( buf, sizeof( char ) * 256, "%s", token );
		mIconTable[numIcons] = GFXTexHandle( buf, &GFXDefaultStaticDiffuseProfile );
		token = dStrtok( NULL, ":", &context );
		numIcons++;
	}
#endif
...
bool GuiTreeViewCtrl::hitTest(const Point2I & pnt, Item* & item, BitSet32 & flags)
{
	...
	// if the cursor is out of the item text
	if(item->mDrawer.getRealLineWidth()>0 &&  pos.x > item->mDrawer.getRealLineWidth() + min )
		return false;
	...
	FrameAllocatorMarker txtAlloc;
	U32 bufLen = item->getDisplayTextLength() + 2;// <Edit>:[soar]:修改了申请内存空间不足导致内存溢出的问题。
	char *buf = (char*)txtAlloc.alloc(bufLen);
...
void GuiTreeViewCtrl::setInstantGroup(SimObject * obj)
{
	...
	if(grp)
	{
		...
		// Notify Script 
		Con::executef(this,"onInstantGroupSelected",Con::getIntArg(grp->getId()));
	}
}
...
// ----------------------------------------------------------------------------
// 由父结点向子结点展开
bool GuiTreeViewCtrl::setItemExpandedTo(Item* item, bool expand)
{
	if(!item)
	{
		Con::errorf(ConsoleLogEntry::General, "GuiTreeViewCtrl::setItemExpandedTo: invalid item id!");
		return false;
	}

	if(item->isParent() && item->mChild)
		setItemExpandedTo(item->mChild, expand);
	
	if(item->mNext)
		setItemExpandedTo(item->mNext, expand);
...
bool GuiTreeViewCtrl::clearChildItem(Item* item)
{
	if(!item)
	{
		Con::errorf(ConsoleLogEntry::General, "GuiTreeViewCtrl::clearChildItem: invalid item!");
		return(false);
	}

	Item* previousItem = item->mPrevious;
	Item* nextItem = item->mNext;
	Item* parentItem = item->mParent;

	deleteItem(item);

	if(previousItem == NULL && nextItem == NULL)
		clearChildItem(parentItem);
	return true;
}
...
/*
	...
	item->mDrawer.setWidth( 1024 );
	item->mDrawer.setContent( newText );

	delete [] item->getValue();
	S32 length = dStrlen( newValue ) + 1;
	item->setValue( new char[length] );
	dStrcpy( item->getValue(), length, newValue );
...
void GuiTreeViewCtrl::onMouseUp(const GuiEvent &event)
{
	...
	for( int j = 0; j < mVisibleItems.size(); j++ )
	{
		mVisibleItems[j]->mDrawer.onMouseUp( event );
	}
	setUpdate();
	...
	if (!mMouseDragged) 
	{
		...
		{
			bool selectFlag = newItem2->mState.test(Item::Selected);
			if (selectFlag == true)
			{
				// already selected, so unselect it and remove it
				removeSelection(newItem2->mId);
				if (newItem2->isInspectorData() && newItem2->getObject())
					Con::executef(this,"onRemoveSelection",Con::getIntArg(newItem2->getObject()->getId()));
			} else
			{
				...
				//if (newSelection) {
				addSelection(newItem2->mId);
				if (newItem2->isInspectorData() && newItem2->getObject())
					Con::executef(this,"onAddSelection",Con::getIntArg(newItem2->getObject()->getId()));
				//}
						...
						for (S32 j = (mCurrentDragCell); j < firstSelectedIndex; j++) {
							...
							if (newSelection)
							{
								...
								if (mVisibleItems[j]->isInspectorData())
									Con::executef(this,"onAddSelection",Con::getIntArg(mVisibleItems[j]->getObject()->getId()));
							}
						}
					}
					else
		...
		else if (event.modifier & SI_ALT)
		{
			if (newItem2->isInspectorData() && newItem2->getObject())
			{
				setInstantGroup(newItem2->getObject());
				mInstantGroup = newItem2->mId;
			}
		} else if (!hitFlags.test(OnImage))
		{
			clearSelection();
			setItemSelected(newItem2->mId,true);
		}
		return;
	}
	else
	{
		...
		{
			clearSelection();
			if(mMouseDownTempItem)
				setItemSelected(mMouseDownTempItem->mId,true);
		}
	}
...
void GuiTreeViewCtrl::onMouseDragged(const GuiEvent &event)
{
	...
	onMouseMove(event);// <Edit>:[soar]:修改了调用父类的函数会出现坐标计算错误的问题
	...
	for( int j = 0; j < mVisibleItems.size(); j++ )
	{
		mVisibleItems[j]->mDrawer.onMouseDragged( event );
	}
	...
	if( mPreviousDragCell >= mVisibleItems.size() )// <Edit>:[soar]:添加了越界限制
		mPreviousDragCell = 0;
	...
	if (mDebug) {
		...
		for(S32 i = 0; i < mSelectedItems.size(); i++) {
			if (mSelectedItems[i]->isInspectorData()) {
				...
				Con::printf("wtf %d", (int)mSelectedItems[i]);
		}
...
void GuiTreeViewCtrl::onMouseDown(const GuiEvent & event)
{
	...
	for( int j = 0; j < mVisibleItems.size(); j++ )
	{
		mVisibleItems[j]->mDrawer.onMouseDown( event );
	}
	//
	//if(event.modifier & SI_CTRL)
	//{
	...
	//		if(*(k) == item)
	//		{
	//			newSelection = true;// <Edit>:[soar]:修改了选中树节点的时候再次点击不执行的情况，false->true;
	//			//setItemSelected(item->mId,true);
	//			break;
	//		}
	//	}
	...
	//	}
	//}
	if (!(event.modifier & SI_CTRL) && !(event.modifier & SI_SHIFT) && !(event.modifier & SI_ALT) && !hitFlags.test(OnImage))
	{
		// first check to see if the item is already selected
		//bool newSelection = true;
		bIsHitSel = false;
		...
		for(k = mSelectedItems.begin(); k != mSelectedItems.end(); k++) {
			if(*(k) == item)
			{
				bIsHitSel = true;// <Edit>:[soar]:修改了选中树节点的时候再次点击不执行的情况，false->true;
				//setItemSelected(item->mId,true);
				break;
			}
		}
	...
	else if ( mFullRowSelect || hitFlags.test( OnImage ) )
	{
		item->setExpanded(!item->isExpanded());
		if( !item->isInspectorData() && item->mState.test(Item::VirtualParent) )
			onVirtualParentExpand(item);
...
void GuiTreeViewCtrl::onMouseMove( const GuiEvent &event )
{
	...
	for( int i=0; i < mVisibleItems.size(); i++ )
		mVisibleItems[i]->mState.clear( Item::MouseOverBmp | Item::MouseOverText );// <Edit>:[soar]:修改了树节点显示状态的清除操作，从单个清除修改为全部清除，避免出现节点状态可能没清除的情况。
...
#ifdef NTJ_CLIENT
		if( mLastMoveHitItem != item && mLastMoveHitItem != NULL )
			mLastMoveHitItem->mDrawer.onMouseLeave( event );

		mLastMoveHitItem = item;
		if (mLastMoveHitItem)			
			mLastMoveHitItem->mDrawer.onMouseMove( event );
...
void GuiTreeViewCtrl::onMouseLeave( const GuiEvent &event )
{
	...
	for( int i=0; i < mVisibleItems.size(); i++ )
		mVisibleItems[i]->mState.clear( Item::MouseOverBmp | Item::MouseOverText );// <Edit>:[soar]:修改了树节点显示状态的清除操作，从单个清除修改为全部清除，避免出现节点状态可能没清除的情况。
	//Parent::onMouseLeave( event );// <Edit>:[soar]:修改了调用父类函数可能会出现计算错误的问题。

	for( int j = 0; j < mVisibleItems.size(); j++ )
	{
		mVisibleItems[j]->mDrawer.onMouseLeave( event );
	}
...
void GuiTreeViewCtrl::onRenderCell(Point2I offset, Point2I cell, bool, bool )
{
	...
	if(item->isInspectorData())
	{
		...
		if (item->mIcon)
		{
			if (mIconTable[item->mIcon])
			{
				...
				drawer->drawBitmap( mIconTable[item->mIcon], drawRect.point );
	...
	RectI rect(drawRect.point.x,drawRect.point.y + 1,item->mDrawer.getRealLineWidth() + 10,mItemHeight - 2);
	if ( item->mState.test( Item::Selected ) && mTexSelected )
		drawer->drawBitmapStretch( mTexSelected, rect );
	else if ( item->mState.test( Item::MouseOverText ) && mTexRollover )
		drawer->drawBitmapStretch( mTexRollover, rect );
	...
	if( mInstantGroup == item->mId || item->mState.test(Item::MouseOverText) )
	{
...
#ifdef NTJ_CLIENT
	item->mDrawer.render(drawRect.point, drawRect);
#endif
...
S32 GuiTreeViewCtrl::getIcon(const char * iconString)
{
	...
	if (!dStrcmp(iconString, "Sky"))
		...
		icon = Shape;
	else if(!dStrcmp(iconString,"NpcObject"))
		icon = StaticShape;
...
StringTableEntry GuiTreeViewCtrl::getTextToRoot( S32 itemId, const char * delimiter )
{
	...
	dStrcpy( bufferOne, sizeof(bufferOne), item->getText() );

	Item *prevNode = item->mParent;
	while ( prevNode )
	{
		dMemset( bufferNodeText, 0, sizeof(bufferNodeText) );
		dStrcpy( bufferNodeText, sizeof(bufferNodeText), prevNode->getText() );
		dSprintf( bufferTwo, 1024, "%s%s%s",bufferNodeText, delimiter, bufferOne );
		dStrcpy( bufferOne, sizeof(bufferOne), bufferTwo );
		dMemset( bufferTwo, 0, sizeof(bufferTwo) );
...
int GuiTreeViewCtrl::getItemByValue( const char* value )
{
	int id = atoi( value );
	...
	for( int i = 0; i < mItems.size(); i++ )
	{
		if(!mItems[i])
			continue;
		if( mItems[i]->getObject() && mItems[i]->getObject()->getId() == id )
			return mItems[i]->mId;
	}
...
ConsoleMethod( GuiTreeViewCtrl, getItemByValue, S32, 3, 3, "" )
{
	return object->getItemByValue( argv[2] );
}

ConsoleMethod(GuiTreeViewCtrl, insertItem, S32, 10, 10, "(TreeItemId parent, name, value, icon, normalImage=0, expandedImage=0, tag, missionId)")
{
	S32 norm=0, expand=0;
	...
	return(object->insertItem(dAtoi(argv[2]),  argv[3], argv[4],argv[5], norm, expand, dAtoi(argv[8]), argv[9]));
}
...
ConsoleMethod(GuiTreeViewCtrl, clearChildItem, bool, 3, 3, "(TreeItemId item)")
{
	S32 id = dAtoi(argv[2]);
	return object->clearChildItem(object->getItem(id));
}

ConsoleMethod(GuiTreeViewCtrl, expandToItem, bool, 3, 4, "(TreeItemId item, bool expand=true)")
{
	S32 id = dAtoi(argv[2]);
	bool expand = true;
	if(argc == 4)
	...
	if(id == 0)
		id = object->getFirstRootItem();
	return(object->setItemExpandedTo(object->getItem(id), expand));
}
...
bool GuiTreeViewCtrl::clearItem()
{
	destroyTree();
	return true;
}
...
//void GuiTreeViewCtrl::setMissionTreeInfo(S32 tpId,S32 mId)
//{
//	mItems[tpId-1]->mMissionId = mId;
//}

// <Edit>:[soar]:任务树所需函数
//ConsoleMethod(GuiTreeViewCtrl, setMissionTreeInfo, void, 4, 4, "obj.setMissionTreeInfo(tpId,mId)")
//{
//	object->setMissionTreeInfo(dAtoi(argv[2]),dAtoi(argv[3]));
//}

S32 GuiTreeViewCtrl::findMissionTreeNode(const char * mId)
{
	for (int i = 0; i < mItems.size(); i++)
	{
		if (mItems[i] != NULL)
		{
			if ( strcmp( mItems[i]->mMissionId , mId) == 0 )
			{
				return i + 1;
			}
		}
...
ConsoleMethod(GuiTreeViewCtrl, findMissionTreeNode, S32, 3, 3, "obj.findMissionTreeNode(mid)")
{
	return(object->findMissionTreeNode(argv[2]));
}

bool GuiTreeViewCtrl::isHasMission(const char * mId)
{
	for (int i = 0; i < mItems.size(); i++)
	{
		if (mItems[i] != NULL)
...
int GuiTreeViewCtrl::_strcmp( const char* s1, const char* s2 )
{
	int n1 = strlen( s1 );
	int n2 = strlen( s2 );
	int len  = n1 > n2 ? n1 : n2;
...
ConsoleMethod(GuiTreeViewCtrl, isHasMission, bool, 3, 3, "obj.isHasMission(mid)")
{
	return(object->isHasMission(argv[2]));
}

ConsoleMethod(GuiTreeViewCtrl, clearitem, bool, 2, 2, "obj.clearitem()")
{
	return object->clearItem();
}