...
ConsoleFunction( StripMLControlChars, const char*, 2, 2, "(string val)"
                "Strip PowerEngineML control characters from the specified string, returning a 'clean' version.")
{
...
void GuiMLTextCtrl::initPersistFields()
{
   ...
   addField("maxLineNum",        TypeS32,    Offset(mMaxLineNum,        GuiMLTextCtrl));
   //addField("deniedSound",       TypeSFXProfilePtr, Offset(mDeniedSound, GuiMLTextCtrl));
   addField("soundID",			 TypeString, Offset(mSoundID,			GuiMLTextCtrl));
...
void GuiMLTextCtrl::drawAtomText(bool sel, U32 start, U32 end, Atom *atom, Line *line, Point2I offset)
{
   ...
   Point2I p2;
   char buf[512];
   CommonFontEX *font = atom->style->font->fontRes;
   if(start != atom->textStart)
   {
      const UTF16* buff = mTextBuffer.getPtr() + atom->textStart;
	  ZeroMemory(buf, sizeof(buf));
	  convertUTF16toUTF8(buff, buf, sizeof(buf));
      xOff += font->getStrNWidth(buf,start - atom->textStart);
   }
   ...
   //  <Edit>:[thinking] 修改中文字体显示不清楚问题,同时转成utf8处理
   const UTF16* tmp = mTextBuffer.getPtr() + start;
   ZeroMemory(buf, sizeof(buf));
   convertUTF16toUTF8(tmp, buf, sizeof(buf));
   U32 tmpWidth = font->getStrNWidth(buf,end-start);
   U32 tmpLen = font->getBreakPos(buf,dStrlen(buf),tmpWidth,true);
   ...
   if(!sel)
   {
      if(atom->style->shadowOffset.x || atom->style->shadowOffset.y)
      {
         ...
         drawer->drawTextN(font, drawPoint + atom->style->shadowOffset, 
            buf, tmpLen, mAllowColorChars ? mProfile->mFontColors : NULL);
      }

      color.alpha = (S32)(mAlpha * color.alpha);
      drawer->setBitmapModulation(color);
      drawer->drawTextN(font, drawPoint, buf, tmpLen, mAllowColorChars ? mProfile->mFontColors : &( mProfile->mFontColor ) );

      //if the atom was "clipped", see if we need to draw a "..." at the end
      if (atom->isClipped)
      {
         p2 = drawPoint;
         p2.x += font->getStrWidth(buf);
   ...
   else
   {
      ...
      rect.extent.x = font->getStrNWidth(buf,end-start) + 1;
      ...
      drawer->drawTextN(font, drawPoint, buf, tmpLen, mAllowColorChars ? mProfile->mFontColors : NULL);
...
void GuiMLTextCtrl::addText(const char* textBuffer, const U32 numChars, bool reformat)
{
   ...
   dMemcpy(tmp, textBuffer, numChars);
   tmp[numChars] = 0;
   ...
   for(Line *walk = mLineList; walk; walk = walk->next)
   {
      if ((mCursorPosition <= walk->textStart + walk->len) || (walk->next == NULL))
      {
         ...
         for(Atom *awalk = walk->atomList; awalk; awalk = awalk->next)
         {
            ...
            x = awalk->xStart;
            CommonFontEX *font = awalk->style->font->fontRes;

            const UTF16* buff = mTextBuffer.getPtr() + awalk->textStart;
			char buf[512];
			ZeroMemory(buf, sizeof(buf));
			convertUTF16toUTF8(buff, buf, sizeof(buf));
			x += font->getStrNWidth(buf,mCursorPosition - awalk->textStart);
                                ...
                                const U32   position)
{
   ...
   if (numCharsToInsert <= 0)
   {
      ...
      if ( numInputChars > 0 && mSoundID != NULL )
...
GuiMLTextCtrl::Font *GuiMLTextCtrl::allocFont(const char *faceName, U32 faceNameLen, U32 size)
{
   ...
   dMemcpy(ret->faceName, faceName, faceNameLen);
   ret->faceName[faceNameLen] = '\0';
   ret->faceNameLen = faceNameLen;
   ret->size = size;
   ret->next = mFontList;
   ret->fontRes = CommonFontManager::GetFont(ret->faceName, size);
   if(bool(ret->fontRes))
   {
      ret->next = mFontList;
      mFontList = ret;
...
GuiMLTextCtrl::Bitmap *GuiMLTextCtrl::allocBitmap(const char *bitmapName, U32 bitmapNameLen)
{
   ...
   dMemcpy(ret->bitmapName, bitmapName, getMin(bitmapNameLen,BitmapNameSize));
...
void GuiMLTextCtrl::emitNewLine(U32 textStart)
{
   ...
   //  <Edit>:[thinking]增加行间距处理
   if(mCurY)
	   mCurY += mLineSpacingPixels;

   //  <Edit>:[thinking] 处理行限制
   if(mMaxLineNum>0 && mMaxLineNum * l->height < mCurY)
	   return;
...
void GuiMLTextCtrl::emitTextToken(U32 textStart, U32 len)
{
   ...
   CommonFontEX *font = mCurStyle->font->fontRes;
...
GuiMLTextCtrl::Atom *GuiMLTextCtrl::splitAtomListEmit(Atom *list, U32 width)
{
   ...
   while(list)
   {
      CommonFontEX *font = list->style->font->fontRes;
      U32 breakPos;
      ...
      if (mCurClipX > 0)
      {
         ...
         else
         {
            //set the atom width == to the string length
			 char buf[512];
			 ZeroMemory(buf, sizeof(buf));
			 convertUTF16toUTF8(tmp16,buf,sizeof(buf));
            list->width = font->getStrWidth(buf);
   ...
   {
      CommonFontEX *font = clipAtom->style->font->fontRes;
      U32 breakPos;
...
textemit:
	  ...
	  while(1)	  
	  {
		  UTF16 ch = mTextBuffer.getChar(mScanPos+idx);
		  if (ch == '\t' || ch == '<' ||ch == '\n' || ch == 0)		  
			  break;		
		  idx++;
	  }
