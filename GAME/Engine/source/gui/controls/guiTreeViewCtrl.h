      ...
      struct Item
      {
         ...
         enum ItemState
         {
            ...
            VirtualParent  = BIT(6), ///< This indicates that we should be rendered as
                                     ...
                                     ///  PowerEngine, which might have thousands of objects).
         };
         ...
         const S32 getDisplayTextWidth(CommonFontEX *font);
...
protected:
	  ...
	  StringTableEntry mTexRolloverName;
      GFXTexHandle	   mTexRollover;
	  StringTableEntry mTexSelectedName;
      GFXTexHandle	   mTexSelected;

	  bool				mSorted;           //按字符内容排序
	  bool              mSortedMid;        //Mid 字符方式排序
	  bool              mSortedNumMid;     //Mid 数字方式排序

	  Item* mLastMoveHitItem;
	  Item* mMouseDownTempItem;
	  bool  bIsHitSel;
   ...
   public:
	  ...
	  int getItemByValue( const char* value);
	  ...
	  bool setItemExpandedTo(Item* item, bool expand);
      ...
      S32 insertItem(S32 parentId, const char * text, const char * value = "",const char * iconString = "", S16 normalImage = 0, S16 expandedImage = 1, S16 tag = 0,const char * missionId = "");
	  ...
	  //void setMissionTreeInfo(S32 tpId,S32 mId);
	  S32 findMissionTreeNode(const char * mId);
	  bool isHasMission(const char * mId);
	int _strcmp( const char* s1, const char* s2 );

	bool clearItem();
	bool clearChildItem(Item* item);
