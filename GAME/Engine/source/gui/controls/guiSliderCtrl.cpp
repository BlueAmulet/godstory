...
void GuiSliderCtrl::initPersistFields()
{
	...
	addField("DisplayTicks", TypeBool,         Offset(mDisplayTicks,GuiSliderCtrl));
	endGroup( "Slider" );
}
...
ConsoleMethod( GuiSliderCtrl, SetValue,void, 3, 3, "Set the Position of the slider.")
{
    object->updateThumb(dAtof(argv[2]),true);
}
//----------------------------------------------------------------------------
...
void GuiSliderCtrl::onRender(Point2I offset, const RectI &updateRect)
{
	...
	if( mHasTexture )
	{
		...
		//left border
		GFX->getDrawUtil()->drawBitmapSR(mProfile->mTextureObject, Point2I(offset.x,offset.y + (getHeight() / 4)), mBitmapBounds[SliderLineLeft]);
		//right border
		GFX->getDrawUtil()->drawBitmapSR(mProfile->mTextureObject, Point2I(offset.x + getWidth() - mBitmapBounds[SliderLineRight].extent.x, offset.y + (getHeight() / 4)), mBitmapBounds[SliderLineRight]);
	...
	if(mDisplayValue)
	{
		...
		S32 txt_w = mProfile->mFont->getStrWidth(( UTF8 *)buf);
