...
//------------------------------------------------------------------------------
//[2009/11/11][thinking]:因closePopUp做了脚本和相应处理,所以在此处什么也不要处理
...
void GuiPopupTextListCtrl::onRenderCell(Point2I offset, Point2I cell, bool selected, bool mouseOver)
{
   ...
   if(mouseOver)
   {
	   if(mPopUpCtrl->mTextureHilight)
	   {
		   RectI rect(offset, mPopUpCtrl->mBitmapBounds);
		   GFX->getDrawUtil()->clearBitmapModulation();
		   GFX->getDrawUtil()->drawBitmapStretch(mPopUpCtrl->mTextureHilight, rect);

	   }
	   else
	   {
		   //  Render a background colour for the cell
		   RectI cellR(offset.x, offset.y, size.x, size.y);
		   ColorI color = mProfile->mFillColorNA; 
		   GFX->getDrawUtil()->drawRectFill(cellR, color);
	   }
   }
   else if(selected)
   {
	   if (mPopUpCtrl->mTextureDepressed)
	   {
		   RectI rect(offset, mPopUpCtrl->mBitmapBounds);
		   GFX->getDrawUtil()->clearBitmapModulation();
		   GFX->getDrawUtil()->drawBitmapStretch(mPopUpCtrl->mTextureDepressed, rect);
	   ...
	   else
	   {
		   //  Render a background colour for the cell
		   RectI cellR(offset.x, offset.y, size.x, size.y);
		   ColorI color = mProfile->mFillColorHL;
		   GFX->getDrawUtil()->drawRectFill(cellR, color);
	   }
   }
   ...
   if(colcount >= 2)
   {
      ...
      GFX->getDrawUtil()->drawText( mFont, Point2I( textXOffset, offset.y ), buff, &fontColor); //  Used mTextOffset as a margin for the text list rather than the hard coded value of '4'.

      // Draw the second column to the right
      getColumn(mList[cell.y].text, buff, 1, "\t");
      S32 txt_w = mFont->getStrWidth(buff);

      GFX->getDrawUtil()->drawText( mFont, Point2I( offset.x+size.x-mProfile->mTextOffset.x-txt_w, offset.y ), buff, &fontColor); //  Used mTextOffset as a margin for the text list rather than the hard coded value of '4'.

   } else
   {
      GFX->getDrawUtil()->drawText( mFont, Point2I( textXOffset, offset.y ), mList[cell.y].text, &fontColor); //  Used mTextOffset as a margin for the text list rather than the hard coded value of '4'.
   }
}
...
void GuiPopUpMenuCtrl::setBitmap(const char *name)
{
   ...
   if (*mBitmapName)
   {
	  ...
	  mTextureHilight = GFXTexHandle((StringTableEntry)buffer, &GFXDefaultGUIProfile);
...
void GuiPopUpMenuCtrl::onRender(Point2I offset, const RectI &updateRect)
{
   ...
   //  <Edit>:[thinking]修改绘制次序，保持文字总是显示在后面，解决文字超长问题
   ...
   else if(mMouseOver && mActive) // TODO: Add onMouseEnter() and onMouseLeave() and a definition of mMouseOver (see guiButtonBaseCtrl) for this to work.
   {
	   S32 l = r.point.x, r2 = r.point.x + r.extent.x - 1;
	   S32 t = r.point.y, b = r.point.y + r.extent.y - 1;
   ...
   else
   {
	   ...
	   if(mProfile->getChildrenProfile() && mProfile->mBitmapArrayRects.size())
	   {
		   // Render the fixed, filled in border
		   if(!mActive)
			   renderFixedBitmapBordersFilled(r, 4, mProfile);
		   else
...
void GuiPopUpMenuCtrl::closePopUp()
{
   ...
   */
   ...
   // Kill the popup:
   //mBackground->removeObject( mSc );
   //mSc->removeObject(mTl);
   //mTl->deleteObject();
   //mSc->deleteObject();
   //mBackground->deleteObject();
   //mBackground = NULL;

	//[2009/12/31][thinking]:因在这释放会引起宕机，
	//原因是onMouseDown要遍历所有子控件，所以不在在这做真正的删除
...
#endif
#ifdef NTJ_CLIENT
	newBounds.point = getParent()->globalToLocalCoord(scrollPoint);
#endif
...
#ifdef  NTJ_CLIENT
   getParent()->addObject(mBackground);
#endif
