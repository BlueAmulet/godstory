...
/// The GUI system in PowerEngine provides a powerful way of creating
...
///
/// GuiControl is the base class for GUI controls in PowerEngine. It provides these
...
protected:
   ...
   bool m_DirtyFlag;
   static U32 m_ForceDirtyFlag;
public:
	void setDirtyFlag(bool isDirtyFlag = true) { m_DirtyFlag = isDirtyFlag; }
   ...
   S32                mTipHoverTime;
    ...
    enum horizSizingOptions
    {
		...
		horizResizeProportion   ///< resize Proportion
    };
    enum vertSizingOptions
    {
		...
		vertResizeProportion    ///< resize Proportion
    };
...
protected:
    ...
    void setFloatVariable(F64 value);
    ...
    F64 getFloatVariable();    ///< Returns value of control's bound variable as a float
...
public:
    ...
    static bool setVisible( void* obj, const char* data ) { static_cast<GuiControl*>(obj)->setVisible( dAtob( data ) ); return false; };
...
public:
	...
	virtual void onSetVisible(bool){};
	virtual bool onParentSetVisible(bool){return false;};
	...
	//[2009/7/14][thinking]:增加指定绘制长度文本
	void renderJustifiedTextN(Point2I offset,Point2I extent,const char *text,U32 len);

    void inspectPostApply();
    void inspectPreApply();
    //className 控件类名，classMethod　控件脚本方法（要求有返回布尔值）
	bool execChildMethod(const char *className,const char *classMethod);
private:
	//cull none
...
public:
	...
	virtual GuiControl* onHitControlMouseDown(const GuiEvent &event, const Point2I &pt,Point2I offset, RectI clipRect, S32 initialLayer = -1);
	virtual GuiControl* onHitControlMouseRightDown(const GuiEvent &event, const Point2I &pt,Point2I offset, RectI clipRect, S32 initialLayer = -1);
	virtual GuiControl* onHitControlMouseMove(const GuiEvent &event, const Point2I &pt,Point2I offset, RectI clipRect, S32 initialLayer = -1);
	bool renderTooltipAdv(Point2I cursorPos, const char* tipText = NULL );
};
/// @}
