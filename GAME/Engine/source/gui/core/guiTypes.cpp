   ...
   mBitmapArrayRects(0)
{
   ...
   if (def)
   {
	  ...
	  mFontBold      = def->mFontBold;
	  mFontItalic    = def->mFontItalic;
	  mBitmapFont    = def->mBitmapFont;
	  mFontOutline   = def->mFontOutline;
...
void GuiControlProfile::initPersistFields()
{
   ...
   addField("fontBold",      TypeBool,       Offset(mFontBold,    GuiControlProfile));
   addField("fontItalic",    TypeBool,       Offset(mFontItalic,  GuiControlProfile));
   addField("bitmapFont",    TypeBool,       Offset(mBitmapFont,  GuiControlProfile));
   addField("fontOutline",   TypeColorI,     Offset(mFontOutline, GuiControlProfile));
   ...
   addField("soundID",		  TypeString,	 Offset(mSoundID, GuiControlProfile));
...
void GuiControlProfile::incRefCount()
{
   if(!mRefCount++)
   {
      ...
      //verify the font
	  mFont = CommonFontManager::GetFont(const_cast<char*>(mFontType), mFontSize,!mBitmapFont,0,0,true,mFontItalic,mFontBold);
      if (!mFont)
         Con::errorf("Failed to load/create profile font (%s/%d)", mFontType, mFontSize);

      //verify the bitmap
      if (mBitmapName && mBitmapName[0] && !mTextureObject.set( mBitmapName, &GFXDefaultPersistentProfile ))
