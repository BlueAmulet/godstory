...
////#define STATEBLOCK
//cull none
GFXStateBlock* GuiCanvas::mCullNoneSB = NULL;
//z enable
GFXStateBlock* GuiCanvas::mZTrueSB = NULL;

IMPLEMENT_CONOBJECT(GuiCanvas);

ColorI gCanvasClearColor( 72,72,72); ///< For GFX->clear
...
bool GuiCanvas::mGlobalDirty = false;
//-----------------------------------------------------------------------------

GuiCanvas::GuiCanvas(): GuiControl(),
mCursorEnabled(true),
mForceMouseToGUI(false),
mClampPowerEngineCursor(true),
mShowCursor(true),
mLastCursorEnabled(false),
mTempWndControl(NULL),
mMouseControl(NULL),
...
void Process1End()
{
	...
	g_MultThreadWorkMgr->tryProcess(true);
#endif
}
...
bool GuiCanvas::onAdd()
{
	...
	// <Edit> [4/15/2009 joy] 初始化DisplayMode
	g_GraphicsProfile.InitDisplayMode();
	...
	if (a && a->mType != NullDevice)
	{
		...
		Win32Window* w32win = (Win32Window*)mPlatformWindow;
		winState.appWindow = w32win->getHWND();

		// Set it as the current render target so we can see outputs.
		// Do we need to do this here? -patw
		...
		mPlatformWindow->syncDisplayEvent.notify(this, &GuiCanvas::handleSyncEvent);
	...
	Process::notify(this, &GuiCanvas::present, PROCESS_BEFORE_RENDER_ORDER);
#ifdef USE_MULTITHREAD_ANIMATE
	Process::notify(Process1End, PROCESS_GRAPHIC_THREAD_ORDER);
#endif

	// Set the initial state of the device
	newDevice->setInitialGFXState();
...
void GuiCanvas::onRemove()
{
	...
	Process::remove(this, &GuiCanvas::present);
#ifdef USE_MULTITHREAD_ANIMATE
	Process::remove(Process1End);
#endif
...
void GuiCanvas::handleResize( DeviceId did, S32 width, S32 height )
{
	// Nothing to do here right now.
}

void GuiCanvas::handleSyncEvent(DeviceId did)
{
	mGlobalDirty = true;
}

void GuiCanvas::handlePaintEvent(DeviceId did)
{
	renderFrame(false);
}

void GuiCanvas::onSetVisible(bool flag)
{
	if(!flag)
	{
		mFirstRender = true;
...
void GuiCanvas::handleAppEvent( DeviceId did, S32 event )
{
	...
	if(event == GainFocus)
	{
		...
		// 失去焦点后，再次获取窗口焦点时，重置初始光标位置
		// 2009.09.21 daniel
		POINT curPos;
		::GetCursorPos(&curPos);
		Win32Window* w32win = (Win32Window*)mPlatformWindow;
		ScreenToClient(w32win->getHWND(), &curPos);
		mCursorPt.set(curPos.x, curPos.y);
	}
	...
	// 从 3DS Max 发送过来的预览更新事件
#ifdef NTJ_EDITOR
	if ( event == PreviewUpdate )
	{
		gPreviewMission = true;
		Con::executef( "PreviewUpdate" );
	}
#endif
}
...
Point2I GuiCanvas::getWindowSize()
{
	...
	return mPlatformWindow->getBounds().extent;
}
...
void GuiCanvas::setClampPowerEngineCursor(bool onOff)
{
	mClampPowerEngineCursor = onOff;
}
...
{
	// <Edit> [1/29/2010 joy] 可能会导致鼠标事件错误
	if(mCursorEnabled != onOff)
	{
...
#pragma message(ENGINE(鼠标移动事件放到脚本，移动范围将不会受限制))
		if (mCursorEnabled || mForceMouseToGUI || (inputEvent.objInst != SI_XAXIS && inputEvent.objInst != SI_YAXIS))
		{
...
void GuiCanvas::rootMouseDown(const GuiEvent &event)
{
	...
	else
	{
		...
		while (i != begin())
		{
			...
			Point2I pt = event.mousePoint;
			GuiControl *controlHit = ctrl->onHitControlMouseDown( event, pt, ctrl->getPosition(), ctrl->getBounds() );
			if( controlHit )
			{
				if (mTempWndControl && !isTempWnd(controlHit))
				{
					mTempWndControl->setVisible(false);
					mTempWndControl = NULL;
				}
			...
			//// }
			//// //---------------------------------------------
			////SimObjectId id = controlHit->getId();
			////         
			////controlHit->onMouseDown(event);
			////
			////if( !Sim::findObject(id ) )
			////	break;

			////         if( !controlHit->mClickOut )
			////	break;
			////else
			////{	
			////	controlHit->mClickOut = false;
			////	continue;
			////}
			////else
			////{
...
void GuiCanvas::rootMouseMove(const GuiEvent &event)
{
	...
	else
	{
		...
		//	GuiControl *controlHit = ctrl->onHitControlMouseMove( event, pt, ctrl->getPosition(), ctrl->getBounds() );
...
void GuiCanvas::rootRightMouseDown(const GuiEvent &event)
{
	...
	else
	{
		...
		while (i != begin())
		{
			...
			GuiControl *controlHit = ctrl->onHitControlMouseRightDown( event, pt, ctrl->getPosition(), ctrl->getBounds() );

			if( controlHit )
			{
				if (mTempWndControl && isTempWnd(controlHit))
					mTempWndControl->setVisible(true);

				if( controlHit != ctrl )
				{
#ifdef NTJ_CLIENT
					if((controlHit != g_UIMouseGamePlay) &&
						g_UIMouseGamePlay->getMouseState() != dGuiMouseGamePlay::MouseState_None)
					{
						g_UIMouseGamePlay->setMouseState(dGuiMouseGamePlay::MouseState_None);
						g_UIMouseGamePlay->UpdateMouseState(event.mousePoint);
					}
#endif
					break;
				}
			...
			{
			#ifdef NTJ_CLIENT
			if((mMouseControl != g_UIMouseGamePlay) &&
			g_UIMouseGamePlay->getMouseState() != dGuiMouseGamePlay::MouseState_None)
			{
			g_UIMouseGamePlay->setMouseState(dGuiMouseGamePlay::MouseState_None);
			g_UIMouseGamePlay->UpdateMouseState(event.mousePoint);
			...
			if (mTempWndControl && !isTempWnd(mMouseControl))
			{
			mTempWndControl->setVisible(false);
			mTempWndControl = NULL;
			}
...
void GuiCanvas::paint()
{
	...
	if( mPlatformWindow && mPlatformWindow->isVisible() && GFX->allowRender())
		mPlatformWindow->displayEvent.trigger(mPlatformWindow->getDeviceId());
}
...
void GuiCanvas::renderFrame(bool preRenderOnly, bool bufferSwap /* = true */)
{
	...
	if(!mPlatformWindow->isVisible() || !GFX->allowRender())
		return;
	...
	if( ((GFXPCD3D9WindowTarget*)renderTarget)->getWindow() == NULL )
	{
		PROFILE_END(CanvasPreRender);
		return;
	}
	...
	PROFILE_START(Canvase_preRender);
	preRender();
	PROFILE_END(Canvase_preRender);
	...
	//	updateMouseGameplayUI(updateUnion);  //处理当前帧的3D渲染准备工作
...
#ifdef STATEBLOCK
			AssertFatal(mCullNoneSB, "GuiCanvas::renderFrame -- mCullNoneSB cannot be NULL.");
			mCullNoneSB->apply();
#else
...
#ifndef NTJ_CLIENT
					mHoverPositionSet = mMouseControl->renderTooltip(mHoverPosition);
#else
					if( mIsShowTooltip == false )
					{
						mIsShowTooltip = true;
						Con::executef( "StartTooltip", Con::getIntArg( mMouseControl->getId() ) );
					}

					mHoverPositionSet = mMouseControl->renderTooltipAdv(mHoverPosition);
#endif
				}
				else
				{
					if( mIsShowTooltip == true )
					{
						mIsShowTooltip = false;
						Con::executef( "EndTooltip" );
					}
				}
	...
	PROFILE_START(updateWaterQuery);
	updateWaterQuery();
	updateReflections();
	PROFILE_END(updateWaterQuery);

	PROFILE_START(GFXEndScene);
	GFX->endScene();
	PROFILE_END(GFXEndScene);

	mFirstRender = false;
	
#ifdef DEBUG
	gClientSceneGraph->setDPtimes(GFXDevice::mDPTimes);
#endif
...
void GuiCanvas::swapBuffers()
{
	...
	PROFILE_START(GuiCanvas_SwapBuffers);
	mPlatformWindow->getGFXTarget()->present();
	PROFILE_END(GuiCanvas_SwapBuffers);
} 
...
// CodeReview -- We should move this.  Maybe with a begin scene signal or something. [btr 7/30/07]
void GuiCanvas::updateReflections()
{
	...
	{
#ifdef STATEBLOCK
		AssertFatal(mZTrueSB, "GuiCanvas::updateReflections -- mZTrueSB cannot be NULL.");
		mZTrueSB->apply();
#else
		GFX->setZEnable( true );
#endif
	...
	PROFILE_END(GuiCanvas_updateReflections);
#endif
}

// CodeReview -- We should move this.  Maybe with a begin scene signal or something. [btr 7/30/07]
void GuiCanvas::updateWaterQuery()
{
#ifndef POWER_TGB_ONLY
	static SimSet *reflectSet = NULL;
...
#ifdef STATEBLOCK
		AssertFatal(mZTrueSB, "GuiCanvas::updateWaterQuery -- mZTrueSB cannot be NULL.");
		mZTrueSB->apply();
#else
		GFX->setZEnable( true );
#endif
...
void GuiCanvas::resetStateBlock()
{
	//mCullNoneSB
	GFX->beginStateBlock();
...
void GuiCanvas::releaseStateBlock()
{
	if (mCullNoneSB)
	{
		mCullNoneSB->release();
...
#if !defined(POWER_OS_MAC) && !defined(POWER_OS_XENON) && !defined(POWER_DEBUG)
			  ...
			  "\\return (string) The current screen mode as \"(int)width (int)height (bool)fullscreen (int)bitdepth (int)refreshRate\"")
{
	...
	GFXVideoMode vm = object->getPlatformWindow()->getCurrentMode();
...
ConsoleMethod( GuiCanvas, toggleFullscreen, void, 2, 2, "() - toggle canvas from fullscreen to windowed mode or back.")
{
	...
	GFXVideoMode origMode = object->getPlatformWindow()->getCurrentMode();
	...
	object->getPlatformWindow()->setMode(newMode);
}
...
ConsoleMethod( GuiCanvas, isFullscreen, bool, 2, 2, "() - Is this canvas currently fullscreen?")
{
	...
	return object->getPlatformWindow()->getCurrentMode().fullScreen;
}
...
ConsoleMethod( GuiCanvas, minimizeWindow, void, 2, 2, "() - minimize this canvas' window.")
{
	...
	object->getPlatformWindow()->minimize();
}

ConsoleMethod( GuiCanvas, restoreWindow, void, 2, 2, "() - show this canvas' window again.")
{
	if (!object->getPlatformWindow())
		return;

	if(!object->getPlatformWindow()->isVisible())
		object->getPlatformWindow()->show();
}

ConsoleMethod( GuiCanvas, showWindow, void, 2, 2, "() - show this canvas' window again.")
{
	if (!object->getPlatformWindow())
		return;    
...
ConsoleMethod( GuiCanvas, hideWindow, void, 2, 2, "() - hide this canvas' window again.")
{
	if (!object->getPlatformWindow())
		return;    
			  ...
			  "\\param antialiasLevel [optional] The level of anti-aliasing to apply 0 = none")
{
	...
	vm.resolution.x = dAtoi(argv[2]);
	vm.resolution.y = dAtoi(argv[3]);
	...
	// 规范mode
	g_GraphicsProfile.ClampDisplayMode(vm);

#ifndef POWER_OS_XENON
	object->getPlatformWindow()->setMode(vm);
#endif
...
//设置打开的窗口为零时窗口
void GuiCanvas::setTempWnd(GuiControl* ctrl)
{
	if(NULL == ctrl)
		mTempWndControl = NULL;
	else 
		mTempWndControl = ctrl;
}

//判断当前点击的窗口是否是零时保存的窗口
bool GuiCanvas::isTempWnd(GuiControl* ctrl)
{
	while (ctrl)
	{
		if (mTempWndControl == ctrl )
		{
			return true;
		}
		ctrl = ctrl->getParent();
...
ConsoleMethod(GuiCanvas,setTempWnd,void,3,3,"setTempWnd( %ctrl )")
{

	if(!dStrcmp(argv[2],""))
	{
		object->setTempWnd(NULL);
	}
	else
	{
		GuiControl *pControl = dynamic_cast<GuiControl *>(Sim::findObject(argv[2]));
		object->setTempWnd(pControl);
	}
}

ConsoleMethod(GuiCanvas,resetMode,void,2,2, "" )
{
	object->getPlatformWindow()->getGFXTarget()->resetMode();
}