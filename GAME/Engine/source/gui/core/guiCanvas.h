...
/// request. PowerEngine keeps track of layers of dialogs. The dialog with the highest
...
protected:
   ...
   SimObjectPtr<GuiControl>   mTempWndControl;        ///< 用来保存零时窗口
   ...
   void handlePaintEvent (DeviceId did);
...
public:
   ...
   SimObjectPtr<GuiControl>	  mLastMoveMouseControl;  ///< the control the mouse was last move
   ...
   /// Sets if the PowerEngine cursor should be clamped to the window.
   /// @param  onOff    True if the PowerEngine cursor should be clamped against the window
   virtual void setClampPowerEngineCursor(bool onOff);

   /// Returns if the PowerEngine cursor is clamped to the window
   virtual bool getClampPowerEngineCursor() {return mClampPowerEngineCursor;}
   ...
   /// to be confused with the PowerEngine accelerator key system, to keep them
   ...
   void setTempWnd(GuiControl* ctrl);
   bool isTempWnd(GuiControl* ctrl);
