...
//---------------------------------------------------------------------------
////#define STATEBLOCK
GFXStateBlock* MissionAreaEditor::mAlphaBlendFalseSB = NULL;
GFXStateBlock* MissionAreaEditor::mAlphaLerpSB = NULL;
...
void MissionAreaEditor::updateTerrainBitmap()
{
   ...
   if( bitmap )
      setBitmap( GFXTexHandle( bitmap, &GFXDefaultGUIProfile, true ) );
}
...
void MissionAreaEditor::onRender(Point2I offset, const RectI & updateRect)
{
   ...
   U32 mask = GameObjectType | VehicleObjectType | StaticShapeObjectType | WaterObjectType | TriggerObjectType;
   gServerContainer.findObjects(mask, findObjectsCallback, &objects);
#ifdef STATEBLOCK
	AssertFatal(mAlphaBlendFalseSB, "MissionAreaEditor::onRender -- mAlphaBlendFalseSB cannot be NULL.");
...
#ifdef STATEBLOCK
		AssertFatal(mAlphaLerpSB, "MissionAreaEditor::onRender -- mAlphaLerpSB cannot be NULL.");
		mAlphaLerpSB->apply();
#else
		// draw it
		GFX->setAlphaBlendEnable( true );
...
void MissionAreaEditor::resetStateBlock()
{
	//mAlphaBlendFalseSB
	GFX->beginStateBlock();
	...
	GFX->setRenderState(GFXRSSrcBlend, GFXBlendSrcAlpha);
	GFX->setRenderState(GFXRSDestBlend, GFXBlendInvSrcAlpha);
	GFX->endStateBlock(mAlphaLerpSB);
}


void MissionAreaEditor::releaseStateBlock()
{
	...
	if (mAlphaLerpSB)
	{
		mAlphaLerpSB->release();
	}
}

void MissionAreaEditor::init()
{
	if (mAlphaBlendFalseSB == NULL)
	{
		mAlphaBlendFalseSB = new GFXD3D9StateBlock;
		mAlphaBlendFalseSB->registerResourceWithDevice(GFX);
		mAlphaBlendFalseSB->mZombify = &releaseStateBlock;
		mAlphaBlendFalseSB->mResurrect = &resetStateBlock;

		mAlphaLerpSB = new GFXD3D9StateBlock;
		resetStateBlock();
	}
}

void MissionAreaEditor::shutdown()
{
	SAFE_DELETE(mAlphaBlendFalseSB);
	SAFE_DELETE(mAlphaLerpSB);
}
