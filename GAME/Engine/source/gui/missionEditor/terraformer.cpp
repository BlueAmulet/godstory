...
GBitmap* Terraformer::getScaledGreyscale(U32 r)
{
   ...
   F32 scale = 1.0f/(fmax-fmin);

   GBitmap* bitmap = new GBitmap(blockSize, blockSize, false);

   U8 *rgb = bitmap->getAddress(0,0);
   for (S32 y = blockSize - 1; y >= 0; y--)
   {
      for (S32 x = 0; x < blockSize; x++)
      {
			ColorF c;
			F32 index = (src->val(wrap(x), wrap(y))-fmin) * scale;
			c.interpolate(ColorF(0, 0, 0), ColorF(1.0f, 1.0f, 1.0f), index);
			*rgb++ = (U8)(c.red*255.0f);
			*rgb++ = (U8)(c.green*255.0f);
			*rgb++ = (U8)(c.blue*255.0f);
      }
   }
   return(bitmap);
}

GBitmap* Terraformer::getScaled(U32 r)
{
	   enum  { NUM_COLORS = 2 };
...
bool Terraformer::loadGreyscale(U32 r, GBitmap *bmp)
{
   ...
   for (S32 y=0; y<blockSize; y++)
      for (S32 x=0; x<blockSize; x++)
      {
         ...
         // compute the luminance of each RGB
         dst->val(x, y) = ((F32)color[0]) * (0.299f/256.0f) +
...
bool Terraformer::setTerrain(U32 r)
{
   ...
   terrBlock->updateGrid(
               Point2I(0,0), 
               Point2I(TerrainBlock::BlockSize, TerrainBlock::BlockSize));
/*   {
...
//------------------------------------------------------------------------------
bool Terraformer::setHeightmap(U32 r, U16 *hmap, F32 scale, U32 offsetX, U32 offsetY)
{
   Heightfield *src = getRegister(r);
   ...
   for (U32 y=0; y < TerrainBlock::BlockSize; y++)
      ...
      {
         F32 f = (src->val(x+offsetX,y+offsetY)) * scale;
         hmap[x + (y << TerrainBlock::BlockShift)] = floatToFixed( f );
      }
...
void Terraformer::setCameraPosition(const Point3F &pos)
{
   ...
   GameBase *camera = NULL;
   if(connection)
      camera = dynamic_cast<GameBase*>(connection->getControlObject());
...
bool Terraformer::smooth(U32 r_src, U32 r_dst, F32 factor, U32 iterations)
{
   ...
   *b = *src;
   ...
   {
      for (S32 y=border; y<blockSize-border; y++)
      {
         for (S32 x=border; x<blockSize-border; x++)
         {
            F32 array[9];
...
bool Terraformer::terrainData(U32 r)
{
   ...
   if (queryList.mList.size() > 0)
   {
      TerrainBlock * terrBlock = dynamic_cast<TerrainBlock*>(queryList.mList[0]);
      if(!terrBlock)
         return(setHeight(r, 0.f));
...
ConsoleMethod( Terraformer, previewScaled, void, 4, 4, "(GuiTerrPreviewCtrl destination, int source)")
{
   ...
   GBitmap* bmp = object->getScaled(dAtoi(argv[3]));

   dcc->setBitmap( GFXTexHandle( bmp, &GFXDefaultGUIProfile, true ) );
}

ConsoleMethod( Terraformer, previewScaledGreyscale, void, 4, 4, "(GuiTerrPreviewCtrl destination, int source) Create a preview of the current terrain block.\n"
			  "@param destination The preview control.\n"
			  "@param source The source terrain\n"
			  "@return No return value.")
{
   GuiTerrPreviewCtrl *dcc;
   Sim::findObject(argv[2], dcc);

   GBitmap* bmp = object->getScaledGreyscale(dAtoi(argv[3]));
...
ConsoleMethod( Terraformer, preview, void, 4, 4, "(GuiTerrPreviewCtrl destination, int register)")
{
   ...
   bmc->setBitmap( GFXTexHandle( bmp, &GFXDefaultGUIProfile, true ) );
}
