...
//---------------------------------------------------------------------------
////#define STATEBLOCK
GFXStateBlock* WorldEditor::mSetSelSB = NULL;
GFXStateBlock* WorldEditor::mSetObjBoxSB = NULL;
GFXStateBlock* WorldEditor::mSetObjFaceSB = NULL;
GFXStateBlock* WorldEditor::mClearObjFaceSB = NULL;
GFXStateBlock* WorldEditor::mSetPlaneSB = NULL;
GFXStateBlock* WorldEditor::mSetPathSB = NULL;
//GFXStateBlock* WorldEditor::mSetPath2SB = NULL;
GFXStateBlock* WorldEditor::mSetAxisSB = NULL;
GFXStateBlock* WorldEditor::mSetAxisTextSB = NULL;
GFXStateBlock* WorldEditor::mSetSceneSB = NULL;
GFXStateBlock* WorldEditor::mAlphaBlendFalseSB = NULL;
GFXStateBlock* WorldEditor::mZTrueSB = NULL;
GFXStateBlock* WorldEditor::mZFalseSB = NULL;

// 点击时不选择水和地形
static U32 onMouseDownMask = 0xFFFFFFFF & ~WaterObjectType & ~TerrainObjectType;
bool WorldEditor::mRenderStatic = true;
bool WorldEditor::mRenderNPC = true;
   ...
   const char * parseObjectFormat(SimObject * obj, const char * format)
   {
      ...
      for(U32 i = 0; i < len; i++)
      {
         if(format[i] == '$')
         {
            ...
            dStrncpy(token, sizeof(token), &format[i+1], (j - i - 1));
            token[j-i-1] = 0;
...
//------------------------------------------------------------------------------
bool b_sCheckmove = true;   //编辑器工具栏按下单选移动按钮，是否可移动。
...
void WorldEditor::Selection::updateCentroid()
{
   ...
   mBoxBounds.min.set(1e10, 1e10, 1e10);
   mBoxBounds.max.set(-1e10, -1e10, -1e10);
   ...
   for(U32 i = 0; i < mObjectList.size(); i++)
   {
      ...
      //
      const Box3F& bounds = ((SceneObject*)mObjectList[i])->getWorldBox();
      mBoxBounds.min.setMin(bounds.min);
      mBoxBounds.max.setMax(bounds.max);
   }

   mCentroid /= (F32)mObjectList.size();
   mBoxCentroid /= (F32)mObjectList.size();
}
...
void WorldEditor::Selection::setScale(const VectorF & scale)
{
   for(U32 i = 0; i < mObjectList.size(); i++)
   {
      ((SceneObject*)mObjectList[i])->setScale(scale);
   }
...
bool WorldEditor::pasteSelection(bool dropSel)
{
   ...
   {
	   //随机旋转 
	   if(mDropWithArbitraryRotation && mStickyObjectAction == ACTION_COPY)
	   {
		   EulerF rot(0.f, 0.f, 0.f);
		   rot.z = mRandF(0,M_2PI_F);
...
void WorldEditor::dropSelection(Selection & sel)
{
   ...
   switch(mDropType)
   {
      ...
      case DropAtOrigin:
      {
         if(mDropAtBounds)
         {
            const Point3F& boxCenter = mSelected.getBoxCentroid();
            const Box3F& bounds = getSelectionBounds();
      ...
      case DropAtScreenCenter:
      {
         ...
         if(collide(event, info))
            sel.offset(Point3F(info.pos - center));
         else
            sel.offset(Point3F(event.pos - center));
      ...
      case DropToGround:
      {
         dropToGround(sel, false, centroid, mDropAtBounds);
         break;
      }
   }
...
void WorldEditor::dropToGround(Selection & sel, bool hitOnly, const Point3F & centroid, bool useBottomBounds)
{
   if(!sel.size())
      return;
   ...
   if(useBottomBounds)
   {
      boxCenter = mSelected.getBoxCentroid();
      start = boxCenter;
   }
   else
   {
   ...
   if(hit)
   {
      ...
      {
         const Box3F& bounds = getSelectionBounds();
         ri.point.z += bounds.len_z() * 0.5f;
         center = boxCenter;
      }

      sel.offset(ri.point - center);
   }
   else if(!hitOnly)
...
bool WorldEditor::collide(const Gui3DMouseEvent & event, CollisionInfo & info,S32 collision_mask)
{
   ...
   bool hit;
   if(mBoundingBoxCollision)
      hit = gServerContainer.collideBox(startPnt, endPnt,collision_mask, &ri);
   else
      hit = gServerContainer.castRay(startPnt, endPnt, collision_mask, &ri);
...
#ifdef STATEBLOCK
	AssertFatal(mSetSelSB, "WorldEditor::renderSelectionWorldBox -- mSetSelSB cannot be NULL.");
	mSetSelSB->apply();
#else
...
#ifdef STATEBLOCK
	AssertFatal(mZTrueSB, "WorldEditor::renderSelectionWorldBox -- mZTrueSB cannot be NULL.");
	mZTrueSB->apply();
#else
	GFX->setZEnable( true );
#endif
...
{
#ifdef STATEBLOCK
	AssertFatal(mSetObjBoxSB, "WorldEditor::renderObjectBox -- mSetObjBoxSB cannot be NULL.");
	mSetObjBoxSB->apply();
#else
	GFX->setCullMode( GFXCullNone );
	GFX->setTextureStageColorOp( 0, GFXTOPDisable );
...
#ifdef STATEBLOCK
	AssertFatal(mZTrueSB, "WorldEditor::renderObjectBox -- mZTrueSB cannot be NULL.");
	mZTrueSB->apply();
#else
	GFX->setZEnable( true );
#endif
...
void WorldEditor::renderObjectFace(SceneObject * obj, const VectorF & normal, const ColorI & col)
{
...
#ifdef STATEBLOCK
	AssertFatal(mSetObjFaceSB, "WorldEditor::renderObjectFace -- mSetObjFaceSB cannot be NULL.");
	mSetObjFaceSB->apply();
#else
	GFX->setCullMode( GFXCullNone );
   ...
   PrimBuild::end();
#ifdef STATEBLOCK
	AssertFatal(mClearObjFaceSB, "WorldEditor::renderObjectFace -- mClearObjFaceSB cannot be NULL.");
	mClearObjFaceSB->apply();
#else
	GFX->setAlphaBlendEnable( false );
	GFX->setZEnable( true );
#endif
...
#ifdef STATEBLOCK
	AssertFatal(mSetPlaneSB, "WorldEditor::renderPlane -- mSetPlaneSB cannot be NULL.");
	mSetPlaneSB->apply();
#else
	GFX->setCullMode( GFXCullNone );
	GFX->setAlphaBlendEnable( true );
...
#ifdef STATEBLOCK
	AssertFatal(mAlphaBlendFalseSB, "WorldEditor::renderPlane -- mAlphaBlendFalseSB cannot be NULL.");
...
void WorldEditor::renderMousePopupInfo()
{
...
#ifdef STATEBLOCK
	AssertFatal(mZFalseSB, "WorldEditor::renderMousePopupInfo -- mZFalseSB cannot be NULL.");
	mZFalseSB->apply();
#else
   GFX->setZEnable( false ); // MDF++
#endif
   Point2I pos(mLastMouseEvent.mousePoint.x,
      mLastMouseEvent.mousePoint.y + 12 /*+ mCurrentCursor->getExtent().y - mCurrentCursor->getHotSpot().y*/);
   ...
   U32 width = mProfile->mFont->getStrWidth(( UTF8 *)buf);

   if(mRenderPopupBackground)
   {
#ifdef STATEBLOCK
		Point2I min;
		min.set(pos.x - width / 2 - 2, pos.y - 1);
		Point2I max;
		max.set(pos.x + width / 2 + 2, pos.y + mProfile->mFont->getHeight() + 1);
#else
      ...
      GFX->getDrawUtil()->drawRectFill(min, max, mPopupBackgroundColor);
   }

	GFX->getDrawUtil()->setBitmapModulation(mPopupTextColor);
   GFX->getDrawUtil()->drawText(mProfile->mFont, Point2I(pos.x - width / 2, pos.y), buf);
#ifdef STATEBLOCK
	AssertFatal(mZTrueSB, "WorldEditor::renderMousePopupInfo -- mZTrueSB cannot be NULL.");
	mZTrueSB->apply();
#else
   GFX->setZEnable( true );
#endif
...
void WorldEditor::renderSplinePath(Path *path)
{
...
#ifdef STATEBLOCK
			AssertFatal(mSetPathSB, "WorldEditor::renderSplinePath -- mSetPathSB cannot be NULL.");
			mSetPathSB->apply();
   ...
   GFX->setBaseRenderState();
#ifdef STATEBLOCK
	AssertFatal(mSetPathSB, "WorldEditor::renderSplinePath -- mSetPath2SB cannot be NULL.");
	mSetPathSB->apply();
	GFX->setVertexBuffer(vb);
#else
	GFX->setLightingEnable(false);
...
void WorldEditor::renderScreenObj(SceneObject * obj, Point2I sPos)
{
   ...
   if(mRenderObjHandle)
   {
      ...
      GFX->getDrawUtil()->drawBitmap(bitmap, sPos);
   }
   ...
   {
#ifdef STATEBLOCK
		AssertFatal(mZFalseSB, "WorldEditor::renderSplinePath -- mZFalseSB cannot be NULL.");
		mZFalseSB->apply();
#else
      GFX->setZEnable( false ); // MDF++
#endif
      ...
      Point2I extent(mProfile->mFont->getStrWidth(( UTF8 *)str), mProfile->mFont->getHeight());
      ...
      if(mRenderObjHandle)
      {
		...
		GFX->getDrawUtil()->setBitmapModulation(mObjectTextColor);
      GFX->getDrawUtil()->drawText(mProfile->mFont, pos, str);

#ifdef STATEBLOCK
		AssertFatal(mZTrueSB, "WorldEditor::renderSplinePath -- mZTrueSB cannot be NULL.");
		mZTrueSB->apply();
#else
...
void WorldEditor::calcAxisInfo()
{
   ...
   if(getDisplayType() == DisplayTypePerspective)
   {
   ...
   {
      // Handle orthographic display
      mAxisGizmoProjLen = mLastCameraQuery.fov * 0.2f;
   }
}
...
bool WorldEditor::collideAxisGizmo(const Gui3DMouseEvent & event)
{
   ...
   {
      F32 tipScale = mAxisGizmoProjLen * 0.1;
      Point3F sp(tipScale, tipScale, tipScale);

      Point3F min = mAxisGizmoCenter - sp;
      Point3F max = mAxisGizmoCenter + sp;
   ...
   // Find planar handle to use...
   for(U32 i = 0; i < 3; i++)
   {
      Point3F p1 = mAxisGizmoVector[mAxisGizmoPlanarVectors[i][0]];
      Point3F p2 = mAxisGizmoVector[mAxisGizmoPlanarVectors[i][1]];
      VectorF normal;
      mCross(p1, p2, &normal);
      ...
      p1 *= mAxisGizmoProjLen * 0.5f;
      p2 *= mAxisGizmoProjLen * 0.5f;

      Point3F poly [] = {
         Point3F(mAxisGizmoCenter + p1 + p2 * 0.75f),
         Point3F(mAxisGizmoCenter + p1 + p2),
         Point3F(mAxisGizmoCenter + p1 * 0.75f + p2),
         Point3F(mAxisGizmoCenter + (p1 + p2) * 0.75f)
      };
...
void WorldEditor::renderAxisGizmo()
{
   ...
   calcAxisInfo();
#ifdef STATEBLOCK
	AssertFatal(mSetAxisSB, "WorldEditor::renderAxisGizmo -- mSetAxisSB cannot be NULL.");
	mSetAxisSB->apply();
#else
	GFX->setCullMode( GFXCullNone );
	GFX->setZEnable( false );
   ...
   // Render the primary axis
   for(U32 i = 0; i < 3; i++)
   {
      PrimBuild::color( mAxisGizmoColors[i][i == mAxisGizmoSelAxis] );
   ...
   if((mSelected.size() == 1) /*&& !((mLastMouseEvent.modifier & SI_SHIFT) && (mCurrentMode == Move))*/)
   {
      MatrixF mat(extractEuler(mSelected[0]->getTransform()));
      mat.setPosition(mAxisGizmoCenter);
      GFX->multWorld(mat);
   }
   else
   {
   ...
   U8 mods = Input::getModifierKeys();
   if(mods & SI_ALT)
   {
      if(mods & SI_CTRL)
      {
         // Scale = Boxes
         renderAxisGizmoBoxes();
      }
      else
      {
         // Rotate = Circles
         renderAxisGizmoCircles();
      }
   }
   else
   {
      // Translate = Arrows
      renderAxisGizmoArrows();
   }
   GFX->popWorldMatrix();
   ...
   for(U32 i = 0; i < 3; i++)
   {
      midpnt[i] = mAxisGizmoVector[i] * mAxisGizmoProjLen * 0.5f;
   }
   ...
   for(U32 i = 0; i < 3; i++)
   {
      Point3F& p1 = midpnt[mAxisGizmoPlanarVectors[i][0]];
      Point3F& p2 = midpnt[mAxisGizmoPlanarVectors[i][1]];

      PrimBuild::color( mAxisGizmoColors[i+3][(i+3) == mAxisGizmoSelAxis] );
      PrimBuild::vertex3fv( centroid + p1 );
      PrimBuild::vertex3fv( centroid + p1 + p2 );
      PrimBuild::vertex3fv( centroid + p2 );
      PrimBuild::vertex3fv( centroid + p1 + p2 );
   }
   ...
   for(U32 i = 0; i < 3; i++)
   {
      ...
      PrimBuild::color( mAxisGizmoColors[i+3][(i+3) == mAxisGizmoSelAxis] );
      PrimBuild::vertex3fv( centroid + p1 + p2 * 0.75f );
      PrimBuild::vertex3fv( centroid + p1 + p2 );
      PrimBuild::vertex3fv( centroid + p1 * 0.75f + p2 );
   }

   PrimBuild::end();

#ifdef STATEBLOCK
	AssertFatal(mZTrueSB, "WorldEditor::renderAxisGizmo -- mZTrueSB cannot be NULL.");
	mZTrueSB->apply();
#else
   GFX->setZEnable( true );
#endif
...
void WorldEditor::renderAxisGizmoArrows()
{
   ...
   {
      PrimBuild::color( mAxisGizmoColors[axis][axis == mAxisGizmoSelAxis] );
...
void WorldEditor::renderAxisGizmoBoxes()
{
   F32 tipScale = mAxisGizmoProjLen * 0.1;
   F32 pos = mAxisGizmoProjLen - 0.5 * tipScale;
   ...
   for(U32 axis = 0; axis < 3; ++axis)
   {
      ...
      for(U32 j = 0; j < 6; j++)
      {
         ...
         PrimBuild::vertex3fv(CenterBoxPnts[BoxVerts[j][0]] * tipScale + axisVector[axis] * pos);
         PrimBuild::vertex3fv(CenterBoxPnts[BoxVerts[j][1]] * tipScale + axisVector[axis] * pos);
         PrimBuild::vertex3fv(CenterBoxPnts[BoxVerts[j][2]] * tipScale + axisVector[axis] * pos);
         PrimBuild::vertex3fv(CenterBoxPnts[BoxVerts[j][3]] * tipScale + axisVector[axis] * pos);

         PrimBuild::end();
      }
   }

   // Render the uniform scale handle
   if(mAxisGizmoSelAxis == 6)
   {
      PrimBuild::color( ColorI(255, 255, 255) );
   }
   else
   {
      PrimBuild::color( ColorI(192, 192, 192) );
   }
   ...
   for(U32 j = 0; j < 6; j++)
   {
      ...
      PrimBuild::vertex3fv(CenterBoxPnts[BoxVerts[j][0]] * tipScale);
      PrimBuild::vertex3fv(CenterBoxPnts[BoxVerts[j][1]] * tipScale);
      PrimBuild::vertex3fv(CenterBoxPnts[BoxVerts[j][2]] * tipScale);
      PrimBuild::vertex3fv(CenterBoxPnts[BoxVerts[j][3]] * tipScale);

      PrimBuild::end();
   }
}

void WorldEditor::renderAxisGizmoCircles()
{
   F32 tipScale = mAxisGizmoProjLen * 0.2;
   F32 pos = mAxisGizmoProjLen - 0.25 * tipScale;
   ...
   for(U32 axis = 0; axis < 3; ++axis)
   {
      PrimBuild::begin( GFXLineStrip, 9 );
      PrimBuild::color( mAxisGizmoColors[axis][axis == mAxisGizmoSelAxis] );
      ...
      for(U32 i = 0; i < 9; ++i)
      {
         Point3F& circlePnt = CirclePnts[i];
         pnt.set(circlePnt[x], circlePnt[y], circlePnt[z]);
         PrimBuild::vertex3fv(pnt * tipScale + axisVector[axis] * pos);
      }
...
#ifdef STATEBLOCK
	AssertFatal(mSetAxisTextSB, "WorldEditor::renderAxisGizmoText -- mSetAxisTextSB cannot be NULL.");
	mSetAxisTextSB->apply();
#else
	GFX->setZEnable( false ); // MDF++
	GFX->setAlphaBlendEnable( true );
	GFX->setSrcBlend( GFXBlendSrcAlpha );
   ...
   for(U32 i = 0; i < 3; i++)
   {
      ...
      if(project(pos, &sPos))
      {
         if( i == mAxisGizmoSelAxis )
         {
            ...
            GFX->getDrawUtil()->drawRectFill( Point2I((S32)sPos.x - 3, (S32)sPos.y + 2 ),
                               Point2I( (S32)sPos.x + 8, (S32)sPos.y + mProfile->mFont->getHeight() + 3 ),
                               fillColor );


            GFX->getDrawUtil()->drawRect( Point2I( (S32)sPos.x - 3, (S32)sPos.y + 2 ),
                           Point2I( (S32)sPos.x + 8, (S32)sPos.y + mProfile->mFont->getHeight() + 3 ),
                           textColor );
         }
         else
         ...
         GFX->getDrawUtil()->drawText(mProfile->mFont, Point2I((S32)sPos.x, (S32)sPos.y), buf);
      }
   }

#ifdef STATEBLOCK
	AssertFatal(mClearObjFaceSB, "WorldEditor::renderAxisGizmoText -- mClearObjFaceSB cannot be NULL.");
	mClearObjFaceSB->apply();
#else
	GFX->setAlphaBlendEnable( false );
	GFX->setZEnable( true ); // MDF++
...
//------------------------------------------------------------------------------
Signal<WorldEditor*> WorldEditor::smRenderSceneSignal;
...
WorldEditor::WorldEditor() : mCurrentCursor(PlatformCursorController::curArrow)
{
   ...
   mSnapToGrid = false;
   mAllowSnapRotations = true;
   mRotationSnap = 15.f;
   mAllowSnapScale = true;
   mScaleSnap = 0.1f;
   mSameScaleAllAxis = false;
   ...
   mStickyObjectAction = ACTION_NULL;
   mDropWithArbitraryRotation = false;

   mAxisGizmoColors[0][0].set(192, 0, 0);
   mAxisGizmoColors[0][1].set(255, 0, 0);
   mAxisGizmoColors[1][0].set(0, 192, 0);
   mAxisGizmoColors[1][1].set(0, 255, 0);
   mAxisGizmoColors[2][0].set(0, 0, 192);
   mAxisGizmoColors[2][1].set(0, 0, 255);
   mAxisGizmoColors[3][0].set(192, 192, 0); // XY
   mAxisGizmoColors[3][1].set(255, 255, 0);
   mAxisGizmoColors[4][0].set(192, 0, 192); // XZ
   mAxisGizmoColors[4][1].set(255, 0, 255);
   mAxisGizmoColors[5][0].set(0, 192, 192); // YZ
   mAxisGizmoColors[6][1].set(0, 255, 255);

   mAxisGizmoPlanarVectors[0][0] = 0; // XY
   mAxisGizmoPlanarVectors[0][1] = 1;
   mAxisGizmoPlanarVectors[1][0] = 0; // XZ
   mAxisGizmoPlanarVectors[1][1] = 2;
   mAxisGizmoPlanarVectors[2][0] = 1; // YZ
   mAxisGizmoPlanarVectors[2][1] = 2;

   mAxisGizmoPlanarRotAxis[0] = 2;
   mAxisGizmoPlanarRotAxis[1] = 1;
   mAxisGizmoPlanarRotAxis[2] = 0;
}
...
void WorldEditor::get3DCursor(GuiCursor *&cursor, bool &visible, const Gui3DMouseEvent &event)
{
   ...
   if(pRoot->mCursorChanged != mCurrentCursor)
   {
      ...
      pController->pushCursor(pController->getSysCursor(mCurrentCursor));
...
void WorldEditor::on3DMouseMove(const Gui3DMouseEvent & event)
{
   ...
   else
   {
      ...
      {
         setCursor(PlatformCursorController::curResizeAll);
         mHitInfo = info;
      }
   }
   ...
   if ((mStickyObjectAction == ACTION_MOVE && b_sCheckmove || mStickyObjectAction == ACTION_COPY)
	   && mSelected.size()>0)
   {
	   if (mSelected[0]->getType() & GameObjectType)
	   {
		   CollisionInfo  info;
		   if (collide(event,info,AtlasObjectType | TerrainObjectType | InteriorObjectType | StaticShapeObjectType
		   	   | StaticTSObjectType | WaterObjectType | VehicleObjectType | PhysicalZoneObjectType))
		   {
		   	if(mSelected[0]->getType() & TerrainObjectType)
		   		return;
		   	//锁定的物品禁止移动
		   	if (mSelected[0]->isLocked() && mStickyObjectAction == ACTION_MOVE)
		   		return;

		   	MatrixF mat = mSelected[0]->getTransform();
			mat.setPosition(info.pos);
		   	mSelected[0]->setTransform(mat);
		   	updateClientTransforms(mSelected);
		   }
	   }
	   else
	   {
		   CollisionInfo  info;
		   if (collide(event,info,TerrainObjectType))
		   {
			   if(mSelected[0]->getType() & TerrainObjectType)
				   return;
			   //锁定的物品禁止移动
...
void WorldEditor::on3DMouseDown(const Gui3DMouseEvent & event)
{
   ...
   mLastRotation = 0.f;
   mMouseScaleBuffer.set(0.0f, 0.0f, 0.0f);
   if(mSelected.size() == 1)
      mCurrentScaleBuffer = ((SceneObject*)mSelected[0])->getScale();
   ...
   if (mStickyObjectAction == ACTION_MOVE && mSelected.size()>0)
   {
	   b_sCheckmove = !b_sCheckmove;
   }
   else if (mStickyObjectAction == ACTION_COPY && mSelected.size()>0)
   {
	   if (copySelection(mSelected))
	   {
   ...
   if(collideAxisGizmo(event))
   {
      ...
      mHitRotation = extractEuler(mSelected[0]->getTransform());
      mHitObject = mSelected[0];
      mLastMousePlanarPos = mAxisGizmoSelPlanePoint;
   }
   else
   {
      CollisionInfo info;
	  // 去除地形碰撞
	  Vector<TerrainBlock*>& terList = gServerSceneGraph->getTerrainList();
	  for (S32 i=0; i<terList.size(); i++)
		  terList[i]->disableCollision();

      if(collide(event, info, onMouseDownMask) && !objClassIgnored(info.obj))
      {
         if(!mSelectionLocked)
         {
            if(event.modifier & SI_SHIFT)
            {
               ...
               if(mSelected.objInSet(info.obj))
               {
                  ...
                  Con::executef(this, "onUnSelect", avar("%d", info.obj->getId()));
               }
               else
               {
                  mSelected.addObject(info.obj);
                  Con::executef(this, "onSelect", avar("%d", info.obj->getId()));
               }
            }
            else
            {
...
void WorldEditor::on3DMouseDragged(const Gui3DMouseEvent & event)
{
   ...
   if(!mMouseDragged)
   {
      if(!mUsingAxisGizmo)
      {
         ...
         {
            if(!mSelectionLocked && mStickyObjectAction == ACTION_NULL)
   ...
   switch(mCurrentMode)
   {
      case Move:
      {
         ...
         if(mUsingAxisGizmo)
         {
            ...
            MatrixF mat;
            if(mSelected.size() > 1) // don't transform under these conditions. add extra conditions here if the apply!
               mat.identity();   
            else
               mat = mSelected[0]->getTransform();

            if(mAxisGizmoSelAxis < 3)
            {
               // Single axis handles
               F32 offset;
            ...
            {
               // Planar handles
               Point3F end = event.pos + event.vec * mProjectDistance;
   	         F32 t = mAxisGizmoSelPlane.intersect(event.pos, end);
               if(t >= 0 && t <= 1)
               {
                  Point3F pos;
                  pos.interpolate(event.pos, end, t);

                  Point3F offset = pos - mAxisGizmoSelPlanePoint;
                  ...
                  ((F32*)projPnt)[mAxisGizmoPlanarVectors[mAxisGizmoSelAxis-3][0]] += ((F32*)offset)[mAxisGizmoPlanarVectors[mAxisGizmoSelAxis-3][0]];
                  ((F32*)projPnt)[mAxisGizmoPlanarVectors[mAxisGizmoSelAxis-3][1]] += ((F32*)offset)[mAxisGizmoPlanarVectors[mAxisGizmoSelAxis-3][1]];
               }
            }

            // snap to the selected axis, if required
            ...
            {
               ((F32*)projPnt)[mAxisGizmoPlanarVectors[mAxisGizmoSelAxis-3][0]] = ((F32*)snap)[mAxisGizmoPlanarVectors[mAxisGizmoSelAxis-3][0]];
               ((F32*)projPnt)[mAxisGizmoPlanarVectors[mAxisGizmoSelAxis-3][1]] = ((F32*)snap)[mAxisGizmoPlanarVectors[mAxisGizmoSelAxis-3][1]];
            }
            ...
            if(mStickToGround)
            {
               dropToGround(mSelected, true, getSelectionCentroid(), mDropAtBounds);
            }
         ...
         else
         {
            ...
            if(mUseVertMove)
            {
               ...
               else
               {
                  ...
                  F32 diff = F32(mLastMouseEvent.mousePoint.x - event.mousePoint.x);
      ...
      case Scale:
      {
         setCursor(PlatformCursorController::curResizeHorz);
         ...
         if(mUsingAxisGizmo)
         {
            if(mAxisGizmoSelAxis < 3)
            {
               ...
               {
                  if(i == mAxisGizmoSelAxis)
                     mMouseScaleBuffer[i] += diff * mMouseScaleScale;

                  // clamp
                  if(mCurrentScaleBuffer[i] + mMouseScaleBuffer[i] < mMinScaleFactor)
                     mMouseScaleBuffer[i] = mMinScaleFactor - mCurrentScaleBuffer[i];
                  if(mCurrentScaleBuffer[i] + mMouseScaleBuffer[i] > mMaxScaleFactor)
                     mMouseScaleBuffer[i] = mMaxScaleFactor - mCurrentScaleBuffer[i];
               }
            }
            else if(mAxisGizmoSelAxis == 6)
            {
               // Uniform scale handle
               F32 diff = F32(mLastMouseEvent.mousePoint.x - event.mousePoint.x);

               // Check to make all axis match the largest one
               if(mSameScaleAllAxis)
               {
                  F32 large = mCurrentScaleBuffer[0];
                  if(mCurrentScaleBuffer[1] > large)
                     large = mCurrentScaleBuffer[1];
                  if(mCurrentScaleBuffer[2] > large)
                     large = mCurrentScaleBuffer[2];
                  for(S32 i = 0; i < 3; ++i)
                  {
                     mCurrentScaleBuffer[i] = large;
                  }
               }
            ...
            else
            {
               ...
               if(t >= 0 && t <= 1)
               {
                  ...
                  Point3F offset = pos - mLastMousePlanarPos;
                  ...
                  for(U32 i = 0; i < 2; i++)
                  {
                     S32 index = mAxisGizmoPlanarVectors[mAxisGizmoSelAxis-3][i];
                     mMouseScaleBuffer[index] += offset[index] * mMouseScaleScale;

                     // clamp
                     if(mCurrentScaleBuffer[index] + mMouseScaleBuffer[index] < mMinScaleFactor)
                        mMouseScaleBuffer[index] = mMinScaleFactor - mCurrentScaleBuffer[index];
                     if(mCurrentScaleBuffer[index] + mMouseScaleBuffer[index] > mMaxScaleFactor)
                        mMouseScaleBuffer[index] = mMaxScaleFactor - mCurrentScaleBuffer[index];
                  }

                  mLastMousePlanarPos = pos;
               }
            }

            Point3F scale = mCurrentScaleBuffer + mMouseScaleBuffer;
            if(mAllowSnapScale && event.modifier & SI_SHIFT)
            {
               scale.x = snapFloat(scale.x, mScaleSnap);
               scale.y = snapFloat(scale.y, mScaleSnap);
               scale.z = snapFloat(scale.z, mScaleSnap);
            }
      ...
      case Rotate:
      {
         ...
         if(mUsingAxisGizmo)
         {
            ...
            {
               axis = mAxisGizmoPlanarRotAxis[mAxisGizmoSelAxis-3];
            }
         }
         ...
         //
         if(mAllowSnapRotations && event.modifier & SI_SHIFT)
         {
...
#ifdef STATEBLOCK
	AssertFatal(mSetSceneSB, "WorldEditor::renderScene -- mSetSceneSB cannot be NULL.");
	mSetSceneSB->apply();
#else
	GFX->setCullMode( GFXCullNone );
	GFX->setTextureStageColorOp( 0, GFXTOPDisable );
...
void WorldEditor::initPersistFields()
{
   ...
   addField("allowSnapRotations", TypeBool, Offset(mAllowSnapRotations, WorldEditor));
   addField("rotationSnap", TypeF32, Offset(mRotationSnap, WorldEditor));
   addField("allowSnapScale", TypeBool, Offset(mAllowSnapScale, WorldEditor));
   addField("scaleSnap", TypeF32, Offset(mScaleSnap, WorldEditor));
   ...
   addField("StickyObjectAction",TypeS32,Offset(mStickyObjectAction,WorldEditor));
   addField("DropWithArbitraryRotation",TypeBool,Offset(mDropWithArbitraryRotation,WorldEditor));
   endGroup("Misc");

   Con::addVariable("$Editor::RenderStatic",   TypeBool,   &WorldEditor::mRenderStatic);
   Con::addVariable("$Editor::RenderNPC",      TypeBool,   &WorldEditor::mRenderNPC);
}
...
bool WorldEditor::alignByBounds(S32 boundsAxis)
{
   ...
   F32 pos;
   S32 baseObj = 0;
   const Box3F& bounds = mSelected[0]->getWorldBox();
   if(useMax)
      pos = bounds.max[axis];
   else
      pos = bounds.min[axis];
   ...
   {
      const Box3F& bounds = mSelected[i]->getWorldBox();

      if(useMax)
      {
         if(bounds.max[axis] > pos)
         {
            pos = bounds.max[axis];
            baseObj = i;
         }
   ...
   for(S32 i=0; i<mSelected.size(); ++i)
   {
      ...
      F32 delta;
      if(useMax)
         delta = pos - bounds.max[axis];
      else
         delta = pos - bounds.min[axis];

      Point3F objPos = mSelected[i]->getPosition();
      objPos[axis] += delta;
      mSelected[i]->setPosition(objPos);
   }
...
bool WorldEditor::alignByAxis(S32 axis)
{
   ...
   Point3F pos = mSelected[0]->getPosition();
...
void WorldEditor::resetSelectedRotation()
{
   ...
   {
      MatrixF mat(true);
      mat.setPosition(mSelected[i]->getPosition());
      mSelected[i]->setTransform(mat);
   }
}
...
void WorldEditor::resetSelectedScale()
{
   for(S32 i=0; i<mSelected.size(); ++i)
   {
      mSelected[i]->setScale(Point3F(1,1,1));
   }
}


void WorldEditor::resetStateBlock()
{
	//mSetSelSB
	GFX->beginStateBlock();
	...
	GFX->setTextureStageState( 0, GFXTSSColorOp, GFXTOPDisable );
	GFX->setRenderState(GFXRSZEnable, true);
	GFX->endStateBlock(mSetObjBoxSB);
	...
	GFX->setRenderState(GFXRSCullMode, GFXCullNone);
	GFX->setRenderState(GFXRSZEnable, true);
	GFX->endStateBlock(mSetObjFaceSB);

	//mClearObjFaceSB
	GFX->beginStateBlock();
	GFX->setRenderState(GFXRSZEnable, true);
	GFX->setRenderState(GFXRSAlphaBlendEnable, false);
	GFX->endStateBlock(mClearObjFaceSB);
	...
	GFX->setRenderState(GFXRSCullMode, GFXCullNone);
	GFX->setTextureStageState( 0, GFXTSSColorOp, GFXTOPDisable );
	GFX->endStateBlock(mSetPathSB);

	//mSetAxisSB
	GFX->beginStateBlock();
	GFX->setRenderState(GFXRSCullMode, GFXCullNone);
	GFX->setRenderState(GFXRSZEnable, false );
	GFX->endStateBlock(mSetAxisSB);
	...
	GFX->setRenderState(GFXRSDestBlend, GFXBlendInvSrcAlpha);
	GFX->setRenderState(GFXRSZEnable, false );
	GFX->endStateBlock(mSetAxisTextSB);
...
void WorldEditor::releaseStateBlock()
{
	if (mSetSelSB)
	{
		mSetSelSB->release();
	...
	if (mSetObjBoxSB)
	{
		mSetObjBoxSB->release();
	}

	if (mSetObjFaceSB)
	{
		mSetObjFaceSB->release();
	}

	if (mClearObjFaceSB)
	{
		mClearObjFaceSB->release();
	}
...
void WorldEditor::init()
{
	if (mSetSelSB == NULL)
	{
		...
		mSetObjBoxSB = new GFXD3D9StateBlock;
		mSetObjFaceSB = new GFXD3D9StateBlock;
		mClearObjFaceSB = new GFXD3D9StateBlock;
		mSetPlaneSB = new GFXD3D9StateBlock;
		mSetPathSB = new GFXD3D9StateBlock;
		//mSetPath2SB = new GFXD3D9StateBlock;
		mSetAxisSB = new GFXD3D9StateBlock;
		mSetAxisTextSB = new GFXD3D9StateBlock;
		mSetSceneSB = new GFXD3D9StateBlock;
...
void WorldEditor::shutdown()
{
	...
	SAFE_DELETE(mSetObjBoxSB);
	SAFE_DELETE(mSetObjFaceSB);
	SAFE_DELETE(mClearObjFaceSB);
	SAFE_DELETE(mSetPlaneSB);
	SAFE_DELETE(mSetPathSB);
	//SAFE_DELETE(mSetPath2SB);
	SAFE_DELETE(mSetAxisSB);
	SAFE_DELETE(mSetAxisTextSB);
	SAFE_DELETE(mSetSceneSB);
