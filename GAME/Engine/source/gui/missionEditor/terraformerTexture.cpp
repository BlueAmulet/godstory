...
bool Terraformer::setMaterials(const char *r_src, const char *materials )
{
	...
	for (int i = 0; i < terrains.size(); i++)
	{
		...
		char buffer[1024];
		dStrcpy(buffer, sizeof(buffer), r_src);
		char *context = NULL;
		char *str = dStrtok(buffer, " \0", &context);
		while (str)
		{
			src.push_back( getRegister(dAtoi(str)) );
			str = dStrtok(NULL, " \0", &context);
		}

		//--------------------------------------
		// extract the materials
		dStrcpy(buffer, sizeof(buffer), materials);
		str = dStrtok(buffer, " \0", &context);
		while (str)
		{
			S32 i;
      ...
      clientTerrBlock->updateGridMaterials(Point2I(0,0),Point2I(TerrainBlock::BlockSize,TerrainBlock::BlockSize));
...
bool Terraformer::mergeMasks(const char *r_src, U32 r_dst)
{
   ...
   dStrcpy(buffer, sizeof(buffer), r_src);
   char * context = NULL;
   char *reg = dStrtok(buffer, " \0", &context);
   while (reg)
   {
      src.push_back( getRegister(dAtoi(reg)) );
      reg = dStrtok(NULL, " \0", &context);
   }
