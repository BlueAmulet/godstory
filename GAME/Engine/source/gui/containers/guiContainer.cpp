...
bool GuiContainer::resize( const Point2I &newPosition, const Point2I &newExtent )
{
   ...
   // Iterate over all children and let them know we've been sized
   iterator nI = begin();
   for( ; nI != end(); nI++ )
   {
      // Sanity
      AssertFatal( dynamic_cast<GuiControl*>(*nI), "GuiContainer - Fatal, Cannot deal with non GuiControl children!");

      GuiControl *control = static_cast<GuiControl*>( *nI );
      control->parentResized( oldBounds, getBounds() );
