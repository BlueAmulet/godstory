...
void WinConsole::printf(const char *s, ...)
{
   ...
   va_list args;
   va_start(args, s);
   _vsnprintf_s(buffer, BufSize, BufSize, s, args);
      ...
      if(numEvents)
      {
         ...
         {			
            ...
            {				
			   ...
			   //Con::printf("VCode=%d, SCode=%d, uChar=%d, KeyState=%d", ke->wVirtualKeyCode, ke->wVirtualScanCode, ke->uChar, ke->dwControlKeyState);
               if(ke->bKeyDown)
               {
				   if((U16)ke->uChar.UnicodeChar > 255)
				   {
					   char szANSI[3];     
