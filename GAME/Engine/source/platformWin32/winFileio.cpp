...
bool dFileDelete(const char * name)
{
   ...
   TempAlloc< TCHAR > buf( length );

#ifdef UNICODE
   convertUTF8toUTF16( name, buf, buf.size );
#else
   dStrcpy( buf, length, name );
#endif
...
bool Platform::deleteDirFiles( const char* DirName, const char* filename )
{
	...
	S32 length = dStrlen( tempFileFind ) + 1;
	TempAlloc< TCHAR > buf( length );

#ifdef UNICODE
	convertUTF8toUTF16( tempFileFind, buf, buf.size );
#else
	dStrcpy( buf, length, tempFileFind );
#endif
	...
	{   
#ifdef UNICODE
		UTF8* str = convertUTF16toUTF8(FindFileData.cFileName);
		sprintf(tempFileFind,"%s/%s",DirName, str);
		delete[] str;
		str = NULL;
		S32 length = dStrlen( tempFileFind ) + 1;
		TempAlloc< TCHAR > buf( length );
		convertUTF8toUTF16( tempFileFind, buf, buf.size );
#else
		sprintf(tempFileFind,"%s/%s",DirName, FindFileData.cFileName);
		dStrcpy( buf, length, tempFileFind );
#endif
		backslash( buf );
		DeleteFile(buf);
		...
		{
			dMemset(tempFileFind, 0, MAX_PATH);

#ifdef UNICODE
			str = convertUTF16toUTF8(FindFileData.cFileName);
			sprintf(tempFileFind,"%s/%s",DirName, str);
			delete[] str;
			str = NULL;
			S32 length = dStrlen( tempFileFind ) + 1;
...
bool dFileRename(const char *oldName, const char *newName)
{
   ...
   S32 oldNameLen = dStrlen( oldName ) + 1;
   S32 newNameLen = dStrlen( newName ) + 1;
   TempAlloc< TCHAR > oldf( oldNameLen );
   TempAlloc< TCHAR > newf( newNameLen );

#ifdef UNICODE
   convertUTF8toUTF16( oldName, oldf, oldf.size );
   convertUTF8toUTF16( newName, newf, newf.size );
#else
   dStrcpy(oldf, oldNameLen, oldName);
   dStrcpy(newf, newNameLen, newName);
#endif
   backslash(oldf);
   backslash(newf);
...
bool dPathCopy(const char *fromName, const char *toName, bool nooverwrite)
{
   ...
   S32 fromLen = dStrlen( fromName ) + 1;
   S32 toLen = dStrlen( toName ) + 1;
   TempAlloc< TCHAR > from( fromLen );
   TempAlloc< TCHAR > to( toLen );

#ifdef UNICODE
   convertUTF8toUTF16( fromName, from, from.size );
   convertUTF8toUTF16( toName, to, to.size );
#else
   dStrcpy( from, fromLen, fromName );
   dStrcpy( to, toLen, toName );
#endif
   ...
   else if (Platform::isDirectory(fromName))
   {
	  ...
	  S32 tempBufLen1 = to.size * 3 + MAX_PATH * 3;
      TempAlloc< char > tempBuf( tempBufLen1 );
      ...
      for (S32 i = 0; i < directoryInfo.size(); i++)
      {
	   ...
	   S32 tempBufLen2 = from.size * 3 + MAX_PATH * 3;
      TempAlloc< char > tempBuf1( tempBufLen2 );
#ifdef UNICODE
      ...
      for (S32 i = 0; i < fileInfo.size(); i++)
      {
         ...
         dStrcat(toFile, tempBufLen1, "/");
         dStrcat(toFile, tempBufLen1, fileInfo[i].pFileName);
...
static bool recurseDumpPath(const char *path, const char *pattern, Vector<Platform::FileInfo> &fileVector, S32 recurseDepth )
{
   ...
   U32 length = lenFullPath + dStrlen( pattern ) + 2;
   TempAlloc< char > buf( length );
   dMemcpy( buf, fullPath, lenFullPath );
   buf[ lenFullPath ] = '/';
   dStrcpy( &buf[ lenFullPath + 1 ], length - lenFullPath - 1, pattern );
...
bool Platform::getFileTimes(const char *filePath, FileTime *createTime, FileTime *modifyTime)
{
   ...
   S32 length = dStrlen( filePath ) + 1;
   TempAlloc< TCHAR > fp( length );

#ifdef UNICODE
   convertUTF8toUTF16( filePath, fp, fp.size );
#else
   dStrcpy( fp, length, filePath );
#endif
...
{
   S32 length = dStrlen( newDir ) + 2;
   TempAlloc< TCHAR > buf( length );

#ifdef UNICODE
   convertUTF8toUTF16( newDir, buf, buf.size - 1 );
#else
   dStrcpy( buf, length, newDir );
#endif
...
bool Platform::isFile(const char *pFilePath)
{
   ...
   S32 length = dStrlen( pFilePath ) + 1;
   TempAlloc< TCHAR > buf( length );

#ifdef UNICODE
   convertUTF8toUTF16( pFilePath, buf, buf.size );
#else
   dStrcpy( buf, length, pFilePath );
#endif
   backslash( buf );
...
bool Platform::isDirectory(const char *pDirPath)
{
   ...
   S32 length = dStrlen( pDirPath ) + 1;
   TempAlloc< TCHAR > buf( length );

#ifdef UNICODE
   convertUTF8toUTF16( pDirPath, buf, buf.size );
#else
   dStrcpy( buf, length, pDirPath );
#endif
   backslash( buf );
...
bool Platform::isSubDirectory(const char *pParent, const char *pDir)
{
   ...
   S32 fileNameLen = dStrlen( fileName ) + 1;
   S32 dirLen = dStrlen( pDir ) + 1;
   TempAlloc< TCHAR > file( fileNameLen );
   TempAlloc< TCHAR > dir( dirLen );

#ifdef UNICODE
   convertUTF8toUTF16( fileName, file, file.size );
   convertUTF8toUTF16( pDir, dir, dir.size );
#else
   dStrcpy( file, fileNameLen, fileName );
   dStrcpy( dir, dirLen, pDir );
#endif
...
bool Platform::hasSubDirectory(const char *pPath)
{
   ...
   if( trail == '/' )
      dStrcpy( searchBuf, sizeof(searchBuf), pPath );
   else
      dSprintf(searchBuf, sizeof(searchBuf), "%s/*", pPath );
