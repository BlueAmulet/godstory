...
DInputDevice::DInputDevice( const DIDEVICEINSTANCE* dii )
{
   ...
   switch ( GET_DIDEVICE_TYPE( mDeviceInstance.dwDevType ) )
   {
      ...
      case DI8DEVTYPE_GAMEPAD: // enables joysticks (not XInput controllers)
...
bool DInputDevice::processImmediate()
{
   ...
   // (oldData = 0 prevents a crashing bug in PowerEngine. There is a case where 
   //  PowerEngine accessed oldData without it ever being set.)
...
const char* DInputDevice::getJoystickAxesString()
{
   ...
   for ( U32 i = 0; i < mObjCount; i++ )
   {
      ...
      {
         case SI_XAXIS:
            dStrcat( buf, sizeof(buf), "\tX" );
            break;
         case SI_YAXIS:
            dStrcat( buf, sizeof(buf), "\tY" );
            break;
         case SI_ZAXIS:
            dStrcat( buf, sizeof(buf), "\tZ" );
            break;
         case SI_RXAXIS:
            dStrcat( buf, sizeof(buf), "\tR" );
            break;
         case SI_RYAXIS:
            dStrcat( buf, sizeof(buf), "\tU" );
            break;
         case SI_RZAXIS:
            dStrcat( buf, sizeof(buf), "\tV" );
            break;
         case SI_SLIDER:
            dStrcat( buf, sizeof(buf), "\tS" );
            break;
      }
   }

   S32 length = dStrlen( buf ) + 1;
