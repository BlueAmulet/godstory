...
S32 Platform::messageBox(const UTF8 *title, const UTF8 *message, MBButtons buttons, MBIcons icon)
{
   ...
   // message box area
   bool cursorLocked = pWindow->isMouseLocked();
   if( cursorLocked )
      pWindow->setMouseLocked( false );

   // Need a visible cursor to click stuff accurately
   bool cursorVisible = pWindow->isCursorVisible();
   if( !cursorVisible )
   ...
   const UTF8 *t = title;
#endif
   S32 ret = ::MessageBox( static_cast<Win32Window*>(pWindow)->getHWND(), msg, t, getMaskFromID(sgButtonMap, buttons) | getMaskFromID(sgIconMap, icon));
