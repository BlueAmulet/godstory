...
bool expandToolScriptFilename(char *filename, U32 size, const char *src)
{
   ...
   switch(*ptr)
   {
      ...
      case '.':
         ...
         if(cbFullPath)
         {
            dStrcpy(varBuf, sizeof(varBuf), cbFullPath);
...
bool expandOldScriptFilename(char *filename, U32 size, const char *src)
{
   ...
   dStrncpy(filename, size, cbName, length);
   dStrcpy(filename+length, size - length, src+1);
   return true;
}
...
bool collapseScriptFilename(char *filename, U32 size, const char *src)
{
   ...
   for(S32 i = 0;!(test[i].path == NULL && test[i].replace == NULL);++i)
   {
      ...
      *filename = 0;
      ...
      dStrcat(filename, size, rel);
      return true;
   }

   dMemcpy(filename, src, size - 1);
   filename[size-1] = 0;
   return true;
}
