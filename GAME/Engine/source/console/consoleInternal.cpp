...
void Dictionary::addVariable(const char *name, S32 type, void *dataPtr)
{
   ...
   {
      scratchBuffer[0] = '$';
      dStrcpy(scratchBuffer + 1, sizeof(scratchBuffer) - 1, name);
      name = scratchBuffer;
   }
...
ConsoleFunction(backtrace, void, 1, 1, "Print the call stack.")
{
   ...
   for(U32 i = 0; i < gEvalState.stack.size(); i++)
   {
      ...
      if(gEvalState.stack[i]->scopeNamespace && gEvalState.stack[i]->scopeNamespace->mName)
      {
         dStrcat(buf, totalSize, gEvalState.stack[i]->scopeNamespace->mName);
         dStrcat(buf, totalSize, "::");
      }
      dStrcat(buf, totalSize, gEvalState.stack[i]->scopeName);
   }
...
void Namespace::addFunction(StringTableEntry name, CodeBlock *cb, S32 minArgs, S32 maxArgs,U32 functionOffset, const char* usage)
{
...
void Namespace::addOverload(const char * name, const char *altUsage)
{
   ...
   dSprintf(lilBuffer, sizeof(lilBuffer), "_%d", uid++);
   dStrcat(buffer, sizeof(buffer), lilBuffer);
...
void Namespace::addScriptCallback(const char *funcName, const char *usage)
{
   static U32 uid=0;
   char buffer[1024];
   char lilBuffer[32];
   dStrcpy(buffer, sizeof(buffer), funcName);
   dSprintf(lilBuffer, sizeof(lilBuffer), "_%d_cb", uid++);
   dStrcat(buffer, sizeof(buffer), lilBuffer);
