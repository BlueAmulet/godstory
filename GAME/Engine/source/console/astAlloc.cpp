...
/// @file
///
/// PowerEngineScript AST node allocators.
///
...
StrConstNode *StrConstNode::alloc(char *str, bool tag, bool doc)
{
   ...
   length = dStrlen(str) + 1;
   ret->str = (char *) consoleAlloc(length);
   ret->tag = tag;
   ret->doc = doc;
   dStrcpy(ret->str, length, str);
...
SlotAssignNode *SlotAssignNode::alloc(ExprNode *objectExpr, ExprNode *arrayExpr, StringTableEntry slotName, ExprNode *valueExpr, U32 typeID /* = -1 */)
{
