...
void CMDerror(char *format, ...)
{
   ...
   _vsnprintf_s( tempBuf, BUFMAX, BUFMAX, format, args );
#else
   vsnprintf( tempBuf, BUFMAX, BUFMAX, format, args );
#endif
   ...
   if(fileName)
   {
      ...
      if (i) dMemcpy(tempBuf,scanBuffer+scanIndex-i,i);
      dMemcpy(tempBuf+i,"##", 2); // bracketing.
      tempBuf[i+2] = scanBuffer[scanIndex]; // copy the halt character.
      dMemcpy(tempBuf+i+3,"##", 2); // bracketing.
      if (j) dMemcpy(tempBuf+i+5,scanBuffer+scanIndex+1,j); // +1 to go past current char.
...
static int Sc_ScanString(int ret)
{
   S32 length = dStrlen(CMDtext);
   CMDtext[CMDleng - 1] = 0;
   if(!collapseEscape(CMDtext+1))
      return -1;
   CMDlval.str = (char *) consoleAlloc(length);
   dStrcpy(CMDlval.str, length, CMDtext + 1);
   return(ret);
}
