...
void printClassHeader(const char* usage, const char * className, const char * superClassName, const bool stub)
{
   ...
   if( usage != NULL )
   {
      ...
      dStrcpy( usageStr, usageLen, usage );
      ...
      do 
      {
         ...
         dStrcpy( lineStr, sizeof(lineStr), usagePtr );
         ...
         if( tempkw )
         {
            dStrncpy( keyword, sizeof(keyword), tempkw + 1, 5 );
            keyword[5] = '\0';
         }
   ...
   for(Entry *ewalk = g->mEntryList; ewalk; ewalk = ewalk->mNext)
   {
      ...
      if(eType >= Entry::ScriptFunctionType || eType == Entry::OverloadMarker)
      {
         ...
         if(use[0] == '(')
         {
            ...
            U32 len = end - use;
            dStrncpy(buffer, sizeof(buffer), use, len);
            buffer[len] = 0;
         ...
         if(dot < bgn && bgn < end)  // And they're in the order dot, bgn, end...
         {
            use++;
            U32 len = end - bgn - 1;
            dStrncpy(buffer, sizeof(buffer), bgn+1, len);
            buffer[len] = 0;
      ...
      else if(ewalk->mType == Entry::ScriptCallbackType)
      {
         ...
         // Grab the signature from the usage.
         char *splitPt = (char*)dStrchr(ewalk->mUsage, '-');

         Con::printf("      /*! %s */", splitPt + 2);

         char signature[1024];
         dMemset(signature, 0, 1024);
         dStrncpy(signature, sizeof(signature), ewalk->mUsage, splitPt - ewalk->mUsage - 1);
         Con::printf("      %s;", signature);
         Con::printf("");
   ...
   for(i = 0; i < vec.size(); i++)
   {
      ...
      if( dumpScript )
      {
         ...
         while( field )
         {
            ...
            // Copy.
            dStrncpy( fieldDoc, sizeof(fieldDoc), field, docLen );
            fieldDoc[docLen] = '\0';
            field += docLen;
