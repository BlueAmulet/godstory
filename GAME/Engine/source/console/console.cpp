...
void init()
{
   ...
   // Set up the parser(s)
   CON_ADD_PARSER(CMD,   "cs",   true);   // PowerEngineScript
...
{
#ifdef POWER_MULTITHREAD
   return ThreadManager::compare( ThreadManager::getCurrentThreadId(), gMainThreadID );
   // This was changed due to mac platform implementation of threads. See notes in mac
   // thread manager. -patw
   //return (ThreadManager::getCurrentThreadId() == gMainThreadID);
#else
...
U32 tabComplete(char* inputBuffer, U32 cursorPos, U32 maxResultLength, bool forwardTab)
{
   ...
   if (dStrcmp(tabBuffer, inputBuffer)) 
   {
      ...
      dStrcpy(tabBuffer, sizeof(tabBuffer), inputBuffer);
      ...
      if (inputBuffer[p - 1] == '.') 
      {
         ...
         dMemcpy(completionBuffer, inputBuffer + p, objLast - p);
   ...
   if (newText) 
   {
      ...
      dMemcpy(inputBuffer + completionBaseStart, newText, len);
   ...
   if(logBufferEnabled || consoleLogMode)
   {
      ...
      for(;;)
      {
         ...
         if(logBufferEnabled && !consoleLogLocked)
         {
            ...
            entry.mLevel  = level;
            entry.mType   = type;
#ifndef POWER_SHIPPING // this is equivalent to a memory leak, turn it off in ship build
			length = dStrlen(pos) + 1;
            entry.mString = (const char *)consoleLogChunker.alloc(length);
            dStrcpy(const_cast<char*>(entry.mString), length, pos);
...
void warnf(ConsoleLogEntry::Type type, const char* fmt,...)
{
	...
	csbi.wAttributes |= FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_INTENSITY; 
...
const char *getVariable(const char *name)
{
   ...
   if(name[0] != '$' && dStrchr(name, '.') && !isFunction(name))
   {
	  ...
	  char *context = NULL;
      char * token = dStrtok(scratchBuffer, ".", &context);
      SimObject * obj = Sim::findObject(token);
      if(!obj)
         return("");

      token = dStrtok(0, ".\0", &context);
      if(!token)
         return("");
...
StringTableEntry getModNameFromPath(const char *path)
{
   ...
   if(path[0] == '/' || path[1] == ':')
   {
      ...
      if(dStrnicmp(exePath, path, len) == 0)
      {
         ...
         if(slash)
         {
            dStrncpy(buf, sizeof(buf), ptr, slash - ptr);
            buf[slash - ptr] = 0;
         }
         else
            return NULL;
   ...
   else
   {
      ...
      if(slash)
      {
         dStrncpy(buf, sizeof(buf), path, slash - path);
         buf[slash - path] = 0;
      }
      else
         return NULL;
