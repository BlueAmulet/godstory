...
namespace Con
{
   ...
   {
      S32 length = dStrlen( stringToCopy ) + 1;
      char *ret = STR.getReturnBuffer( length );
      dStrcpy( ret, length, stringToCopy );
      ret[length - 1] = '\0';
      return ret;
   }
...
void CodeBlock::getFunctionArgs(char buffer[1024], U32 ip)
{
   ...
   for(U32 i = 0; i < fnArgc; i++)
   {
      ...
      dStrcat(buffer, 1024, "var ");

      // Try to capture junked parameters
      if(var[0])
        dStrcat(buffer, 1024, var+1);
      else
        dStrcat(buffer, 1024, "JUNK");
   }
}
...
const char *CodeBlock::exec(U32 ip, const char *functionName, Namespace *thisNamespace, U32 argc, const char **argv, bool noCalls, StringTableEntry packageName, S32 setFrame)
{
   ...
   if(argv)
   {
      ...
      {
         traceBuffer[0] = 0;
         dStrcat(traceBuffer, sizeof(traceBuffer), "Entering ");
         if(packageName)
         {
            dStrcat(traceBuffer, sizeof(traceBuffer), "[");
            dStrcat(traceBuffer, sizeof(traceBuffer), packageName);
            dStrcat(traceBuffer, sizeof(traceBuffer), "]");
         }
         ...
         for(i = 0; i < argc; i++)
         {
            dStrcat(traceBuffer, sizeof(traceBuffer), argv[i+1]);
            if(i != argc - 1)
               dStrcat(traceBuffer, sizeof(traceBuffer), ", ");
         }
         dStrcat(traceBuffer, sizeof(traceBuffer), ")");
      ...
      switch(instruction)
      {
         case OP_FUNC_DECL:
            if(!noCalls)
            {
			   ...
			   int argc		= code[ip + 5] + 1;
               
               Namespace::unlinkPackages();
               ns = Namespace::find(fnNamespace, fnPackage);
               ns->addFunction(fnName, this, argc,argc,hasBody ? ip : 0, curFNDocBlock ? dStrdup( curFNDocBlock ) : NULL );// if no body, set the IP to 0
               if( curNSDocBlock )
               {
         ...
         case OP_ADD_OBJECT:
         {
            ...
            U32 groupAddId = intStack[UINT_INDEX];
            SimGroup *grp = NULL;
            SimSet   *set = NULL;
            ...
            if(placeAtRoot) 
               intStack[UINT_INDEX] = currentNewObject->getId();
            else
               intStack[++UINT_INDEX] = currentNewObject->getId();
         ...
         case OP_RETURN:
            goto execFinished;
         case OP_CMPEQ:
            intStack[UINT_INDEX+1] = bool(floatStack[FLT] == floatStack[FLT-1]);
            UINT_INDEX++;
            FLT -= 2;
            break;
         ...
         case OP_XOR:
            intStack[UINT_INDEX-1] = intStack[UINT_INDEX] ^ intStack[UINT_INDEX-1];
            UINT_INDEX--;
            break;

         case OP_MOD:
            if(  intStack[UINT_INDEX-1] != 0 )
               intStack[UINT_INDEX-1] = intStack[UINT_INDEX] % intStack[UINT_INDEX-1];
            else
               intStack[UINT_INDEX-1] = 0;
            UINT_INDEX--;
            break;
         ...
         case OP_NOT:
            intStack[UINT_INDEX] = !intStack[UINT_INDEX];
            break;

         case OP_NOTF:
            intStack[UINT_INDEX+1] = !floatStack[FLT];
            FLT--;
            UINT_INDEX++;
            break;
         ...
         case OP_LOADVAR_UINT:
            intStack[UINT_INDEX+1] = gEvalState.getIntVariable();
            UINT_INDEX++;
            break;
         ...
         case OP_SAVEVAR_UINT:
            gEvalState.setIntVariable(intStack[UINT_INDEX]);
            break;
         ...
         case OP_SETCUROBJECT_INTERNAL:
            ...
            if(curObject)
            {
               ...
               if(set)
               {
                  ...
                  intStack[UINT_INDEX+1] = obj ? obj->getId() : 0;
                  UINT_INDEX++;
               }
               else
               {
         ...
         case OP_SETCURFIELD_ARRAY:
            dStrcpy(curFieldArray, sizeof(curFieldArray), STR.getStringValue());
            break;
         ...
         case OP_LOADFIELD_UINT:
            if(curObject)
               intStack[UINT_INDEX+1] = U32(dAtoi(curObject->getDataField(curField, curFieldArray)));
            else
               intStack[UINT_INDEX+1] = 0;
            UINT_INDEX++;
            break;
         ...
         case OP_SAVEFIELD_UINT:
            STR.setIntValue(intStack[UINT_INDEX]);
            if(curObject)
         ...
         case OP_STR_TO_UINT:
            intStack[UINT_INDEX+1] = STR.getIntValue();
            UINT_INDEX++;
            break;
         ...
         case OP_FLT_TO_UINT:
            intStack[UINT_INDEX+1] = (S64)floatStack[FLT];
            FLT--;
            UINT_INDEX++;
            break;
         ...
         case OP_UINT_TO_FLT:
            floatStack[FLT+1] = (F32)intStack[UINT_INDEX];
            UINT_INDEX--;
            FLT++;
            break;
            ...
            if(nsEntry->mType == Namespace::Entry::ScriptFunctionType)
            {
               ...
               if(nsEntry->mFunctionOffset)
			   {
				   if((nsEntry->mMinArgs && S32(callArgc) < nsEntry->mMinArgs) || (nsEntry->mMaxArgs && S32(callArgc) > nsEntry->mMaxArgs))
				   {
					   ...
					   Con::errorf(ConsoleLogEntry::Script, "%s: %s::%s - wrong number of arguments.", getFileLine(ip-4), nsName, fnName);
				   }
               ...
               else
               {
                  switch(nsEntry->mType)
                  {
                     ...
                     case Namespace::Entry::FloatCallbackType:
                     {
                        ...
                        {
                           ip++;
                           intStack[++UINT_INDEX] = (S64)result;
                           break;
                        }
                        else if(code[ip] == OP_STR_TO_FLT)
                        {
         ...
         case OP_COMPARE_STR:
            intStack[++UINT_INDEX] = STR.compare();
            break;
         case OP_PUSH:
            STR.push();
            break;
...
execFinished:
   ...
   if(argv)
   {
      ...
      {
         traceBuffer[0] = 0;
         dStrcat(traceBuffer, sizeof(traceBuffer), "Leaving ");
