...
SimDataBlockGroup *gUserDataBlockGroup;
SimDataBlockGroup *getUserDataBlockGroup()
{
	return gUserDataBlockGroup;
}
...
void init()
{
   ...
   gUserDataBlockGroup = new SimDataBlockGroup();
   gUserDataBlockGroup->registerObject("UserDataBlockGroup");
   gRootGroup->addObject(gUserDataBlockGroup);
}
...
//#ifdef NTJ_RELEASE
//   // <Edit> [3/5/2009 joy] 注册失败需要查错
//   AssertRelease(ret,"Object onAdd field!");
//#endif
...
//---------------------------------------------------------------------------
void SimObject::removeObjectFromScene()
{
	OLD_DO_LOCK( &SimLock ); // 增加多线程支持
...
void SimObject::assignName(const char *name)
{
   ...
   if(!(getType() & StaticTSObjectType))			//Ray: 为防止静态物件同名，禁止为静态物件取名
   {
	   char mat = name[0];
	   if(mat)
	   {
		   ...
		   {
			   char cName[64];
			   dSprintf(cName,64,"__%s",name);      //Ray: 为了防止数字开头的名字无法用做变量，前面增加了__
			   newName = StringTable->insert(cName);
		   }
		   else
