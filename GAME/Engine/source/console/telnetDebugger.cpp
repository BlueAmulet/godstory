...
TelnetDebugger::Breakpoint **TelnetDebugger::findBreakpoint(StringTableEntry fileName, S32 lineNumber)
{
   ...
   while((cur = *walk) != NULL)
   {
      ...
      // insensitive... PowerEngine needs a dFilenameCmp() function.
...
//      return;
   ...
   dStrncpy(mDebuggerPassword, sizeof(mDebuggerPassword), password, PasswordMaxLength);
   ...
   for(S32 i = (S32) gEvalState.stack.size() - 1; i >= last; i--)
   {
      ...
      if ( ns ) {
         ...
         if ( ns->mParent && ns->mParent->mPackage && ns->mParent->mPackage[0] ) {
            dStrcat( scope, sizeof(scope), ns->mParent->mPackage );
            dStrcat( scope, sizeof(scope), "::" );
         }
         if ( ns->mName && ns->mName[0] ) {
            dStrcat( scope, sizeof(scope), ns->mName );
            dStrcat( scope, sizeof(scope), "::" );
         }
      }
      ...
      if ((!function) || (!function[0]))
         function = "<none>";
      dStrcat( scope, sizeof(scope), function );
