   ...
   LPVOID lpContext )
{
   ...
   char name[ 256 ];
   char desc[ 256 ];
   convertUTF16toUTF8( lpszDrvName, name, sizeof( name ) );
   convertUTF16toUTF8( lpszDesc, desc, sizeof( desc ) );
#else
   char* name = lpszDrvName;
   char* desc = lpszDesc;
#endif
   provider->addDeviceDesc( lpGUID, name, desc );
   return TRUE;
}
...
void SFXDSProvider::addDeviceDesc( GUID* guid, const char* name, const char* desc )
{
   ...
   dStrncpy( info->name, sizeof(info->name), desc, sizeof( info->name ) );
   dStrncpy( info->driver, sizeof(info->driver), name, sizeof( info->driver ) );
