...

{
	...
	setCone( mDegToRad( (F32)desc->mConeInsideAngle ),
		mDegToRad( (F32)desc->mConeOutsideAngle ),
...
bool SFXSource::_allocVoice( SFXDevice* device )
{
   ...
   if ( !buffer )
      return false;
	SFXDescription* desc = mProfile->getDescription();
	
   // Ask the device for a voice based on this buffer.
   mVoice = device->createVoice( mIs3D, buffer, desc->mPriority );
   if ( !mVoice )
      return false;
	
   setVolume( mVolume );
   // MP3文件的播放不用设置它的频率
   if(mProfile->getResource()->getfiletype() != 2)
   {
	 setPitch( mPitch );
   }
...
void SFXSource::_freeVoice( SFXDevice* device )
{
   ...
   AssertFatal( device, "SFXSource::_freeVoice() - Device was null!" );
...
void SFXSource::_updateVolume( const MatrixF& listener )
{
   ...
   if ( !mIs3D ) 
   {
	 ...
	 if(SFX->mStartTime > 0)
	 {
		SFX->controlSoundVolume(currentTime);
	 }
      return;
	}
   ...
   // If the min distance is zero or we're 
   // outside the maximum distance then force
   // the attenuated volume to zero.
   if ( isEqual( mMinDistance, 0.0f ) || mDistToListener > mMaxDistance )
   {
      mAttenuatedVolume = 0.0f;
      return;
   }
...
bool SFXSource::_setStatus( SFXStatus status )
{
   ...
   if ( mStatusCallback && mStatusCallback[0] )
      Con::executef( mStatusCallback, statusString );
...
SFXStatus SFXSource::_updateStatus()
{
   ...
   if ( mLoopcount !=0 || mStatus != SFXStatusPlaying )
      return mStatus;
...
void SFXSource::setCone(   F32 innerAngle,
                           ...
                           F32 outerVolume )
{
   mConeInsideAngle     = mClampF( innerAngle, 0.0f, M_2PI_F );
   mConeOutsideAngle    = mClampF( outerAngle, mConeInsideAngle, M_2PI_F );
...
ConsoleMethod( SFXSource, play, void, 3, 3,  "SFXSource.play(soundID)\n"
...
ConsoleMethod( SFXSource, stop, void, 3, 3,  "SFXSource.stop(soundID)\n"
