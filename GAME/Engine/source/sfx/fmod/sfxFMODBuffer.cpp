...
U32 SFXFMODBuffer::mSoundChannel = 0;
StringTableEntry SFXFMODBuffer::mSoundID = NULL;
SFXFMODBuffer* SFXFMODBuffer::create( SFXProfile *profile )
{
  ...
  const SFXDescription *desc = profile->getDescription();
   ...
   mSoundChannel = profile->getSoundChannel();
   mSoundID = profile->getSoundID();
   //<Editor>:iceRain 
   //MP3 create
   if(res->getfiletype() == 2)
   {
	   
	   FMOD_CREATESOUNDEXINFO exinfomation;
	   memset( &exinfomation, 0, sizeof(FMOD_CREATESOUNDEXINFO) );
	   exinfomation.cbsize = sizeof(FMOD_CREATESOUNDEXINFO);
	   exinfomation.length = res->getSize();
	   exinfomation.numchannels       = 2;

	   // create new Sound
	   FMOD_MODE fMode = FMOD_HARDWARE | FMOD_OPENMEMORY | ( desc->mIs3D ? FMOD_3D : FMOD_2D );

	   const char *nameOrData = res->mSourceResource->getFullPath();
	   FMOD_RESULT result = FMOD_OK;
	   result = SFXFMODDevice::smFunc->FMOD_System_CreateSound(
		   SFXFMODDevice::smSystem, 
		   (const char*)res->getData(), 
		   fMode, 
		   &exinfomation, 
		   &sound );
	   ...
	   //MP3不需要拷贝，拷贝后会出现磁磁的声音
	   //buffer->_fillBuffer(true);
	  return buffer;
   }
   // ogg和wav文件
   else
   {
		...
		//{
			...
			fMode |= FMOD_OPENONLY; // this tells fmod we are supplying the data directly

			    
			U32 frequency = res->getFrequency();
			U32 bitsPerChannel = res->getSampleBits() / channels;
		...
		*/
        ...
        //修改从文件流里读取，这样可以兼容文件系统
		/*nameOrData = res->mSourceResource->getFullPath();
       ...
       Stream* pStream = ResourceManager->openStream(nameOrData);
       ...
       FModAssert( SFXFMODDevice::smFunc->FMOD_System_CreateSound(
           SFXFMODDevice::smSystem, 
           (const char*)pMemStream->getBuffer(), 
           fMode | FMOD_OPENMEMORY, 
           pCreatesoundexinfo, 
           &sound ), "SFXFMODDevice::createBuffer - Failed to alloc sound!");
