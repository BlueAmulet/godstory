...
void Win32WindowManager::getWindows(Vector<PlatformWindow*> &windows)
{
...
PlatformWindow *Win32WindowManager::createWindow(GFXDevice *device, const GFXVideoMode &mode)
{
   ...
   w32w->mDeviceId = mDeviceIdSource++;
   ...
   // <Edit> [4/20/2009 joy] 先于SetMode执行是因为SetMode的时候要用到
...
#ifdef NTJ_CLIENT
    ShowWindow( w32w->mWindowHandle, SW_HIDE);
#else
...
//          }
		 ...
		 // 注意：目前发现QQ输入法切换窗口时，获取window指针不是CreateWindow时的指针，
		 //因此引发调用异常
...
PlatformWindow * Win32WindowManager::getWindowById( DeviceId id )
{
   // Walk the list and find the matching id, if any.
   Win32Window *win = mWindowListHead;
   while(win)
   {
      if(win->getDeviceId() == id)
         return win;
