...
IMPLEMENT_CO_CLIENTEVENT_V1(SyncResCRCEvent);
...
void SyncResCRCEvent::pack(NetConnection *, BitStream * bstream)
{
	S8 TerrSize = mTerrCrc.size();
	bstream->write(TerrSize);
	for(int i=0; i<TerrSize; ++i)
		bstream->write(mTerrCrc[i]);
	bstream->write(mMisCrc);
	bstream->writeString(mMisFile);
}

void SyncResCRCEvent::unpack(NetConnection *, BitStream * bstream)
{
	S8 TerrSize = 0;
	U32 TempCrc;
	bstream->read(&TerrSize);
	for(int i=0; i<TerrSize; ++i)
	{
		bstream->read(&TempCrc);
		mTerrCrc.push_back(TempCrc);
	}
	bstream->read(&mMisCrc);
	mMisFile = bstream->readSTString();
}
