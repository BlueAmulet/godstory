...
//////#define STATEBLOCK
//sun
GFXStateBlock* fxSunLight::mLocalSunSB = NULL;
GFXStateBlock* fxSunLight::mSunSB = NULL;
//blend parameter
GFXStateBlock* fxSunLight::mZeroSB = NULL;//add alpha
GFXStateBlock* fxSunLight::mOneSB = NULL;//lerp alpha
GFXStateBlock* fxSunLight::mTwoSB = NULL;//add
//clear
GFXStateBlock* fxSunLight::mClearSB = NULL;
...
void fxSunLight::InitialiseAnimation(void)
{
	...
	mRemoteFlareTextureHandle	= NULL;
	// Flare Texture Names.
	mLocalFlareTextureName		= StringTable->insert("gameres/data/environments/lighting/corona");
	mRemoteFlareTextureName		= StringTable->insert("gameres/data/environments/lighting/corona");
...
bool fxSunLight::onAdd()
{
	...
	// Set Global Bounds so this is always renderable
	setGlobalBounds();
	// Reset the World Box.
	resetWorldBox();
								...
								const bool modifyBaseZoneState)
{
   ...
   PROFILE_START(fxSunLight_prepRenderImage);
   ...
   PROFILE_END(fxSunLight_prepRenderImage);
   return false;
}
...
void fxSunLight::renderObject( SceneState* state, RenderInst *ri )
{
	...
	if(!conn)
		return;
	GameBase* ControlObj = dynamic_cast<GameBase*>(conn->getControlObject());

	// Fetch Eye Position.
	F32 transFlag = 0.0f;   // This is used in getCameraTransform to get the eye pos instead of a translated camera position.
	Point3F	eyePos;
	MatrixF	eye;

	ControlObj->getCameraTransform(&transFlag, &eye );
	eye.getColumn(3, &eyePos);
	...
	if (LocalSun)
	{
		AssertFatal(mLocalSunSB, "fxSunLight::renderObject -- mLocalSunSB cannot be NULL.");
		mLocalSunSB->apply();
	}
	else
	{
		AssertFatal(mSunSB, "fxSunLight::renderObject -- mSunSB cannot be NULL.");
		mSunSB->apply();
	}
	...
	{
	case 0:
		AssertFatal(mZeroSB, "fxSunLight::renderObject -- mZeroSB cannot be NULL.");
		mZeroSB->apply();
		break;
	case 1:
		AssertFatal(mOneSB, "fxSunLight::renderObject -- mOneSB cannot be NULL.");
		mOneSB->apply();
		break;
	case 2:
		AssertFatal(mTwoSB, "fxSunLight::renderObject -- mTwoSB cannot be NULL.");
		mTwoSB->apply();
		break;
	...
	// Exit
	AssertFatal(mClearSB, "fxSunLight::renderObject -- mClearSB cannot be NULL.");
	mClearSB->apply();
	GFX->setBaseRenderState();
}
...
bool fxSunLight::TestLOS(const Point3F& ObjectPosition)
{
   ...
   if(out.z < 0.0 || out.z > 1.0 || out.x > viewport.extent.x || out.x < 0  || out.y > viewport.extent.y || out.y < 0)
	...
	// Get the Control Object.
	ShapeBase* ControlObj = dynamic_cast<ShapeBase*>(conn->getControlObject());
...
U64 fxSunLight::packUpdate(NetConnection * con, U64 mask, BitStream * stream)
{
	// Pack Parent.
	...
	if (stream->writeFlag(mask & fxSunLightConfigChangeMask))
	{
      ...
      stream->writeFlag(mLockToRealSun);
...
void fxSunLight::unpackUpdate(NetConnection * con, BitStream * stream)
{
	...
	if(stream->readFlag())
	{
      ...
      mLockToRealSun       = stream->readFlag();
...
void fxSunLight::resetStateBlock()
{
...
void fxSunLight::releaseStateBlock()
{
	if (mLocalSunSB)
	{
		mLocalSunSB->release();
...
void fxSunLight::init()
{
	if (mLocalSunSB == NULL)
	{
		mLocalSunSB = new GFXD3D9StateBlock;
		mLocalSunSB->registerResourceWithDevice(GFX);
		mLocalSunSB->mZombify = &releaseStateBlock;
		mLocalSunSB->mResurrect = &resetStateBlock;

		mSunSB = new GFXD3D9StateBlock;
		mZeroSB = new GFXD3D9StateBlock;
		mOneSB = new GFXD3D9StateBlock;
		mTwoSB = new GFXD3D9StateBlock;
...
void fxSunLight::shutdown()
{
	SAFE_DELETE(mLocalSunSB);
	SAFE_DELETE(mSunSB);
	SAFE_DELETE(mZeroSB);
	SAFE_DELETE(mOneSB);
	SAFE_DELETE(mTwoSB);
