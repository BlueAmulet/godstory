...
ConsoleFunction( calcExplosionCoverage, F32, 4, 4, "(Point3F source, SceneObject originator, bitset coverageMask)")
{
   ...
   if (sceneObject->getContainer()->castRay(pos, center, covMask, &rayInfo, sceneObject->getLayerID() ) == true) {
      // Try casting up and then out
      if (sceneObject->getContainer()->castRay(pos, pos + Point3F(0.0f, 0.0f, 1.0f), covMask, &rayInfo, sceneObject->getLayerID() ) == false)
      {
         if (sceneObject->getContainer()->castRay(pos + Point3F(0.0f, 0.0f, 1.0f), center, covMask, &rayInfo, sceneObject->getLayerID() ) == false)
         {
...
//   addField("shockwaveOnTerrain", TypeBool,                 Offset(shockwaveOnTerrain, ExplosionData));
   ...
   addField("shakeId",           TypeS32,       Offset(shakeId,            ExplosionData));
