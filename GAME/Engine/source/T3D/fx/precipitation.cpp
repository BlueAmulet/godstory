...
//---------------------------------------------------------------------------
//////#define STATEBLOCK
GFXStateBlock* Precipitation::mSetSB = NULL;
GFXStateBlock* Precipitation::mSetFixSB = NULL;
GFXStateBlock* Precipitation::mClearSB = NULL;
...
void Precipitation::initMaterials()
{
   ...
   if( dStrlen(pd->mDropName) > 0 && !mDropHandle.set(pd->mDropName, &GFXDefaultStaticDiffuseProfile, true) )
   ...
   if( dStrlen(pd->mSplashName) > 0 && !mSplashHandle.set(pd->mSplashName, &GFXDefaultStaticDiffuseProfile, true) )
...
U64 Precipitation::packUpdate(NetConnection* con, U64 mask, BitStream* stream)
{
...
void Precipitation::unpackUpdate(NetConnection* con, BitStream* stream)
{
   ...
   if (stream->readFlag())
   {
      ...
      mDropHitMask = dropHitMask | 
         ( mDropHitPlayers ? GameObjectType : 0 ) | 
...
void Precipitation::findDropCutoff(Raindrop *drop, const Box3F &box, const VectorF &windVel)
{
   ...
   PROFILE_END(PrecipFindDropCutoff);
}
...
void Precipitation::createSplash(Raindrop *drop)
{
   ...
   PROFILE_END(PrecipCreateSplash);
}

void Precipitation::destroySplash(Raindrop *drop)
{
   PROFILE_SCOPE(PrecipDestroySplash);
   if (drop == mSplashHead)
   {
      mSplashHead = NULL;
      return;
...
void Precipitation::interpolateTick(F32 delta)
{
   ...
   if (mLastRenderFrame != SceneGraph::smFrameKey)
      return;
...
void Precipitation::processTick(const Move *)
{
   ...
   MatrixF camMat(true);
   conn->getControlCameraTransform(0.032f, &camMat);

   F32 camFov = 90.0f;
#ifdef NTJ_EDITOR
   camFov = dWorldEditor::GetCameraFov();
#endif
#ifdef NTJ_CLIENT
   camFov = dGuiMouseGamePlay::GetCameraFov();
#endif
   ...
   while (curr)
   {
      ...
      if (curr->valid && curr->position.z < curr->hitPos.z)
      {
         ...
         if (curr->hitType & (GameObjectType | VehicleObjectType))
         {
...
void Precipitation::renderObject(SceneState* state, RenderInst *ri)
{
...
#ifdef STATEBLOCK
	AssertFatal(mSetSB, "Precipitation::renderObject -- mSetSB cannot be NULL.");
	mSetSB->apply();
#else
   // Setup render state
   ...
   if (mDropShader || mSplashShader) 
   {
      ...
      GFX->setVertexShaderConstF( 4, (float*)&fadeStartEnd, 1, 2 );
      GFX->setVertexShaderConstF( 5, (float*)&camPos, 1, 3 );
      GFX->setVertexShaderConstF( 6, (float*)&ambient, 1 );
    }
   ...
   else
   {
      ...
      GFX->disableShaders();
#ifdef STATEBLOCK
		AssertFatal(mSetFixSB, "Precipitation::renderObject -- mSetFixSB cannot be NULL.");
		mSetFixSB->apply();
#else
...
#ifdef STATEBLOCK
	AssertFatal(mClearSB, "Precipitation::renderObject -- mClearSB cannot be NULL.");
	mClearSB->apply();
#else
   ...
   mLastRenderFrame = SceneGraph::smFrameKey+1;

   GFX->popWorldMatrix();
}


void Precipitation::resetStateBlock()
{
	//mSetSB
	GFX->beginStateBlock();
...
void Precipitation::releaseStateBlock()
{
	if (mSetSB)
	{
		mSetSB->release();
...
void Precipitation::init()
{
	if (mSetSB == NULL)
	{
		...
		mSetFixSB = new GFXD3D9StateBlock;
		mClearSB = new GFXD3D9StateBlock;
		resetStateBlock();
	}
}

void Precipitation::shutdown()
{
	SAFE_DELETE(mSetSB);
	SAFE_DELETE(mSetFixSB);
