...
//---------------------------------------------------------------------------
//////#define STATEBLOCK
GFXStateBlock* fxLight::mBlendZeroSB = NULL;
GFXStateBlock* fxLight::mBlendOneSB = NULL;
GFXStateBlock* fxLight::mBlendTwoSB = NULL;
GFXStateBlock* fxLight::mTexOpSB = NULL;
GFXStateBlock* fxLight::mBlendTexSB = NULL;
GFXStateBlock* fxLight::mClearSB = NULL;
//------------------------------------------------------------------------------
//
...
char* fxLight::smIconTextureFilename = FXLIGHTDBICONTEXTURE;
...
void fxLight::initPersistFields()
{
	...
	removeField("dataBlockName"); //Ray: 使用datablock
...
void fxLight::consoleInit()
{
   Parent::consoleInit();
   Con::addVariable("fxLight::IconName", TypeString, &smIconTextureFilename);
}
...
void fxLight::OnSerialize( CTinyStream& stream )
{
...
void fxLight::OnUnserialize( CTinyStream& stream )
{
...
void fxLight::ResetAnimation(void)
{
	...
	// Reset Last Animation Time.
	mLastAnimateTime = mLastRenderTime = Platform::getVirtualMilliseconds();
...
{
   PROFILE_SCOPE(fxLight_onNewDataBlock);
	// Cast new Datablock.
...
bool fxLight::onAdd()
{
	...
	{
		// Fetch Textures.
		mIconTextureHandle = GFXTexHandle(smIconTextureFilename, &GFXDefaultStaticDiffuseProfile);
		setFlareBitmap(mDataBlock->mFlareTextureName);
...
void fxLight::inspectPostApply()
{
	...
	// Fetch Icon Texture.
	if (isServerObject()) mIconTextureHandle = GFXTexHandle(smIconTextureFilename, &GFXDefaultStaticDiffuseProfile);
...
void fxLight::renderObject(SceneState* state, RenderInst *ri)
{
	...
	{
	case 0:
		AssertFatal(mBlendZeroSB, "fxLight::renderObject -- mBlendZeroSB cannot be NULL.");
		mBlendZeroSB->apply(); 
		break;
	case 1:
		AssertFatal(mBlendOneSB, "fxLight::renderObject -- mBlendOneSB cannot be NULL.");
		mBlendOneSB->apply(); 
		break;
	case 2:
		AssertFatal(mBlendTwoSB, "fxLight::renderObject -- mBlendTwoSB cannot be NULL.");
		mBlendTwoSB->apply(); 
		break;
	...
	if (mDataBlock->mFlareOn && mEnable)
	{
		...
		// Modulate Texture.
		AssertFatal(mTexOpSB, "fxLight::renderObject -- mTexOpSB cannot be NULL.");
		mTexOpSB->apply(); 
		Point3F ScreenPoint;
		Point3F ObjectPoint;
		...
		MathUtils::projectWorldToScreen(ObjectPoint,ScreenPoint,viewport,ModelView,projection);
		if(true)
		{
			// Fetch Eye Position.
		...
		if (mDataBlock->mLinkFlare)
		{
			...
			ColorF flare_clr = mAnimationColour;
			flare_clr.clamp();
			PrimBuild::color3f(flare_clr.red, flare_clr.green, flare_clr.blue);
		}
		else
		{
	...
	else
	{
		...
		// Icon Blend Function.
		AssertFatal(mBlendTexSB, "fxLight::renderObject -- mBlendTexSB cannot be NULL.");
		mBlendTexSB->apply(); 
	...
	if (BBRadius > 0)
	{
		// Render flare billboard with correct rotation and orientation.
		if (mDataBlock->mUseRotation)
		{
	...
	AssertFatal(mClearSB, "fxLight::renderObject -- mClearSB cannot be NULL.");
	mClearSB->apply(); 
...
bool fxLight::TestLOS(const Point3F& ObjectPosition, SceneObject* AttachedObj)
{
	...
	const U32 ObjectMask = GameObjectType | VehicleObjectType | CameraObjectType;
	...
	ShapeBase* ControlObj = dynamic_cast<ShapeBase*>(con->getControlObject());
...
U64 fxLight::packUpdate(NetConnection * con, U64 mask, BitStream * stream)
{
	// Pack Parent.
...
{	
	...
	GFX->endStateBlock(mBlendZeroSB);
...
void fxLight::releaseStateBlock()
{
	if (mBlendZeroSB)
	{
		mBlendZeroSB->release();
	}
...
void fxLight::init()
{
	if (mBlendZeroSB == NULL)
	{
		mBlendZeroSB = new GFXD3D9StateBlock;
		mBlendZeroSB->registerResourceWithDevice(GFX);
		mBlendZeroSB->mZombify = &releaseStateBlock;
		mBlendZeroSB->mResurrect = &resetStateBlock;

		mBlendOneSB = new GFXD3D9StateBlock;
		mBlendTwoSB = new GFXD3D9StateBlock;
		mTexOpSB = new GFXD3D9StateBlock;
		mBlendTexSB = new GFXD3D9StateBlock;
...
void fxLight::shutdown()
{
	SAFE_DELETE(mBlendZeroSB);
	SAFE_DELETE(mBlendOneSB);
	SAFE_DELETE(mBlendTwoSB);
	SAFE_DELETE(mTexOpSB);
	SAFE_DELETE(mBlendTexSB);
