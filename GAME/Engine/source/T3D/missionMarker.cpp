...
U64 MissionMarker::packUpdate(NetConnection * con, U64 mask, BitStream * stream)
{
...
U64 WayPoint::packUpdate(NetConnection * con, U64 mask, BitStream * stream)
{
   U64 retMask = Parent::packUpdate(con, mask, stream);
...
U64 SpawnSphere::packUpdate(NetConnection * con, U64 mask, BitStream * stream)
{
   U64 retMask = Parent::packUpdate(con, mask, stream);
...
U64 CameraBookmark::packUpdate(NetConnection * con, U64 mask, BitStream * stream)
{
   U64 retMask = Parent::packUpdate(con, mask, stream);
   if(stream->writeFlag(mask & UpdateNameMask))
      stream->writeString(mName);
