...
U32 AIClient::getMoveList( Move **movePtr,U32 *numMoves ) {
   ...
   *numMoves = 1;
   ...
   case ModeMove:
      ...
      Point3F headRotation(0,0,0); //= mPlayer->getHeadRotation();
      curYaw = rotation.z;
      ...
      if( !isZero( horzDist ) ) {
         ...
         newPitch = mAtan( horzDist, vertDist ) - ( M_2PI_F / 2.0f );
...
ConsoleFunction( aiAddPlayer, S32 , 2, 3, "aiAddPlayer( 'playerName'[, 'AIClassType'] );" ) {
   ...
   S32 nameLen = dStrlen( argv[1] ) + 1;
   S32 nsLen = dStrlen( argv[2] ) + 1;
   char *name = new char[nameLen];
   char *ns = new char[nsLen];

   dStrcpy( name, nameLen, argv[1] );
   dStrcpy( ns, nsLen, argv[2] );
