...
//------------------------------------------------------------------------------
ConsoleFunction(setPowerAudioProfiles, void, 3, 3, "( SFXProfile powerUp, SFXProfile powerDown)"
                "Set the ambient audio manager's power up/down profiles.")
{
   gAmbientAudioManager.mPowerUpProfile = dynamic_cast<SFXProfile*>(Sim::findObject(argv[1]));
   gAmbientAudioManager.mPowerDownProfile = dynamic_cast<SFXProfile*>(Sim::findObject(argv[2]));
}
...
void AmbientAudioManager::update()
{
   ...
   if(interior)
   {
      ...
      {
         SFXProfile * profile = interior->getAudioProfile();
         if(profile)
         {
