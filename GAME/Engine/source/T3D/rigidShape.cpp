...
//---------------------------------------------------------------------------
//////#define STATEBLOCK
GFXStateBlock* RigidShape::mSetSB = NULL;
GFXStateBlock* RigidShape::mClearSB = NULL;
...
namespace {
   ...
   const U32 sCollisionMoveMask = (TerrainObjectType        | InteriorObjectType   |
      GameObjectType           | StaticTSObjectType   |
...
void RigidShapeData::initPersistFields()
{
   ...
   addField("softImpactSound", TypeSFXProfilePtr, Offset(body.sound[Body::SoftImpactSound], RigidShapeData));
   addField("hardImpactSound", TypeSFXProfilePtr, Offset(body.sound[Body::HardImpactSound], RigidShapeData));
   ...
   addField("exitingWater",      TypeSFXProfilePtr,   Offset(waterSound[ExitWater],   RigidShapeData));
   addField("impactWaterEasy",   TypeSFXProfilePtr,   Offset(waterSound[ImpactSoft],   RigidShapeData));
   addField("impactWaterMedium", TypeSFXProfilePtr,   Offset(waterSound[ImpactMedium],   RigidShapeData));
   addField("impactWaterHard",   TypeSFXProfilePtr,   Offset(waterSound[ImpactHard],   RigidShapeData));
   addField("waterWakeSound",    TypeSFXProfilePtr,   Offset(waterSound[Wake],   RigidShapeData));
...
void RigidShape::getCameraTransform(F32* pos,MatrixF* mat)
{
   ...
   if (mContainer->castRay(sp, ep,
      ~(WaterObjectType | GameBaseObjectType | DefaultObjectType, getLayerID() ),
      &collision) == true) 
   {
...
U64 RigidShape::packUpdate(NetConnection *con, U64 mask, BitStream *stream)
{
...
void RigidShape::updateLiftoffDust( F32 dt )
{
   ...
   RayInfo rayInfo;
   if( !getContainer()->castRay( startPos, endPos, AtlasObjectType | TerrainObjectType, &rayInfo, getLayerID() ) )
   {
      return;
   }
...
bool RigidShape::collidingWithWater( Point3F &waterHeight )
{
   ...
   RayInfo rInfo;
   if( gClientContainer.castRay( curPos + Point3F(0.0, 0.0, height), curPos, WaterObjectType, &rInfo, getLayerID() ) )
   {
...
//-----------------------------------------------------------------------------
//
void RigidShape::renderImage(SceneState *state)
{
   ...
   if (gShowBoundingBox) 
   {
...
#ifdef STATEBLOCK
		AssertFatal(mSetSB, "RigidShape::renderImage -- mSetSB cannot be NULL.");
		mSetSB->apply();  
		GFX->pushWorldMatrix();
      ...
      GFX->getDrawUtil()->drawWireCube(Point3F(0.1f,0.1f,0.1f),mDataBlock->massCenter, ColorI(255, 255, 255));
      ...
      for (int i = 0; i < mCollisionList.getCount(); i++)
      {
         ...
         GFX->getDrawUtil()->drawWireCube(Point3F(0.05f,0.05f,0.05f),collision.point, ColorI(0, 0, 255));
      }
      ...
      polyList.render();
#ifdef STATEBLOCK
		AssertFatal(mClearSB, "RigidShape::renderImage -- mClearSB cannot be NULL.");
		mClearSB->apply();  
#else
		//
		GFX->setZEnable(true);
...
void RigidShape::resetStateBlock()
{
	//mSetSB
	GFX->beginStateBlock();
...
void RigidShape::releaseStateBlock()
{
	if (mSetSB)
	{
		mSetSB->release();
