...
// This include might be unnecessary, search CodeReview below. [4/23/2007 justind]
...
{
   if (!isGhost())
      Con::executef(mDataBlock, "onNode",scriptThis(), Con::getIntArg(node));
}
...
{
   // CodeReview : This include is causing dependency on the editor headers...
   //  It seems this is not needed because of the lack of rendering.  
   //  I hesitate to remove this just incase it's just simply not been ported.  
   //  Can someone confirm or deny this as being useful? [4/23/2007 justind]
   if (gEditingMission)
   {
/*
      glPushMatrix();
...
U64 PathCamera::packUpdate(NetConnection *con, U64 mask, BitStream *stream)
{
