...
bool HoverVehicleData::preload(bool server, char errorBuffer[256])
{
   ...
   for (S32 j = 0; j < MaxJetNodes; j++)
      jetNode[j] = -1;//shape->findNode(sJetNode[j]);
...
U64 HoverVehicle::packUpdate(NetConnection* con, U64 mask, BitStream* stream)
{
...
void HoverVehicle::updateForces(F32 /*dt*/)
{
   ...
   for (j = 0; j < 2; j++) {
      if (getContainer()->castRay(stabPoints[j].wsPoint, stabPoints[j].wsPoint + stabPoints[j].wsExtension * 2.0,
                                  ...
                                  InteriorObjectType | WaterObjectType, &rinfo, getLayerID())) 
      {
...
void HoverVehicle::updateDustTrail( F32 dt )
{
   ...
   RayInfo rayInfo;
   if( !getContainer()->castRay( startPos, endPos, TerrainObjectType, &rayInfo, getLayerID()) )
   {
      return;
   }
      ...
      if (active) {
         if (mDataBlock->jetNode[j] != -1) {
            ...
            mat.mul(getRenderTransform(),
                    mShapeInstance->getNodeTransforms()[mDataBlock->jetNode[j]]);
