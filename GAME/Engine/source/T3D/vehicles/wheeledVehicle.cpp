...
static U32 sClientCollisionMask =
      ...
      GameObjectType       | StaticShapeObjectType    |
...
bool WheeledVehicleData::preload(bool server, char errorBuffer[256])
{
   ...
   //         si->getNodeTransforms()[wp->springNode].getColumn(3, &wp->pos);
   //      else {
   ...
   //         si->animate();
   //         si->getNodeTransforms()[wp->springNode].getColumn(3, &downPos);
...
void WheeledVehicleData::initPersistFields()
{
   ...
   addField("brakePowerEngine", TypeF32, Offset(brakePowerEngine, WheeledVehicleData));
}
...
void WheeledVehicle::updateForces(F32 dt)
{
   ...
   if (mBraking) 
   {
      brakeVel = (mDataBlock->brakePowerEngine / aMomentum) * dt;
      engineTorque = 0;
   }
   else 
   {
...
void WheeledVehicle::extendWheels(bool clientHack)
{
   ...
   for (Wheel* wheel = mWheel; wheel < wend; wheel++) 
   {
      if (wheel->tire && wheel->spring) 
      {
         ...
         RayInfo rInfo;
         if (mContainer->castRay(sp, ep, sClientCollisionMask & ~GameObjectType, &rInfo, getLayerID())) 
         {
...
U64 WheeledVehicle::packUpdate(NetConnection *con, U64 mask, BitStream *stream)
{
