...
//---------------------------------------------------------------------------
//////#define STATEBLOCK
GFXStateBlock* Vehicle::mSetSB = NULL;
GFXStateBlock* Vehicle::mClearSB = NULL;
//----------------------------------------------------------------------------
...
void Vehicle::updateMove(const Move* move)
{
   ...
   // Image Triggers
   #pragma message(ENGINE(精简了MountedImage))
...
U64 Vehicle::packUpdate(NetConnection *con, U64 mask, BitStream *stream)
{
...
void Vehicle::mountObject(ShapeBase* obj, StringTableEntry node)
{
...
void Vehicle::renderImage(SceneState *state)
{
   ...
   {
#ifdef STATEBLOCK
		AssertFatal(mSetSB, "Vehicle::renderImage -- mSetSB cannot be NULL.");
		mSetSB->apply();
		GFX->pushWorldMatrix();
      ...
      polyList.render();
#ifdef STATEBLOCK
		AssertFatal(mClearSB, "Vehicle::renderImage -- mClearSB cannot be NULL.");
		mClearSB->apply();
#else
      //
      GFX->setZEnable(true);
...
void Vehicle::releaseStateBlock()
{
	if (mSetSB)
	{
		mSetSB->release();
