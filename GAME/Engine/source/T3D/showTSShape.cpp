...
void showUpdateThreadControl()
{
   ...
   threadList->setCellSize(Point2I(threadList->getWidth(),16));
   ...
   sequenceList->setCellSize(Point2I(sequenceList->getWidth(),16));
...
ShowTSShape::ShowTSShape(const char * shapeName)
{
   ...
   shapeInstance = NULL;
   dStrcpy(shapeNameBuffer, sizeof(shapeNameBuffer), shapeName);
   timeScale = 1.0f;
   addGround = false;
   stayGrounded = true;
   char fileBuffer[512];
   dStrcpy(fileBuffer, sizeof(fileBuffer), shapeName);

   // before loading shape, execute associated script
   char *ext = dStrstr( static_cast<char*>( fileBuffer ), const_cast<char*>( ".mod" ) );
   if (ext)
   {
      ext[1]='c';
      ext[2]='s';
      ext[3]='\0';
      Con::executef("exec",fileBuffer);
      ext[1]='d';
      ext[2]='t';
      ext[3]='s';
   }
   ...
   char matPath[4096];
   dStrcpy(matPath, sizeof(matPath), shapeName);
   char* lastSlash = dStrrchr(matPath, '/');
   if (lastSlash)
   {
      dStrcpy(lastSlash, sizeof(matPath) - (lastSlash - matPath), "/materials.cs");
      Con::executef("exec",matPath);
   }
...
{
   // setup light
   LightInfo *sunlight = gClientSceneGraph->getLightManager()->getSpecialLight(LightManager::slSunLightType);
   ...
   sunlight->mBackColor.set( 0.0f, 0.0f, 0.0f ); // ´òÉÏ±³¹â
...
{
   MatrixF xRot, zRot;
   xRot.set(EulerF(camRot.x, 0.0f, 0.0f));
   zRot.set(EulerF(0.0f, 0.0f, camRot.z));
