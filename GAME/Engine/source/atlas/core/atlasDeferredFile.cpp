...
{
   PROFILE_SCOPE(AtlasDeferredFile_hasPendingIO);
   if(isLoading())
   {
      return true;
   }
...
void AtlasDeferredFile::sync()
{
   ...
   PROFILE_END(AtlasDeferredFile_sync_lock);

   PROFILE_START(AtlasDeferredFile_sync_process);
   // Now process results.
   for(S32 i=0; i<resCount; i++)
      resData[i]->complete();
   PROFILE_END(AtlasDeferredFile_sync_process);
}
...
void AtlasDeferredFile::ensureStreamWritable()
{
   ...
   Con::printf("AtlasDeferredFile::ensureStreamWritable - switching file into writable mode... This may cause a crash if you have other copies of PowerEngine accessing it!");
