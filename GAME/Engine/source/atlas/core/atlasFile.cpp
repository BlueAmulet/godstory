...
ResourceInstance *constructAtlasFileResource(Stream &stream, ResourceObject *)
{
...
Resource<AtlasFile> AtlasFile::load(const char *filename)
{
   ...
   if(!af->hasStream())
   {
      Con::printf("AtlasFile::load - loading Atlas resource 0x%p with %s", (AtlasFile*)af, filename);
      if(!af->open(filename))
         Con::errorf("AtlasFile::load - cannot open atlas file '%s'.", filename);
   }
...
AtlasFile::AtlasFile()
{
   ...
   mDeserializerSemaphore = new Semaphore(0);
}
...
void AtlasFile::syncThreads()
{
   ...
   PROFILE_END(AtlasFile_syncThreads_callback);
   ...
   PROFILE_END(AtlasFile_syncThreads_notes);
   ...
   // Pass stuff to the TOCs for instatement.
   {
      ...
      for(S32 i=0; i<pendingCount; i++)
      {
         ...
         PROFILE_END(AtlasFile_syncThreads_instatement_callback);
   ...
   PROFILE_END(AtlasFile_syncThreads_instatement);

   // Make sure we're loading.
   enqueueNextPendingLoad(false);

   PROFILE_END(AtlasFile_syncThreads);
}
...
void AtlasFile::deserializerThread()
{
   while(true)
   {
      ...
      mDeserializerSemaphore->acquire(true);
...
{
   PROFILE_SCOPE(AtlasFile_enqueueNextPendingLoad);
   ...
   // Throttle the threaded part of the pipeline.
   ...
   if(arn)
   {
      ...
      PROFILE_END(AtlasFile_enqueueNextPendingLoad_arn);
   }
...
void AtlasFile::waitForPendingWrites()
{
   ...
   {
      Con::printf("AtlasFile (0x%p) - Waiting for write IO to finish...", this);
      Platform::sleep(32);
   }
...
{
   status.mLoadQueueLength = mPendingLoadQueue.lockVector().size();
   mPendingLoadQueue.unlockVector();

   status.mDeserializeQueueLength = mPendingDeserializeQueue.lockVector().size();
   mPendingDeserializeQueue.unlockVector();

   status.mInstatementQueueLength = mPendingProcessQueue.lockVector().size();
   mPendingProcessQueue.unlockVector();

   U32 numTOCs = mTOCs.size();
   status.mTOCStats.reserve( numTOCs );
   status.mTOCStats.clear();

   status.mNumRequests = 0;
   status.mNumRequestsDOA = 0;
   status.mBytesLoading = 0;
   status.mBytesPending = 0;

   for( U32 i = 0; i < mTOCs.size(); ++ i )
   {
      IOStatus::TOCIOStatus tocStatus;
      mTOCs[ i ]->getIOStatus( tocStatus );
      status.mTOCStats.push_back( tocStatus );

      status.mNumRequests += tocStatus.mLoadQueueLength;
      status.mNumRequestsDOA += tocStatus.mNumChunksDeadOnArrival;
      status.mBytesLoading += tocStatus.mBytesLoading;
      status.mBytesPending += tocStatus.mBytesPending;
   }
}
