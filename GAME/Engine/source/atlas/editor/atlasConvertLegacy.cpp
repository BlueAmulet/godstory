                ...
                "output directory.")
{
   ...
   ResourceManager->getPaths(terrObj->getFile()->mSourceResource->getFullPath(), terrFilePath, terrFileName);

   for(S32 i=0; i<TerrainBlock::MaterialGroups; i++)
   {
      // Only count alpha maps with specified textures.
      if(!terrObj->mMaterialFileName[i] || !terrObj->mMaterialFileName[i][0])
         continue;
