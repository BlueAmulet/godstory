...
static U8 gPacketBuffer[Net::MaxPacketBuffSize];
...
void BitStream::sendPacketStream(const NetAddress *addr, int udpS )
{
	Net::sendtoEx(addr, udpS, gPacketStream.getBuffer(), gPacketStream.getPosition());
}
...
void BitStream::setBuffer(void *bufPtr, S32 size, S32 maxSize)
{
   ...
   bitsLimite = Net::LimitedPacketDataSize<<3; //Ray:设定的阀值 /*(bufSize/2)<<3;*/
   error = false;
...
static U32 gBitCounts[8] = {
   1, 10, 13, 14, 15, 16, 20, 32
};
...
void BitStream::readCompressedPoint(Point3F* p,F32 scale)
{
   ...
   if(type==0)
   {
	   p->x = mCompressPoint.x;
	   p->y = mCompressPoint.y;
	   p->z = mCompressPoint.z;
...
{
	AssertFatal(maxSize<=256, "BitStream::readString maxSize>256");
   ...
   if(stringBuffer)
   {
      if(readFlag())
      {
         ...
         dStrcpy(buf, maxSize, stringBuffer);
         return;
      }
   }
   HuffmanProcessor::g_huffProcessor.readHuffBuffer(this, buf);
   if(stringBuffer)
      dStrcpy(stringBuffer, maxSize, buf);
}
...
void BitStream::writeString(const char *string, S32 maxLen)
{
   ...
   if(stringBuffer)
   {
      ...
      for(j = 0; j < maxLen && stringBuffer[j] == string[j] && string[j];j++)
         ;
      dMemcpy(stringBuffer, string, len+1);
