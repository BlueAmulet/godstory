...
#include "core/zip/zipStatFilter.h"
...
bool ZipArchive::readCentralDirectory()
{
   ...
   SAFE_DELETE(mRoot);
   mRoot = new ZipEntry;
   mRoot->mName = StringTable->insert("");
...
{
   char path[1024];
   dStrncpy(path, sizeof(path), ze->mCD.mFilename, sizeof(path));
   ...
   do
   {
      ...
      if(slash)
      {
         ...
         *slash = 0;
         ...
         ZipEntry *newEntry;
         if((newEntry = root->mChildren.retreive(ptr)) == NULL)
         {
...
{
   char path[1024];
   dStrncpy(path, sizeof(path), filename, sizeof(path));
   path[sizeof(path) - 1] = 0;
   ...
   do
   {
      ...
      else
      {
         // Find the file
         ZipEntry *ze;
         if((ze = root->mChildren.retreive(ptr)) != NULL)
            return ze;
      }
   } while(slash);
...
bool ZipArchive::rebuildZip()
{
  ...
  if(mMode == ReadWrite)
  {
   ...
   if(mMode == ReadWrite)
   {
      ...
      if(! dFileRename(mFilename, oldRename))
         return false;

      if(! dFileRename(newZipName, mFilename))
         return false;

      dFileDelete(oldRename);
   }
   return true;
}
