...
void* FrameAllocator::alloc(const U32 allocSize)
{
   ...
   smWaterMark = ( smWaterMark + ( POWER_BYTE_ALIGNMENT - 1 ) ) & (~( POWER_BYTE_ALIGNMENT - 1 ));

   // Sanity check.
   AssertFatal( !( smWaterMark & ( POWER_BYTE_ALIGNMENT - 1 ) ), "Frame allocation is not on a 4-byte boundry." );
...
public:
   ...
   {
	   U8* pBuffer = (U8*)(FrameAllocator::alloc(allocSize));
	   memset( pBuffer, 0, allocSize );// <Edit>:[soar]:修改了调用之后没清空导致申请空间大小出错的问题
      return pBuffer;
   }
};
