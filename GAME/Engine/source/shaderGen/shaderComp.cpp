...
Var * ConnectorStruct::getElement( RegisterType type )
{
   switch( type )
   {
      ...
      case RT_COLOR:
      {
		 ...
		 char out[32];
		 dSprintf( (char*)out, sizeof(out), "COLOR%d", mCurColorElem );
		 newVar->setConnectName( out );
		 newVar->constNum = mCurColorElem;
		 mCurColorElem++;
         return newVar;
      }
...
void ConnectorStruct::sortVars()
{
   ...
   for( U32 i=0; i<mElementList.size(); i++ )
   {
	  ...
	  if( dStrstr( (const char *)var->connectName, "COLOR" ) )
	  {
		  colorList.push_back( var );
	  }
   }
   ...
   //Ray: 多颜色参数排序
   for( U32 i=0; i<colorList.size(); i++ )
   {
	   char out[32];
	   dSprintf( (char*)out, sizeof(out), "COLOR%d", i );
	   colorList[i]->setConnectName( out );
   }
   if( colorList.size() )
   {
	   U32 index = 0;
	   ...
	   for( U32 i=0; i<mElementList.size(); i++ )
	   {
		   ...
		   if( dStrstr( (const char *)var->connectName, "COLOR" ) )
		   {
			   mElementList[i] = colorList[index];
			   index++;
		   }
	   }
   }
...
{
   dStrcpy( (char*)mName, sizeof(mName), newName );
}
