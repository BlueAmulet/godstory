                              ...
                              GFXShaderFeatureData &fd )
{
   ...
   if(fd.features[GFXShaderFeatureData::DynamicLight])
   {
	   ...
	   {
         Var *objnorm = new Var();
         objnorm->setName("objnorm");
         objnorm->setType("float3");
         LangElement *objnormdec = new DecOp(objnorm);

         meta->addStatement(new GenOp("   @ = normalize(@.xyz - @.xyz);\r\n", objnormdec, lightpos, inpos));
         meta->addStatement(new GenOp("   @.xyz = @;\r\n", lightVec, objnorm));

		  if(fd.features[GFXShaderFeatureData::DynamicLightAttenuateBackFace] &&
             dlightcoord && N && (GFX->getPixelShaderVersion() >= 2.0))
                            ...
                            GFXShaderFeatureData &fd )
{
   ...
   if( GFX->getPixelShaderVersion() < 2.0 && !fd.features[GFXShaderFeatureData::DynamicLight] )
   {
                           ...
                           GFXShaderFeatureData &fd )
{
	   ...
	   (!fd.features[GFXShaderFeatureData::DynamicLight]))
   {
      if(fd.features[GFXShaderFeatureData::DynamicLight])
      {
         Var *normcube = new Var;
         normcube->setType("samplerCUBE");
         normcube->setName("normcube");
         normcube->uniform = true;
         normcube->sampler = true;
         normcube->constNum = Var::getTexUnitNum();

         meta->addStatement(new GenOp("   @ = saturate( dot(@.xyz * 2.0 - 1.0, texCUBE(@, @).xyz * 2.0 - 1.0) );\r\n", 
                                  bumpDotDecl, bumpNorm, normcube, lightVec));
      }
	  else
                           ...
                           U32 &texIndex )
{
   ...
   {
	   passData.texFlags[texIndex++] = Material::NormalizeCube;
   }
}
