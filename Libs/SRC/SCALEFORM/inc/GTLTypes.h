...
public:
    ...
    garray_base()
        : Buffer(0), Size_(0), BufferSize(0)     {   }
    garray_base(int size_hint)
        : Buffer(0), Size_(0), BufferSize(0)     { resize(size_hint);  }
    garray_base(const garray_base<C, Base>& a)
        : Buffer(0), Size_(0), BufferSize(0)     { operator=(a); }
    ~garray_base()
    { 
         clear();
         if (Buffer)
    ...
    C&  back()                  { return (*this)[Size_-1];   }
    const C&    back() const    { return (*this)[Size_-1];   }
    ...
    void    removeMultiple(UPInt index, UPInt num)
    {
        ...
        else
        {
            ...
            memmove(Buffer+index, Buffer+index+num, sizeof(C) * (Size_ - num - index));
            Size_ -= num;
        }
    }
    ...
    void    remove(UPInt index)
    {
        ...
        else
        {
            ...
            memmove(Buffer+index, Buffer+index+1, sizeof(C) * (Size_ - 1 - index));
            Size_--;
        }
    }
    ...
    void    insert(UPInt index, const C& val = C())
    {
        ...
        if (index < Size_ - 1)
        {
            ...
            memmove(Buffer+index+1, Buffer+index, sizeof(C) * (Size_ - 1 - index));
        }
    ...
    void    insertMultiple(UPInt index, UPInt num, const C& val = C())
    {
        ...
        {
            // is it safe to use memmove?
            memmove(Buffer+index+num, Buffer+index, sizeof(C) * (Size_ - num - index));
        }
    ...
    public:
        ...
        const_iterator(const const_iterator& it) : pArray(it.pArray), CurIndex(it.CurIndex) {}
...
public:
    ...
    void    resize_impl(UPInt new_size)
    {
        ...
        else if (this->Size_ <= bufferSize && this->Size_ > bufferSize >> 1)
        {
            // don't compact yet.
            // GASSERT(Buffer != 0);
        }
        ...
        {
            UPInt new_buffer_size = this->Size_ + (this->Size_ >> 2);
...
template <class Comparator, class Container, class Comparable>
UPInt find_lower_bound(const Container& arr, const Comparable &e, const Comparator& cmp = Comparator())
{
    SPInt len   = (SPInt)arr.size();
    SPInt first = 0;
    SPInt half;
    SPInt middle;
...
{
    UPInt idx = find_lower_bound(c, e, cmp);
    if (idx == c.size())
        return SPInt(idx) - 1;
    return SPInt(idx);
}
...
struct gdeleter
{
    static void destroy(C* object) { delete object; }
};

// Functor template for deleting array of objects.
template<class C>
struct garray_deleter
{
    static void destroy(C* object) { delete[] object; }
};
...
// ***** gauto_ptr
//
// The main difference of this calls from STL::auto_ptr is 
// that it can be put in containers (garray, ghash) 

template<class C, class D = gdeleter<C> >
class gauto_ptr
{
public:

    gauto_ptr(C* p = 0) : pPtr(p), Owner(true) {}
    gauto_ptr(C* p, bool owner) : pPtr(p), Owner(owner) {}
    gauto_ptr(const gauto_ptr<C, D>& p) : pPtr(0), Owner(p.Owner) { pPtr = p.const_release(); } 
    ~gauto_ptr(void) { reset(); }
 
    gauto_ptr<C, D>& operator=(const gauto_ptr<C, D>& p)
    {
        if (this != &p) 
        {
            bool owner = p.Owner;
            reset(p.const_release());
            Owner = owner;
        }
        return *this;
    }
 
    gauto_ptr<C, D>& operator=(C* p)
    {
         reset(p);
         return *this;
    }

    operator bool(void) const {  return pPtr != 0; }    

    C& operator* () const { return *pPtr; }
    C* operator->() const { return pPtr; }

    C* get       () const { return  pPtr; }
    C* release()
    {
        Owner = false;
        return pPtr;
    }
    void reset(C* p = 0, bool owner = true)
    {
        if (pPtr != p) 
        {
            if (pPtr && Owner) 
...
private:
    ...
    C* const_release(void) const
    {
        return const_cast<gauto_ptr<C, D>*>(this)->release();
    }
};
