...
struct GFxXMLStringRef
{
    ...
    GFxXMLStringRef() 
        : Data(NULL), Length(0) {}
    GFxXMLStringRef(const char* data, UPInt len) 
        : Data(data), Length(len) {}
    GFxXMLStringRef(const char* data)
        : Data(data) 
    {
        Length = (data != NULL) ? gfc_strlen(data) : 0;
    }
};
