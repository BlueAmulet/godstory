...
// the GFx core.
//
class GFxASXMLFileLoader : public GRefCountBase<GFxASXMLFileLoader>
{
public:
    GFxASXMLFileLoader() { SetRefCountMode(GFC_REFCOUNT_THREADSAFE); }
    virtual         ~GFxASXMLFileLoader() {}

    //
    // Load a an xml file into a DOM tree using the provide file opener
    //
    virtual void    Load(const char* pfilename, GFxFileOpener* pfo) = 0;
...
//
// A seperate implementation is provided so that XML can be compiled
...
//
// Implementation of Set/GetXMLParserState members of the GFxSharedState class.
// They are defined here because the GFxXMLParserState interface is declared
// here.
//
inline void GFxSharedState::SetXMLSupport(GFxXMLSupportBase *ptr)         
{ 
    SetState(GFxState::State_XMLSupport, ptr); 
}
inline GPtr<GFxXMLSupportBase>  GFxSharedState::GetXMLSupport() const     
{ 
