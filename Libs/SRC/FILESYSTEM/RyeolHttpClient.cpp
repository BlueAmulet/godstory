	...
	throw (Exception &)
{
	...
	if ( !::HttpAddRequestHeadersA (
            ...
            HTTP_ADDREQ_FLAG_REPLACE// flags
			)
		) {
		SAFEFREE (szHeader) ;
	...
	throw (Exception &)
{
    ...
    {			// for the exception thrown by the new operator
		if ( bNeedToFreeGetUrl )	SAFEFREE (szGetUrl) ;
		HttpTool::CloseRequest (hRequest) ;
		if ( !bBorrowedConnection )	HttpTool::CloseRequest (hConnection) ;
		if ( !bBorrowedInternet )	HttpTool::CloseRequest (hInternet) ;
...
template <typename HttpTool, typename HttpEncoder>
void
CHttpClientT<HttpTool, HttpEncoder>::RequestGetEx(typename CHttpClientT<HttpTool, HttpEncoder>::CHttpResponse* resp)
{
    
    HINTERNET hRequest = resp->GetRequestHandle();
